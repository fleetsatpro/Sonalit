-- Sonalit intelligence schema reconciliation replay.
--
-- 112 was intentionally forward-only and idempotent, but its filename can already
-- exist in schema_migrations on installations where the physical DDL was lost or
-- never reached the same database. This migration deliberately replays ONLY the
-- non-destructive contract repair, under a new migration identity.
--
-- It is safe to run against a fully reconciled database: every operation is
-- IF EXISTS / IF NOT EXISTS and no existing data is dropped or rewritten.
...
VALUES (
  '20260908_113_intelligence_schema_reconciliation_replay',
  jsonb_build_object('mode','forward_only_replay','data_destructive',false)
)
