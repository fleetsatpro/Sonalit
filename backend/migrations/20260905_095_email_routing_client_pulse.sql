...
CREATE INDEX IF NOT EXISTS idx_email_routing_policies_org_event
  ON email_routing_policies (org_id, event_key, enabled, priority);
...
CREATE POLICY org_isolation_email_routing_policies ON email_routing_policies
...
CREATE INDEX IF NOT EXISTS idx_email_notifications_org_type_created
  ON email_notifications (org_id, notification_type, created_at DESC);
...
CREATE TABLE IF NOT EXISTS cds_client_pulse_runs (
  ...
  manifest_hash TEXT,
  email_notification_ids UUID[] NOT NULL DEFAULT '{}',
  error TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_cds_client_pulse_runs_org_created
  ON cds_client_pulse_runs (org_id, created_at DESC);
...
CREATE POLICY org_isolation_cds_client_pulse_runs ON cds_client_pulse_runs
  USING (org_id = NULLIF(current_setting('app.current_org_id', true), '')::UUID)
  WITH CHECK (org_id = NULLIF(current_setting('app.current_org_id', true), '')::UUID);
GRANT SELECT, INSERT, UPDATE, DELETE ON cds_client_pulse_runs TO sonalit_app;

-- Default policy is deliberately conservative: operational traffic stays on
-- notifications@; only explicitly classified security events use security@.
-- Client Pulse is its own scheduled event and never enters the alert stream.
