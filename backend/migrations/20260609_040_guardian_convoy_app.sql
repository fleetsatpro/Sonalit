...
-- org_id is denormalised (no FK to orgs) matching the codebase pattern
...
CREATE POLICY photo_uploads_org ON photo_uploads
...
CREATE POLICY convoy_reports_org ON convoy_reports
  USING (org_id = current_setting('app.current_org_id', true)::uuid);
...
CREATE POLICY seal_verifications_org ON seal_verifications
  USING (org_id = current_setting('app.current_org_id', true)::uuid);
...
CREATE POLICY sos_events_org ON sos_events
  USING (org_id = current_setting('app.current_org_id', true)::uuid);
