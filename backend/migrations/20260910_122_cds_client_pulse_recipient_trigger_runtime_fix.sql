-- Runtime repair for the already-applied 120 trigger function.
-- Migration 120 was previously recorded as applied before its UUID aggregate
-- correction reached production. Recreate the function in-place without using
-- MIN(uuid) or an ON CONFLICT target that is not guaranteed by the schema.
...
BEGIN
  ...
  IF v_old_customer_id IS NOT NULL
     AND v_old_customer_id IS DISTINCT FROM NEW.cds_customer_id THEN
  ...
  IF NEW.enabled IS TRUE
     AND NEW.cds_client_pulse IS TRUE
     AND NEW.cds_customer_id IS NOT NULL THEN
...
-- Reconcile only stale routes; do not delete recipient/enrollment history.
UPDATE communication_subscriptions s
   SET enabled=FALSE,updated_at=NOW()
  FROM communication_enrollments e
 ...
 WHERE s.enrollment_id=e.id
   ...
   AND (r.deleted_at IS NOT NULL
     OR r.enabled IS NOT TRUE
     OR r.cds_client_pulse IS NOT TRUE
     OR r.cds_customer_id IS NULL
     OR e.cds_customer_id<>r.cds_customer_id);
