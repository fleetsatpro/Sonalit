...
const replacement = String.raw`function unzip(buffer) {
  ...
  // Last-resort recovery for malformed templates whose EOCD/central-directory
  // offsets are unusable. XLSX files normally contain complete local headers
  // with sizes, so the archive can be reconstructed without trusting EOCD.
  const recovered = [];
  let p = 0;
