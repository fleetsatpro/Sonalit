...
async function processEmail(job) {
  ...
  if (!claim.rows.length) return; const email = claim.rows[0];
  try {
    const result = await sendEmail({ from: email.sender || FROM, to: email.recipient, replyTo: email.reply_to || REPLY_TO, subject: email.subject, html: email.html_body, text: email.text_body, attachments: Array.isArray(email.attachments) ? email.attachments : [], idempotencyKey: email.idempotency_key, tags: [{ name: 'notification_type', value: email.notification_type }, { name: 'severity', value: email.severity }, { name: 'org_id', value: email.org_id }] });
...
async function dispatchPanicEmail(panicId) {
  ...
  if (!recipients.rows.length) { logger.error(`Panic email has no eligible security recipients: event=${panicId} org=${event.org_id} client=${event.client_id || 'unassigned'}`); return { queued: 0, reason: 'no_security_recipients' }; }
