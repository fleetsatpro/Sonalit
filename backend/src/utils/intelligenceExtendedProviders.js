...
function countriesFromWatchlists(watchlists) {
  ...
  const out = new Set(configured.map(v => String(v).toUpperCase()).filter(v => COUNTRY[v]));
  for (const w of watchlists || []) {
    const t = w.target || {};
    const raw = t.country_code || t.country || t.iso2;
    if (raw && COUNTRY[String(raw).toUpperCase()]) out.add(String(raw).toUpperCase());
    if (raw && ISO[String(raw)]) out.add(ISO[String(raw)]);
  }
...
async function collectAcled(countries) {
  ...
  try {
    const auth = await timeoutFetch('https://acleddata.com/oauth/token', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:new URLSearchParams({username,password,grant_type:'password',client_id:'acled',scope:'authenticated'}) });
    ...
    for (const cc of countries) {
      try {
        ...
        for (const x of d.data || []) {
          ...
          out.push({provider:'acled',external_id:`acled:${x.event_id_cnty}`,title:`ACLED ${x.event_type || 'event'} — ${x.location || COUNTRY[cc]}`,body,url:null,country_code:cc,latitude:x.latitude,longitude:x.longitude,published_at:x.event_date,credibility:Number(x.fatalities)>0?90:85,raw_metadata:{event_type:x.event_type,sub_event_type:x.sub_event_type,fatalities:x.fatalities,source:'acled'}});
        }
...
async function collectGdacs(countries) {
  try {
    ...
    for (const f of d.features || []) {
      ...
      const affected=(p.affectedcountries || []).map(x=>String(x.countrycode || x.iso3 || x.countryname || '').toUpperCase());
      const cc=countries.find(c=>affected.includes(c) || affected.some(a=>a.includes(c)));
      if (!cc) continue;
