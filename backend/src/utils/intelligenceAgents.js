...
async function publicationForCountry(orgId,country,type='daily'){
  ...
  const evidenceContract=evidenceCount>=3&&sourceCount.size>=2;const title=`${COUNTRY_NAMES[country]} Security Intelligence — ${type.toUpperCase()} Report`;
  ...
  const status=(evidenceContract&&board?board.agents.some(a=>a.id==='publication-qa'&&a.status==='complete'):evidenceContract)?'published':'draft';
