...
function isActiveRow(row) { const bs = String(row.booking_status || '').toLowerCase(); const cs = String(row.status || '').toLowerCase(); if (INACTIVE_BOOKING_STATUSES.has(bs)) return false; return ACTIVE_CONTAINER_STATUSES.has(cs) || !['delivered', 'completed'].includes(cs); }
...
async function fetchManifestSnapshot(orgId) {
  return withOrg(orgId, client => client.query(`
    SELECT bc.id, bc.booking_id, bc.container_number, bc.seal_number, bc.seal_number_2, bc.packing_list_no, bc.iso_type,
           ...
           b.booking_number, b.reference AS file_reference, b.vessel, b.commodity, b.controller, b.country_code, b.direction,
...
function excelValue(key, value) { if (value == null || value === '') return ''; if (['clamped_at','clamped_at_t','unclamped_at'].includes(key)) { const d = new Date(value); if (!Number.isNaN(d.getTime())) return d; } return value; }
async function buildManifestWorkbook(rows, snapshotAt) {
  const workbook = new ExcelJS.Workbook(); workbook.creator = 'Sonalit'; workbook.lastModifiedBy = 'Sonalit'; workbook.created = snapshotAt; workbook.modified = snapshotAt; workbook.properties.title = 'CDS Active Booking Manifest'; workbook.properties.subject = 'Sonalit CDS Client Pulse';
  ...
  for (const row of rows) { const data = {}; for (const [key] of MANIFEST_COLUMNS) data[key] = excelValue(key, row[key]); sheet.addRow(data); }
  sheet.getColumn('clamped_at').numFmt = 'dd/mm/yyyy'; sheet.getColumn('clamped_at_t').numFmt = 'hhmm"hrs"'; sheet.getColumn('unclamped_at').numFmt = 'hhmm"hrs"';
...
async function resolveClientPulseRecipients(orgId) {
  const result = await withOrg(orgId, client => client.query(`
    SELECT DISTINCT c.email, c.name
      FROM cargo_clients c JOIN client_notification_prefs p ON p.client_id=c.id AND p.org_id=c.org_id
     WHERE c.org_id=$1 AND c.deleted_at IS NULL
       AND ('cds_client_pulse'=ANY(p.events) OR 'client_pulse'=ANY(p.events))
       AND 'email'=ANY(p.channels)
       AND (p.convoy_id IS NULL OR EXISTS (SELECT 1 FROM cargo_client_links l WHERE l.client_id=c.id AND l.convoy_id=p.convoy_id AND l.org_id=$1))`, [orgId]));
  return result.rows;
}
...
async function generateAndQueueClientPulse(orgId, { snapshotAt = new Date() } = {}) {
  if (!orgId) throw new Error('orgId is required for Client Pulse'); const snapshot = snapshotAt instanceof Date ? snapshotAt : new Date(snapshotAt);
  ...
  if (!claim.rows.length) return { skipped:true, reason:'duplicate_snapshot' }; const runId=claim.rows[0].id;
  try {
    const active=(await fetchManifestSnapshot(orgId)).rows.filter(isActiveRow).map(manifestRow); const recipients=await resolveClientPulseRecipients(orgId); const activeBookingCount=new Set(active.map(r=>r.booking_number).filter(Boolean)).size;
    if (!recipients.length) { await query(`UPDATE cds_client_pulse_runs SET status='skipped',active_booking_count=$2,row_count=$3,updated_at=NOW() WHERE id=$1`,[runId,activeBookingCount,active.length]); return {skipped:true,reason:'no_opted_in_recipients',rows:active.length}; }
    const workbook=await buildManifestWorkbook(active,snapshot); if(!workbook.length) throw new Error('Generated Client Pulse workbook is empty'); const manifestHash=crypto.createHash('sha256').update(workbook).digest('hex');
    const stamp=snapshot.toISOString().replace(/[:]/g,'').replace(/\.\d{3}Z$/,'Z'); const filename=`CDS_Client_Pulse_Active_Bookings_${stamp}_EAT.xlsx`;
    const result=await queueClientPulseEmail({orgId,recipients:recipients.map(r=>r.email),snapshotAt:snapshot.toISOString(),activeBookingCount,dateLabel:dateLabel(snapshot),attachment:{filename,content:workbook.toString('base64'),contentType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'},correlationId:`cds-client-pulse:${runId}`,idempotencyKey});
  ...
  } catch(err) { await query(`UPDATE cds_client_pulse_runs SET status='failed',error=$2,updated_at=NOW() WHERE id=$1`,[runId,String(err.message||err).slice(0,4000)]); logger.error(`CDS Client Pulse failed: org=${orgId} run=${runId} error=${err.message}`); throw err; }
}
