...
function normalizeRecipients(recipients) {
  return [...new Set((Array.isArray(recipients) ? recipients : [recipients]).filter(Boolean).map(String).map(s => s.trim().toLowerCase()))];
}
...
async function enqueueEmail({ orgId, to, recipientName, notificationType, severity = 'normal', entityType, entityId, correlationId, idempotencyKey, templateData }) {
  ...
  if (!process.env.RESEND_API_KEY) {
    logger.error('RESEND_API_KEY is not configured; refusing to enqueue production email');
  ...
  for (const recipient of recipients) {
    const key = idempotencyKey || stableKey({ orgId, recipient, notificationType, entityType, entityId, subject: rendered.subject });
    const inserted = await withOrg(orgId, async client => client.query(
      `INSERT INTO email_notifications
       (org_id, recipient, recipient_name, notification_type, severity, subject, text_body, html_body, entity_type, entity_id, correlation_id, idempotency_key, status)
    ...
    await queue.add('email.send', { emailNotificationId: inserted.rows[0].id }, {
...
async function queueAlertEmail({ orgId, recipients, alert, correlationId, ctaUrl }) {
  return enqueueEmail({
    ...
    templateData: {
      alertType: alert.type, severity: alert.severity, vehicle: alert.registration,
      region: alert.region, convoy: alert.convoy_name, message: alert.message,
      time: alert.created_at, ctaLabel: 'Open alert', ctaUrl,
    },
  });
}
