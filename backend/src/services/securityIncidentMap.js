...
async function fetchWithTimeout(url) {
  ...
  try {
    return await fetch(url, { signal: controller.signal, headers: { 'User-Agent': 'Sonalit-SecurityMap/1.0' } });
...
function styleParts(style) {
  ...
  if (parts.length !== 2) return ['mapbox', 'streets-v12'];
  return parts;
}
...
function bbox(points) {
  ...
  const dx = Math.max(maxX - minX, 0.003);
  const dy = Math.max(maxY - minY, 0.003);
  minX -= dx * 0.35; maxX += dx * 0.35;
  minY -= dy * 0.35; maxY += dy * 0.35;
...
async function renderWithMapbox({ incident, trail, label }) {
  ...
  const path = `https://api.mapbox.com/styles/v1/${encodeURIComponent(username)}/${encodeURIComponent(styleId)}/static/geojson(${overlay})/${minLon},${minLat},${maxLon},${maxLat}/${MAP_WIDTH}x${MAP_HEIGHT}@2x.png`;
  const url = `${path}?access_token=${encodeURIComponent(token)}&logo=true`;
  const response = await fetchWithTimeout(url);
  const contentType = response.headers.get('content-type') || '';
  if (!response.ok || !contentType.includes('image/')) {
    const body = await response.text().catch(() => '');
    throw new Error(`Mapbox static map failed: ${response.status} ${body.slice(0, 240)}`);
  }
...
async function getIncidentContext(panicId) {
  ...
  const trail = await query(`SELECT latitude, longitude, heading, recorded_at
...
async function renderSecurityMap(panicId) {
  ...
  const points = trail.length ? trail : [incident];
  let image;
  try { image = await renderWithMapbox({ incident, trail: points, label: panic.vehicle_plate || panic.device_name || 'INCIDENT' }); }
  catch (error) {
    console.error(`Security incident map provider failure panic=${panicId}: ${error.message}`);
    throw error;
  }
  return image;
}
