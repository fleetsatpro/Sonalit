...
async function resolveIncidentMapContext(panicId) {
  const result = await query(`
    ...
    LEFT JOIN vehicles v ON v.id=d.assignment_id AND lower(COALESCE(d.assignment_type,'')) IN ('vehicle','fleet_vehicle') AND v.deleted_at IS NULL
  ...
  if (!event.convoy_id && event.vehicle_id) {
    ...
    `, [event.vehicle_id]);
    if (assignment.rows.length) { const convoy = assignment.rows[0]; Object.assign(event, { convoy_id: convoy.id, convoy_name: convoy.name, convoy_region: convoy.region, convoy_status: convoy.status, route_origin: convoy.route_origin, route_destination: convoy.route_destination }); }
  }
...
async function fetchTile(z, x, y) {
  ...
  const response = await fetch(`https://tile.openstreetmap.org/${z}/${wrappedX}/${y}.png`, { headers: { 'User-Agent': 'Sonalit-SecurityMap/1.0 (+https://sonalit.com)' } });
...
async function renderSecurityMap(panicId) {
  ...
  const composites = requests.map((r, i) => tiles[i].length ? { input: tiles[i], left: r.col * TILE_SIZE, top: r.row * TILE_SIZE } : null).filter(Boolean);
  const extractLeft = Math.max(0, Math.floor((centerX - cropOriginX) - MAP_WIDTH / 2));
  const extractTop = Math.max(0, Math.floor((centerY - cropOriginY) - MAP_HEIGHT / 2));
  const markerX = Math.round(centerX - cropOriginX - extractLeft); const markerY = Math.round(centerY - cropOriginY - extractTop);
  const label = String(event.vehicle_display || 'INCIDENT').slice(0, 32); const region = String(event.region || 'Unknown').slice(0, 42);
  ...
  const overlay = Buffer.from(`
    <svg width="${MAP_WIDTH}" height="${MAP_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="${MAP_WIDTH}" height="140" fill="url(#top)"/><rect x="24" y="22" width="430" height="74" rx="12" fill="#07111f" fill-opacity="0.88"/>
  ...
  return sharp({ create: { width: MAP_WIDTH, height: MAP_HEIGHT, channels: 4, background: '#dbeafe' } }).composite(composites)
