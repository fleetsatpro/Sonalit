...
function latToY(lat, zoom) {
  ...
  return (0.5 - Math.log((1 + sin) / (1 - sin)) / (4 * Math.PI)) * (2 ** zoom) * TILE_SIZE;
}
...
function mapUrlForPanic(panicId) {
  ...
  return `${base}/api/v1/public/security-map/${encodeURIComponent(panicId)}?token=${encodeURIComponent(token)}`;
}
...
async function renderSecurityMap(panicId) {
  ...
  const lat = clampLat(Number(event.lat));
  const lng = Number(event.lng);
  const zoom = Number(process.env.SECURITY_MAP_ZOOM) || DEFAULT_ZOOM;
  const centerX = lonToX(lng, zoom);
  const centerY = latToY(lat, zoom);
  const cols = Math.ceil(MAP_WIDTH / TILE_SIZE) + 2;
  const rows = Math.ceil(MAP_HEIGHT / TILE_SIZE) + 2;
  const startX = Math.floor(centerX / TILE_SIZE) - Math.floor(cols / 2);
  const startY = Math.floor(centerY / TILE_SIZE) - Math.floor(rows / 2);
  const cropOriginX = startX * TILE_SIZE;
  const cropOriginY = startY * TILE_SIZE;
  ...
  const markerX = Math.round(centerX - cropOriginX);
  const markerY = Math.round(centerY - cropOriginY);
  ...
  const overlay = Buffer.from(`
    <svg width="${MAP_WIDTH}" height="${MAP_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="top" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#07111f" stop-opacity="0.92"/><stop offset="1" stop-color="#07111f" stop-opacity="0"/></linearGradient>
        <filter id="shadow" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="3" stdDeviation="4" flood-opacity="0.45"/></filter>
      </defs>
      ...
      <circle cx="${markerX}" cy="${markerY}" r="24" fill="#ef4444" fill-opacity="0.20"/>
      <circle cx="${markerX}" cy="${markerY}" r="13" fill="#ef4444" stroke="#ffffff" stroke-width="4" filter="url(#shadow)"/>
      <circle cx="${markerX}" cy="${markerY}" r="4" fill="#ffffff"/>
