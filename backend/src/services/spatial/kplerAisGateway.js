...
async function queryProvider(bbox,maxRecords,signal){
  ...
  const query='query SonalitVessels($first:Int,$aoi:AreaOfInterest,$window:TimeRange){ vessels(first:$first,areaOfInterest:$aoi,lastPositionUpdate:$window){ pageInfo{hasNextPage endCursor} nodes{ id staticData{name mmsi imo shipType} lastPositionUpdate{timestamp updateTimestamp latitude longitude heading speed accuracy course collectionType navigationalStatus} } } }';
