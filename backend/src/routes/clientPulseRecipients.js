...
async function bindCdsRecipient(c, orgId, recipientId, cdsCustomerId) {
  ...
  let enrollment;
  if (existing.rows.length) enrollment = (await c.query(`UPDATE communication_enrollments SET status='active',contact_role='client_pulse',updated_at=NOW() WHERE id=$1 RETURNING *`, [existing.rows[0].id])).rows[0];
...
async function save(req,res,next) {
  try {
    ...
    const out=await withOrg(req.user.org_id,async c=>{
      ...
      const u=await c.query(`UPDATE client_email_recipients SET email=COALESCE($2,email),name=COALESCE($3,name),company=COALESCE($4,company),enabled=COALESCE($5,enabled),cds_customer_id=$6,cds_client_pulse=COALESCE($7,cds_client_pulse),sonalit_operational=COALESCE($8,sonalit_operational),sonalit_security=COALESCE($9,sonalit_security),updated_at=NOW() WHERE id=$1 AND org_id=$10 RETURNING id,org_id,email,name,company,enabled,authority_role,client_id,cds_customer_id,sonalit_operational,sonalit_security,cds_client_pulse`,[req.params.id,req.body.email==null?null:String(req.body.email).trim().toLowerCase(),req.body.name??null,req.body.company??null,req.body.enabled==null?null:req.body.enabled!==false,customerId,req.body.cds_client_pulse==null?null:req.body.cds_client_pulse!==false,req.body.sonalit_operational==null?null:req.body.sonalit_operational!==false,req.body.sonalit_security==null?null:req.body.sonalit_security===true,req.user.org_id]); const row=u.rows[0];
      if(row.cds_customer_id&&row.cds_client_pulse&&row.enabled&&row.authority_role!=='super_admin')await bindCdsRecipient(c,req.user.org_id,row.id,row.cds_customer_id);
      if(row.cds_customer_id&&row.cds_client_pulse===false)await c.query(`UPDATE communication_subscriptions s SET enabled=FALSE,updated_at=NOW() FROM communication_enrollments e WHERE e.id=s.enrollment_id AND e.org_id=$1 AND e.recipient_id=$2 AND e.domain='cds' AND e.cds_customer_id=$3 AND s.event_type='cds.client_pulse' AND s.channel='email'`,[req.user.org_id,row.id,row.cds_customer_id]); return row;
