...
function deterministicHeadline(cluster){const lead=[...cluster].sort((a,b)=>scoreEvent(b)-scoreEvent(a))[0]||{};return clean(lead.title||lead.summary||lead.headline||'Security development',180).replace(/[.!?]+$/,'').toUpperCase();}
...
async function synthesize(clusters){
 const out=new Map();
 const candidates=clusters.slice(0,MAX_AI_STORIES).map(cluster=>({story_id:storyKey(cluster),observations:cluster.slice(0,8).map(e=>({id:String(e.id),title:clean(e.title,500),summary:clean(e.summary,1400),country_code:e.country_code||null,severity:e.severity||null,confidence:e.confidence||null,evidence_count:e.observation_count||0,last_seen_at:e.last_seen_at||null}))}));
...
router.get('/stories', asyncHandler(async(req,res)=>{
 const s=normaliseScope(req),windowHours=Math.min(168,Math.max(1,Number(req.query.window_hours)||24)),limit=Math.min(30,Math.max(1,Number(req.query.limit)||18));
 const key=sha(JSON.stringify({org:req.user.org_id,scope:s,windowHours,limit}));const hit=cache.get(key);if(hit&&hit.expires>Date.now())return res.json(hit.value);
 const geo=countryClause(s,'e',3),limitIndex=3+geo.params.length;
 ...
 const events=rows.filter(e=>e.title||e.summary),clusters=[],used=new Set();
 for(const event of [...events].sort((a,b)=>scoreEvent(b)-scoreEvent(a))){
  if(used.has(String(event.id)))continue;const cluster=[event];used.add(String(event.id));const eventText=`${event.title||''} ${event.summary||''}`,eventDomain=domainFor(eventText);
  for(const other of events){
   ...
   const otherText=`${other.title||''} ${other.summary||''}`,semantic=overlap(eventText,otherText)>=0.42,proximity=km(event.latitude,event.longitude,other.latitude,other.longitude)<=75,sameDomain=eventDomain===domainFor(otherText);
   const timeA=new Date(event.last_seen_at||event.first_seen_at||0).getTime(),timeB=new Date(other.last_seen_at||other.first_seen_at||0).getTime(),within=Math.abs(timeA-timeB)<=48*3600000;
   if(sameCountry&&within&&(semantic||(proximity&&sameDomain))){cluster.push(other);used.add(String(other.id));}
  }
  clusters.push(cluster);if(clusters.length>=MAX_STORIES*2)break;
 }
 const ai=await synthesize(clusters);
 const stories=clusters.slice(0,limit).map((cluster,index)=>{const lead=[...cluster].sort((a,b)=>scoreEvent(b)-scoreEvent(a))[0],id=storyKey(cluster),generated=ai.get(id)||{};const severity=cluster.some(e=>String(e.severity||'').toLowerCase()==='critical')?'critical':cluster.some(e=>String(e.severity||'').toLowerCase()==='high')?'high':cluster.some(e=>String(e.severity||'').toLowerCase()==='moderate')?'moderate':'low';const confidence=generated.confidence_label?String(generated.confidence_label).toUpperCase():confidenceFromCluster(cluster)>=75?'HIGH':confidenceFromCluster(cluster)>=45?'MODERATE':'LOW';return {id,rank:index+1,headline:clean(generated.headline||deterministicHeadline(cluster),220).toUpperCase(),topic:clean(generated.topic||deterministicTopic(cluster),100).toUpperCase(),brief:clean(generated.brief||lead.summary||lead.description||lead.title,700),key_facts:Array.isArray(generated.key_facts)?generated.key_facts.slice(0,5):[],why_it_matters:Array.isArray(generated.why_it_matters)?generated.why_it_matters.slice(0,4):[],caveats:Array.isArray(generated.caveats)?generated.caveats.slice(0,4):[],confidence_label:confidence,operational_relevance:String(generated.operational_relevance||'MEDIUM').toUpperCase(),severity,country_code:lead.country_code||null,status:cluster.some(e=>String(e.status||'').toLowerCase()==='closed')?'CLOSED':'DEVELOPING',source_count:cluster.reduce((n,e)=>n+Number(e.source_count||0),0),observation_count:cluster.length,first_seen_at:cluster.reduce((v,e)=>!v||new Date(e.first_seen_at||e.last_seen_at)<new Date(v)?(e.first_seen_at||e.last_seen_at):v,null),last_seen_at:cluster.reduce((v,e)=>!v||new Date(e.last_seen_at||e.first_seen_at)>new Date(v)?(e.last_seen_at||e.first_seen_at):v,null),event_ids:cluster.map(e=>e.id),events:cluster.slice(0,10)};});
 const topicMap=new Map();for(const story of stories){const current=topicMap.get(story.topic)||{topic:story.topic,story_count:0,signal_count:0,severity:'low',countries:new Set()};current.story_count++;current.signal_count+=story.observation_count;current.severity=['critical','high','moderate','low'].indexOf(story.severity)<['critical','high','moderate','low'].indexOf(current.severity)?story.severity:current.severity;if(story.country_code)current.countries.add(story.country_code);topicMap.set(story.topic,current);}
 const topics=[...topicMap.values()].map(x=>({...x,countries:[...x.countries]})).sort((a,b)=>b.signal_count-a.signal_count).slice(0,8),changes={signals_24h:events.length,high_priority:events.filter(e=>['critical','high'].includes(String(e.severity||'').toLowerCase())).length,developing_stories:stories.filter(s=>s.status==='DEVELOPING').length,source_observations:events.reduce((n,e)=>n+Number(e.observation_count||0),0)};
 const payload={stories,topics,changes,scope:s,generated_at:new Date().toISOString(),engine:{name:'SONALIT SYNTHESIS ENGINE',ai_used:ai.size>0,stories_synthesized:ai.size,cache_ttl_seconds:CACHE_TTL_MS/1000}};cache.set(key,{expires:Date.now()+CACHE_TTL_MS,value:payload});res.json(payload);
}));

router.get('/stories/:id', asyncHandler(async(req,res)=>{const ids=Array.isArray(req.query.event_ids)?req.query.event_ids.filter(Boolean):String(req.query.event_ids||req.params.id).split(',').map(x=>x.trim()).filter(Boolean);const safeIds=ids.filter(x=>/^[0-9a-f-]{36}$/i.test(x)).slice(0,20);if(!safeIds.length)return res.status(400).json({error:'event_ids must contain valid event UUIDs'});const {rows}=await req.db(`SELECT e.id,e.title,e.summary,e.description,e.country_code,e.scope_type,e.scope_key,e.severity,e.confidence,e.status,e.latitude,e.longitude,e.first_seen_at,e.last_seen_at,COUNT(eo.observation_id)::int AS observation_count,COUNT(DISTINCT o.source_id)::int AS source_count FROM intel_events e LEFT JOIN intel_event_observations eo ON eo.event_id=e.id LEFT JOIN intel_observations o ON o.id=eo.observation_id WHERE e.org_id=$1 AND e.id=ANY($2::uuid[]) GROUP BY e.id ORDER BY e.last_seen_at DESC`,[req.user.org_id,safeIds]);res.json({events:rows});}));
