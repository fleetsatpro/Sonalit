...
function manifestRow(row) {
  return {
    ...
    transporter: row.transporter, horse_reg: row.horse_reg || row.horse_reg_derived,
    trailer_reg: row.trailer_reg, driver_name: row.driver_name || row.driver_name_derived,
    driver_contact: row.driver_contact || row.driver_contact_derived, invoiced: row.invoiced ? 'YES' : 'NO',
  };
}
...
async function queueManualSuperAdminPulse(orgId, snapshotAt) {
  const recipients = await withOrg(orgId, client => client.query(
    `SELECT DISTINCT email,name FROM client_email_recipients
     WHERE org_id=$1 AND authority_role='super_admin' AND enabled=true
       AND deleted_at IS NULL AND email IS NOT NULL ORDER BY email`, [orgId]
  ));
  if (!recipients.rows.length) return { skipped: true, reason: 'no_super_admin_recipients', queued: 0 };

  const run = await withOrg(orgId, client => client.query(
    ...
    [orgId, snapshotAt, `cds-client-pulse:super-admin:${snapshotAt.toISOString().slice(0,13)}`]
  ));
  ...
  try {
    ...
    const active = rows.filter(isActiveRow).map(manifestRow);
    ...
    const result = await queueClientPulseEmail({
      ...
      correlationId: `cds-client-pulse:${runId}`, idempotencyKey: `cds-client-pulse:super-admin:${snapshotAt.toISOString().slice(0,13)}`,
    });
    ...
    await withOrg(orgId, client => client.query(`UPDATE cds_client_pulse_runs SET status='failed',error=$2,updated_at=NOW() WHERE id=$1`, [runId,String(err.message||err).slice(0,4000)]));
    logger.error(`Manual Super Admin Client Pulse failed: org=${orgId} run=${runId} error=${err.message}`);
    throw err;
  }
}
