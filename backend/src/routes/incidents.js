...
router.post('/', async (req, res, next) => {
  try { const { title, description, severity = 'medium' } = req.body; if (!title) return res.status(400).json({ error: 'Title is required' }); const { rows } = await pool.query(`INSERT INTO incidents (title, description, severity, status) VALUES ($1,$2,$3,'open') RETURNING *`, [title, description, severity]); res.status(201).json({ data: rows[0] }); } catch (err) { next(err); }
});
