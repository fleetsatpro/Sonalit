...
const CAPTURE_STAGE = {
  ...
  cc_take_error:    { ok: false, msg: 'The camera closed before the shot completed — another app holds the camera, or a previous capture had not released it.' },
  cc_retry:         { ok: false, msg: 'Camera was closed on the first try — retrying the shot.' },
  gs_busy:          { ok: false, msg: 'A capture was already running, so this duplicate request was ignored.' },
  gs_capture_timeout:{ ok: false, msg: 'The capture never reported back and was abandoned after 90s.' },
