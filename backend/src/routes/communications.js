...
function validateEnrollment(body) {
  const allowed = new Set([
    'recipient_id', 'domain', 'client_id', 'cds_customer_id',
    'contact_role', 'locale', 'timezone', 'status'
  ]);
  const unknown = Object.keys(body || {}).filter(k => !allowed.has(k));
  if (unknown.length) return { error: `"${unknown[0]}" is not allowed` };
  for (const key of ['recipient_id', 'client_id', 'cds_customer_id']) {
    if (body[key] != null && !UUID_RE.test(String(body[key]))) {
      return { error: `"${key}" must be a valid UUID` };
    }
  }
...
router.post('/enrollments', async (req, res, next) => {
  try {
    ...
    const {
      recipient_id, domain, client_id = null, cds_customer_id = null,
      contact_role = null, locale = null, timezone = null,
      status = 'pending_verification'
    } = req.body;

    const result = await withOrg(req.user.org_id, c => c.query(
      `INSERT INTO communication_enrollments
        (org_id, recipient_id, domain, client_id, cds_customer_id, contact_role, locale, timezone, status)
...
router.get('/enrollments', async (req, res, next) => {
  try {
    const result = await withOrg(req.user.org_id, c => c.query(
      `SELECT * FROM communication_enrollments WHERE org_id=$1 ORDER BY created_at DESC`,
      [req.user.org_id]
    ));
