...
export type City = { lon: number; lat: number; label: string; mode: Mode };
...
export const ROUTES: readonly { a: string; b: string; mode: Mode }[] = [
...
export const MODE_COL: Record<Mode, string> = { road: '#F0B429', air: '#4A9EFF', sea: '#35C4D7' };
export const PK_COL:   Record<Mode, string> = { road: '#FFE29A', air: '#BBDCFF', sea: '#BFF3FA' };
export const DOT_COL: readonly string[] = ['#243349', '#2B3D57', '#35496A', '#5C86B8', '#7A6222', '#BE922E'];
export const DOT_R:   readonly number[] = [0.9, 1.0, 1.05, 1.2, 1.25, 1.45];
...
export function v3(lonDeg: number, latDeg: number): Vec3 {
  const la = latDeg * DEG;
  const lo = lonDeg * DEG;
...
export function slerp(a: Vec3, b: Vec3, t: number): Vec3 {
  ...
  if (om < 1e-6) return [a[0], a[1], a[2]] as const;
...
export function vll(v: Vec3): [number, number] {
  return [
