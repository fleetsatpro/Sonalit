...
export default function RiskIntelPage() {
    ...
    const [activeCont, setActiveCont] = useState('global');
    ...
    const handleExport = async () => {
        ...
        if (activeCont !== 'global')
            params.set('continent', activeCont);
        if (levelFilter !== 'all')
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: isMobile ? 8 : 14, alignItems: 'center', flexWrap: 'wrap' }}>
          {!isLoading && ['high', 'medium', 'low'].map(level => (<div key={level} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {!isLoading && (<span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#6e6c64' }}>/{counts.total}</span>)}
        </div>

        {/* Controls */}
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => setHeatVisible(v => !v)} style={{
            padding: '3px 8px',
          ...
          {!isMobile && (<button onClick={handleExport} style={{ padding: '3px 8px', background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 5, cursor: 'pointer', fontSize: 10, color: '#9a9890' }}>CSV</button>)}
        </div>
      </div>
      ...
      {/* ── Continent bar ── */}
      <ContinentBar active={activeCont} zones={zones} onChange={c => { setActiveCont(c); if (isMobile)
        setActiveTab('map'); }}/>

      {/* ── Mobile tab bar ── */}
      {isMobile && (<div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
          {['list', 'map'].map(t => (<button key={t} onClick={() => setActiveTab(t)} style={{
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{
          ...
          <ThreatList zones={zones} levelFilter={levelFilter} activeId={activeZoneId} onSelect={id => { handleZoneSelect(id); }} onLevelChange={setLevelFilter}/>
        </div>
        ...
        <div style={{
          ...
          {isLoading && (<div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, pointerEvents: 'none' }}>
              <span style={{ fontSize: 12, color: '#4e4c44' }}>Loading threat data…</span>
            </div>)}
          <RiskMap zones={zones} activeCont={activeCont} activeZoneId={activeZoneId} heatVisible={heatVisible} onZoneClick={id => {
            handleZoneSelect(id);
            if (isMobile)
                setActiveTab('map');
        }}/>
        ...
        <DetailDrawer zone={activeZone} onClose={() => setActiveZoneId(null)} isMobile={isMobile}/>
      </div>
    </div>);
}
