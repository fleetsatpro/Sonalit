...
function VelocityIcon({ v }) {
    if (v === 'rising')
        return <IconTrendingUp size={11} color="#E24B4A"/>;
    if (v === 'falling')
        return <IconTrendingDown size={11} color="#4ade80"/>;
...
export default function ThreatCard({ zone, active, onClick }) {
    ...
    return (<div onClick={onClick} style={{
      ...
      <div style={{ padding: '10px 10px 8px', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          {(zone.tags ?? []).slice(0, 2).map(t => (<span key={t} style={{ fontSize: 9, padding: '1px 6px', borderRadius: 10, background: 'rgba(255,255,255,.06)', color: '#9a9890' }}>{t}</span>))}
