...
export default function ConvoysTab({ zone }: Props) {
  const { data, isLoading } = useRiskConvoys(zone.continent)
  const convoys = (data?.convoys ?? []).filter(c =>
    c.affected_zones.some(z => z === zone.id || z === zone.zone_code)
  )
      ...
      {convoys.map(c => {
        ...
        return (
          <div key={c.id} style={{
            ...
            {c.driver_name && (
              <div style={{ fontSize: 10, color: '#9a9890', marginTop: 3 }}>Driver: {c.driver_name}</div>
            )}
