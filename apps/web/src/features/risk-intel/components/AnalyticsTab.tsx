...
import type { RiskZone, RiskEvent } from '../types/risk.js'
...
export default function AnalyticsTab({ zone, events }: Props) {
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      {zone.precaution && (
        <div style={{ marginTop: 12, padding: '10px 12px', background: 'rgba(240,180,41,.07)', borderRadius: 6, border: '1px solid rgba(240,180,41,.2)' }}>
      ...
      {events.length > 0 && (
        <div style={{ marginTop: 16 }}>
          ...
          {events.slice(0, 8).map(ev => (
            <div key={ev.id} style={{ padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,.05)', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
