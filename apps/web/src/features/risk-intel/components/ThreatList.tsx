...
import type { RiskZone, RiskLevel } from '../types/risk.js'
...
const LEVELS: Array<{ key: RiskLevel | 'all'; label: string }> = [
  { key: 'all',    label: 'All' },
  { key: 'high',   label: 'High' },
...
export default function ThreatList({ zones, levelFilter, activeId, onSelect, onLevelChange }: Props) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {LEVELS.map(({ key, label }) => (
          ...
          >
            {key !== 'all' && (
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: LEVEL_DOT[key] }} />
            )}
            {label}
          </button>
        ))}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
        {filtered.length === 0 && (
          <div style={{ padding: 24, textAlign: 'center', fontSize: 12, color: '#4e4c44' }}>
