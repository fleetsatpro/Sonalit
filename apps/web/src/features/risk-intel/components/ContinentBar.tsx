import type { Continent } from '../types/risk.js'
...
interface Props {
  active: Continent
  zones: RiskZone[]     // all zones (unfiltered) for badge counts
  onChange: (c: Continent) => void
}

export default function ContinentBar({ active, zones, onChange }: Props) {
  const countFor = (key: Continent) =>
    key === 'global' ? zones.length : zones.filter(z => z.continent === key).length
