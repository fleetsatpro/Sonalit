...
function coordsToPoints(c) {
    if (!c)
        return null;
    if (typeof c === 'string') {
        try {
            return coordsToPoints(JSON.parse(c));
        }
        catch {
            return null;
        }
    ...
    if (typeof c === 'object' && !Array.isArray(c)) {
        if (Array.isArray(c.coordinates))
            return coordsToPoints(c.coordinates);
        // OSRM corridor: {path:[[lat,lng],...]} — swap axes to [lng,lat]
        if (Array.isArray(c.path)) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const pts = c.path.map((p) => Array.isArray(p) ? [Number(p[1]), Number(p[0])] : null).filter((p) => p !== null);
    ...
    if (Array.isArray(c[0]) && Array.isArray(c[0][0]))
        return coordsToPoints(c[0]);
    if (Array.isArray(c[0]))
        return c.map(p => [Number(p[0]), Number(p[1])]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return c.map(p => [p.lng ?? p.longitude ?? 0, p.lat ?? p.latitude ?? 0]);
}
function isCorridor(g) {
    const t = (g.type ?? '').toLowerCase();
    return t === 'corridor' || t === 'linear' || t === 'line' || t === 'linestring' || t === 'route';
}
function buildRiskFC(zones) {
    return { type: 'FeatureCollection', features: zones.filter(z => z.center_lat && z.center_lon).map(z => ({
            type: 'Feature',
            geometry: { type: 'Polygon', coordinates: [circlePolygon(z.center_lat, z.center_lon, z.radius_km * 1000)] },
...
export default function FleetMap({ vehicles, selectedId, onSelect }) {
    ...
    const { data: geofences } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/geofences'); return r.data.data ?? []; },
        staleTime: 120_000, refetchInterval: 120_000,
    });
    const { data: riskZones } = useQuery({
        queryKey: ['live-fleet-riskzones'],
        queryFn: async () => { const r = await api.get('/riskzones'); return r.data.data ?? []; },
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const polySrc = map.getSource('gf-polys');
        if (polySrc) {
            polySrc.setData({ type: 'FeatureCollection', features: polyFeats });
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            map.addSource('gf-polys', { type: 'geojson', data: { type: 'FeatureCollection', features: polyFeats } });
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const lineSrc = map.getSource('gf-lines');
        if (lineSrc) {
            lineSrc.setData({ type: 'FeatureCollection', features: lineFeats });
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            map.addSource('gf-lines', { type: 'geojson', data: { type: 'FeatureCollection', features: lineFeats } });
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const src = map.getSource('riskzones');
        if (src) {
            src.setData(fc);
            return;
        }
    ...
    useEffect(() => {
        ...
        let t = 0;
        const loop = () => {
            const p = 0.08 + 0.18 * (0.5 + 0.5 * Math.sin(t));
            t += 0.04;
            try {
                if (map.getLayer('rz-glow'))
                    map.setPaintProperty('rz-glow', 'fill-opacity', p);
            }
            catch (_) { }
    ...
    useEffect(() => {
        ...
        for (const v of positioned) {
            ...
            if (existing) {
                ...
                el.innerHTML = makeEl(v).innerHTML;
                const inner = el.firstElementChild;
                if (inner)
                    inner.addEventListener('click', e => { e.stopPropagation(); onSelect(v); }, { once: true });
            }
            else {
                const el = makeEl(v);
                const inner = el.firstElementChild;
    ...
    // keyframes
    useEffect(() => {
        if (document.getElementById('lf-map-styles'))
            return;
    ...
    return (<div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', right: 14, top: 14, zIndex: 500, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <button onClick={() => {
            ...
            if (!p.length || !mapRef.current)
                return;
            const lngs = p.map(v => v.lng);
            const lats = p.map(v => v.lat);
      ...
      <div style={{ position: 'absolute', right: 56, top: 14, zIndex: 500, display: 'flex', flexDirection: 'column', gap: 4 }}>
        {geoCount > 0 && (<div style={{ background: 'rgba(8,11,20,.92)', border: '1px solid rgba(34,211,238,.3)', borderRadius: 5, padding: '4px 10px', display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        {riskCount > 0 && (<div style={{ background: 'rgba(8,11,20,.92)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 5, padding: '4px 10px', display: 'flex', alignItems: 'center', gap: 5 }}>
