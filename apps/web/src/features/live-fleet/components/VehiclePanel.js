...
function VehicleRow({ v, selected, onSelect }) {
    ...
    return (<div onClick={onSelect} style={{
      ...
      {v.status === 'sos' && (<div style={{ position: 'absolute', left: 22, top: 0, bottom: 0, width: 2, background: '#ef4444', animation: 'lf-sos-stripe .8s ease-in-out infinite' }}/>)}
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 1 }}>
          ...
          {v.status === 'sos' && (<span style={{ fontSize: 8, fontFamily: 'IBM Plex Mono, monospace', fontWeight: 700, color: '#ef4444', background: 'rgba(239,68,68,.12)', border: '1px solid rgba(239,68,68,.25)', borderRadius: 2, padding: '1px 4px' }}>SOS</span>)}
        </div>
...
function ConvoySection({ group, selectedId, onSelect }) {
    ...
    return (<div style={{ borderBottom: '1px solid rgba(255,255,255,.06)' }}>
      ...
      {open && group.vehicles.map(v => (<VehicleRow key={v.id} v={v} selected={selectedId === v.id} onSelect={() => onSelect(v)}/>))}
    </div>);
}
export default function VehiclePanel({ groups, selectedId, collapsed, onToggleCollapse, onSelect }) {
    ...
    return (<div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 20 }}>
        {filtered.length === 0 && (<div style={{ padding: 24, textAlign: 'center', fontSize: 12, color: '#3e4252' }}>No vehicles</div>)}
        {filtered.map(g => (<ConvoySection key={g.id} group={g} selectedId={selectedId} onSelect={onSelect}/>))}
      </div>
