...
type Tab = 'log' | 'anomalies' | 'analytics' | 'stations';
...
export default function FuelPage() {
  ...
  const fuelQ = useQuery<{ data: FuelEntry[] }>({
    queryKey: ['fuel-entries'],
    queryFn: () => api.get('/fuel').then(r => r.data),
    enabled: tab === 'log',
  });

  const anomaliesQ = useQuery<{ data: FuelAnomaly[] }>({
  ...
  const summaryQ = useQuery<{ data: FuelSummary[] }>({
    queryKey: ['fuel-summary'],
    queryFn: () => api.get('/fuel/summary').then(r => r.data),
    enabled: tab === 'analytics',
  });

  const stationsQ = useQuery<{ data: Station[] }>({
  ...
  const resolveMut = useMutation({
    mutationFn: (id: string) => api.patch(`/fuel/anomalies/${id}/resolve`, { resolution_note: 'Resolved via dashboard' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['fuel-anomalies'] }),
  });

  const addStationMut = useMutation({
    mutationFn: (body: object) => api.post('/fuel/stations', body),
  ...
  const deleteStationMut = useMutation({
    mutationFn: (id: string) => api.delete(`/fuel/stations/${id}`),
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        {tab === 'log' && (
          <button onClick={() => setShowAddEntry(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm">
            <Plus className="w-4 h-4" /> Log Fuel
          </button>
        )}
        {tab === 'stations' && (
          <button onClick={() => setShowAddStation(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm">
          ...
          {fuelQ.data && (
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {fuelQ.data.data.length === 0 && (
                  <tr><td colSpan={7} className="p-8 text-center text-gray-500">No fuel entries yet</td></tr>
                )}
              </tbody>
            </table>
          )}
          ...
          {anomaliesQ.data && (
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {anomaliesQ.data.data.length === 0 && (
                  <tr><td colSpan={7} className="p-8 text-center text-gray-500">No anomalies detected</td></tr>
                )}
              </tbody>
            </table>
          )}
          ...
          {summaryQ.data && (
            <div className="bg-gray-800 rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {summaryQ.data.data.map(s => (
                    <tr key={s.vehicle_id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                  ...
                  {summaryQ.data.data.length === 0 && (
                    <tr><td colSpan={5} className="p-8 text-center text-gray-500">No data yet</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <tbody>
                {stationsQ.data.data.map(s => (
                  <tr key={s.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                    ...
                    <td className="p-3">
                      <button
                        onClick={() => deleteStationMut.mutate(s.id)}
                ...
                {stationsQ.data.data.length === 0 && (
                  <tr><td colSpan={5} className="p-8 text-center text-gray-500">No approved stations configured</td></tr>
                )}
              </tbody>
            </table>
          )}
          ...
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                disabled={!newEntry.litres || addEntryMut.isPending}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => addStationMut.mutate({
                  name: newStation.name,
                  lat: parseFloat(newStation.lat),
                  lng: parseFloat(newStation.lng),
                  radius_km: parseFloat(newStation.radius_km),
                })}
                disabled={!newStation.name || !newStation.lat || !newStation.lng || addStationMut.isPending}
                className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg"
              >
