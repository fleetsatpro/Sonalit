...
import ReportDoc from '../components/reports/ReportDoc.js';
// ── Style helpers ─────────────────────────────────────────────────────────────
const S = {
    root: {
    ...
    viewer: {
        flex: 1, overflowY: 'auto', background: '#0b0f1a', padding: '24px 32px',
    },
    emptyViewer: {
...
function ConvoyCard({ item, selected, onClick, }) {
    ...
    return (<div style={S.convoyCard(selected)} onClick={onClick}>
      ...
      {lr && (<>
          ...
          {lr.location_mismatch_count > 0 && (<div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 5 }}>
...
export default function ConvoyReports() {
    ...
    const filtered = filter === 'all' ? items : items.filter(c => {
        ...
        if (filter === 'pending')
            return !c.latest_report || c.latest_report.status === 'pending';
        return true;
    });
    const stats = {
        total: items.length,
        ...
        complete: items.filter(c => c.latest_report?.status === 'complete' || c.latest_report?.status === 'generated').length,
    };
    function handleSelectConvoy(item) {
    ...
    return (<div style={S.root}>
      ...
      <aside style={S.sidebar}>
        <div style={S.sidebarHeader}>
          <button onClick={() => void navigate({ to: '/convoys' })} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none',
        ...
        ].map(({ label, value, icon, accent }) => (<div key={label} style={S.statCard(accent)}>
        ...
        <div style={S.tabs}>
          {['all', 'active', 'completed', 'pending'].map(t => (<button key={t} style={S.tab(filter === t)} onClick={() => setFilter(t)}>
              {t}
            </button>))}
        </div>

        <div style={S.convoyList}>
          {overviewLoading && (<div style={{ display: 'flex', justifyContent: 'center', padding: 24 }}>
              <Loader2 size={18} className="animate-spin" style={{ color: '#4b5563' }}/>
            </div>)}
          {filtered.length === 0 && !overviewLoading && (<div style={{ padding: '32px 16px', textAlign: 'center', fontSize: 12, color: '#4b5563' }}>
              No convoys found.
            </div>)}
          {filtered.map(item => (<ConvoyCard key={item.id} item={item} selected={selectedId === item.id} onClick={() => handleSelectConvoy(item)}/>))}
        </div>
      </aside>

      {/* ── Viewer ── */}
      <main style={S.viewer}>
        {!selectedId && (<div style={{ ...S.emptyViewer, height: '100%' }}>
        ...
        {selectedId && detailLoading && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
                height: '40vh', gap: 10, color: '#4b5563' }}>
            <Loader2 size={20} className="animate-spin"/>
            <span style={{ fontSize: 13 }}>Loading report…</span>
          </div>)}

        {selectedId && !detailLoading && !selectedDate && (<div style={{ ...S.emptyViewer, height: '40vh', color: '#6b7280' }}>
            <span style={{ fontSize: 13 }}>No report available for this convoy yet.</span>
          </div>)}

        {selectedId && selectedDate && detailError && !detailLoading && (<div style={{ ...S.emptyViewer, height: '40vh', color: '#ef4444', gap: 8 }}>
            <AlertTriangle size={28} style={{ opacity: 0.7 }}/>
            <span style={{ fontSize: 13 }}>Failed to load report. Try again or check server logs.</span>
          </div>)}

        {detail?.data && !detailLoading && (<ReportDoc detail={detail.data} onRegenerate={() => regenerateMutation.mutate()} regenerating={regenerateMutation.isPending}/>)}
      </main>
    </div>);
}
