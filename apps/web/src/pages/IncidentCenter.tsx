...
function CameraFeeds({ incidentId }: { incidentId: string }) {
  return (
    <div className="mt-4">
      <p className="text-xs text-slate-400 uppercase font-medium mb-2">Camera Feeds</p>
      <div className="grid grid-cols-2 gap-2">
        {CAMERA_DEVICE_IDS.map((deviceId) => (
          <div
            key={`${incidentId}-${deviceId}`}
            className="bg-slate-900 border border-slate-700 rounded aspect-video flex items-center justify-center text-slate-500 text-xs"
          >
  ...
  return (
    <div className="flex h-full gap-0 -m-6">
      ...
      <div className="w-80 shrink-0 bg-slate-900 border-r border-slate-700 flex flex-col">
        ...
        {isLoading && (
          <div className="p-4 text-slate-400 text-sm">Loading incidents…</div>
        )}
        {isError && (
          <div className="p-4 text-red-400 text-sm">Failed to load incidents.</div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          {data?.map((incident) => (
            <button
              key={incident.id}
              className={`w-full text-left px-4 py-3 border-b border-slate-700 hover:bg-slate-800 transition-colors ${
                selectedId === incident.id ? 'bg-slate-800 border-l-2 border-l-blue-500' : ''
              }`}
            ...
            >
              <div className="flex items-start gap-2">
                <Circle
                  size={8}
                  className={`mt-1.5 shrink-0 fill-current ${SEVERITY_COLORS[incident.severity]}`}
                />
