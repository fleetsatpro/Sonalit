...
function HealthBars({ battery, signal }) {
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 3, width: 60 }}>
      {['BAT', 'SIG'].map((label, i) => {
...
const MBOX = { background: C.surf, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' };
...
function EnrollDeviceModal({ devices, onClose, onApproved }) {
    const pending = devices.filter(d => d.status === 'pending');
    const [approving, setApproving] = useState(null);
    async function approve(id) {
        setApproving(id);
        try {
            await api.post(`/guardian/devices/${id}/approve`);
            onApproved();
        }
        catch { }
        finally {
    ...
    return (<div style={OVERLAY} onClick={onClose}>
      <div style={MBOX} onClick={e => e.stopPropagation()}>
        ...
        {pending.length === 0
            ...
            : pending.map(d => (<div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: C.panel, borderRadius: 4, marginBottom: 6 }}>
              ...
              <button onClick={() => approve(d.id)} disabled={approving === d.id} style={{ background: `${C.green}20`, border: `1px solid ${C.green}50`, borderRadius: 3, padding: '4px 10px', color: C.green, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, cursor: 'pointer' }}>
...
function LinkOfficerModal({ device, onClose, onLinked }) {
    const [officers, setOfficers] = useState([]);
    const [selOfficer, setSelOfficer] = useState('');
    const [saving, setSaving] = useState(false);
    const [err, setErr] = useState('');
    useEffect(() => {
        api.get('/field-officers')
    ...
    return (<div style={OVERLAY} onClick={onClose}>
      <div style={{ ...MBOX, maxWidth: 380 }} onClick={e => e.stopPropagation()}>
        ...
        <select value={selOfficer} onChange={e => setSelOfficer(e.target.value)} style={{ width: '100%', background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, padding: '8px 10px', fontSize: 11, color: selOfficer ? C.txt : C.sub, outline: 'none', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={link} disabled={!selOfficer || saving} style={{ flex: 2, background: selOfficer ? C.gold : C.dim, border: 'none', borderRadius: 4, padding: 8, fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 800, fontSize: 13, letterSpacing: '0.08em', color: selOfficer ? '#000' : C.sub, cursor: selOfficer ? 'pointer' : 'not-allowed' }}>
...
function DeviceMatrix({ devices, onLink, onApprove }) {
    ...
    return (<div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, overflow: 'auto' }}>
      <div style={{ padding: '10px 12px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {pendingCount > 0 && (<button onClick={onApprove} style={{ background: `${C.purple}20`, border: `1px solid ${C.purple}50`, borderRadius: 3, padding: '3px 8px', color: C.purple, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        ...
        <tbody>
          {devices.map(d => {
            ...
            return (<>
                <tr key={d.id} style={{ borderBottom: `1px solid ${C.border}`, background: exp ? C.panel : 'transparent', cursor: 'pointer' }} onClick={() => setExpandedId(prev => prev === d.id ? null : d.id)}>
                  ...
                  <td style={{ padding: '8px 10px' }} onClick={e => e.stopPropagation()}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={() => onLink(d)} style={{ background: `${C.blue}18`, border: `1px solid ${C.blue}40`, borderRadius: 3, padding: '3px 7px', color: C.blue, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, cursor: 'pointer' }}>
                        LINK
                      </button>
                      <button onClick={() => d.knox_do_enrolled
                    ? navigate({ to: '/guardian/devices/$deviceId/remote', params: { deviceId: d.id } })
                    : alert('Knox enrollment required for remote session')} title={d.knox_do_enrolled ? 'Open Knox remote session' : 'Knox enrollment required'} style={{ background: d.knox_do_enrolled ? `${C.cyan}18` : `${C.dim}20`, border: `1px solid ${d.knox_do_enrolled ? C.cyan + '40' : C.border}`, borderRadius: 3, padding: '3px 8px', color: d.knox_do_enrolled ? C.cyan : C.sub, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, cursor: d.knox_do_enrolled ? 'pointer' : 'default', opacity: d.knox_do_enrolled ? 1 : 0.5 }}>
                        REMOTE
                      </button>
                    </div>
                  </td>
                    ...
                    <td colSpan={7} style={{ background: C.panel, padding: '12px 16px', borderBottom: `1px solid ${C.border}` }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
                        <div>
                          ...
                          {[['IMEI', maskedImei], ['Model', d.model || '—'], ['Android', d.android_version || '—'], ['Knox', d.knox_version || '—'], ['App', d.app_version || '—']].map(([k, v]) => (<div key={k} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        ...
                        <div>
                          ...
                          {[['Battery', `${d.battery_pct ?? '—'}%`], ['Signal', `${d.signal_pct ?? '—'}%`], ['GPS', d.gps_locked ? 'Locked' : 'Searching'], ['Last Seen', fmtRel(d.last_heartbeat_at || d.last_seen_at)]].map(([k, v]) => (<div key={k} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                              <span style={{ color: C.sub, fontSize: 10 }}>{k}</span>
                              <span style={{ color: C.txt, fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }}>{v}</span>
                            </div>))}
                        </div>
                        <div>
                          ...
                          {cmds.length === 0 ? <div style={{ color: C.sub, fontSize: 10 }}>No commands issued</div> : cmds.slice(0, 5).map((c, i) => (<div key={i} style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                              <div style={{ flex: 1 }}>
...
function CommandTerminal({ devices }) {
    ...
    async function issue() {
        ...
        if (!selDevice || !cmd)
            return;
        if (cmd.confirm && !window.confirm(`Execute "${cmd.label}" on this device?`))
            return;
        setSending(true);
        setMsg(null);
        try {
    ...
    return (<div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
      ...
      <select value={selDevice} onChange={e => setSelDevice(e.target.value)} style={{ width: '100%', background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, padding: '6px 10px', fontSize: 11, color: selDevice ? C.txt : C.sub, outline: 'none', marginBottom: 10 }}>
        <option value="">— Select Device —</option>
        {devices.map(d => <option key={d.id} value={d.id}>{d.officer_name || d.assigned_to || d.name || d.id} — {d.model || '?'}</option>)}
      </select>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>
        {COMMANDS.map(cmd => (<button key={cmd.id} onClick={() => setSelCmd(prev => prev === cmd.id ? null : cmd.id)} style={{ background: selCmd === cmd.id ? `${cmd.color}25` : `${cmd.color}10`, border: `1px solid ${selCmd === cmd.id ? cmd.color + '70' : cmd.color + '30'}`, borderRadius: 4, padding: '6px 8px', textAlign: 'left', cursor: 'pointer' }}>
      ...
      <button onClick={issue} disabled={!selDevice || !selCmd || sending} style={{ width: '100%', background: selDevice && selCmd ? C.gold : C.dim, border: 'none', borderRadius: 4, padding: '8px', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 800, fontSize: 13, letterSpacing: '0.08em', color: selDevice && selCmd ? '#000' : C.sub, cursor: selDevice && selCmd ? 'pointer' : 'not-allowed' }}>
...
export default function Guardian() {
    ...
    const [linkDevice, setLinkDevice] = useState(null);
    ...
    useEffect(() => {
        ...
        return subscribe(`org#${user.org_id}`, (msg) => {
            ...
            if (msg.type === 'command:queued')
                setCommands(prev => [msg, ...prev].slice(0, 30));
    ...
    return (<div style={{ padding: 16, background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif', fontSize: 12 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setShowEnroll(true)} style={{ background: `${C.gold}18`, border: `1px solid ${C.gold}50`, borderRadius: 4, padding: '7px 16px', color: C.gold, fontSize: 11, fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, letterSpacing: '0.08em', cursor: 'pointer' }}>
          + ENROLL DEVICE
        </button>
      </div>

      {loading ? (<div style={{ textAlign: 'center', padding: 40, color: C.sub, fontFamily: 'JetBrains Mono, monospace', fontSize: 11 }}>Loading…</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12 }}>
            ...
            <div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
              ...
              {commands.length === 0
                ...
                : commands.slice(0, 10).map((cmd, i) => {
                    const sc = { pending: C.amber, acknowledged: C.green, expired: C.sub, failed: C.red }[cmd.status] || C.mid;
                    return (<div key={cmd.id || i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
                ...
                : rules.map((rule, i) => (<div key={rule.id || i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <button onClick={() => toggleRule(rule)} style={{ width: 32, height: 16, borderRadius: 8, background: rule.enabled ? `${C.green}40` : C.panel, border: `1px solid ${rule.enabled ? C.green : C.border}`, cursor: 'pointer', position: 'relative', flexShrink: 0 }}>
              ...
              <div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
                ...
                : auditEvents.map((ev, i) => (<div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
