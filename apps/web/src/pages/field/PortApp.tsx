...
export default function PortApp() {
  ...
  return (
    <div className="min-h-screen bg-[#0B111C] text-white flex flex-col">
      <header className="sticky top-0 z-10 bg-[#0B111C]/95 backdrop-blur border-b border-white/[.06] px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[15px] font-bold text-white leading-tight">Port ops</div>
          <div className="text-[11px] text-white/40 font-mono">{rows.length} containers in transit</div>
        </div>
        <div className="flex gap-0 border border-white/10 rounded-lg overflow-hidden">
          <button
            onClick={() => setTab('list')}
            className={`px-3 h-9 text-[11px] font-mono border-none ${tab === 'list' ? 'bg-white/[.10] text-white' : 'bg-transparent text-white/50'}`}
          >
            <ListIcon size={13} />
          </button>
          <button
            onClick={() => setTab('map')}
            className={`px-3 h-9 text-[11px] font-mono border-none ${tab === 'map' ? 'bg-white/[.10] text-white' : 'bg-transparent text-white/50'}`}
          >
            <MapIcon size={13} />
          </button>
        </div>
        ...
        <div className="flex-1 relative">
            ...
            {withGps.map(r => (
              ...
              >
                <div
                  ...
                  style={{ background: '#37e6ff', borderColor: '#0B111C', boxShadow: '0 4px 12px rgba(55,230,255,.5)' }}
                >
                  <Truck size={16} color="#0B111C" strokeWidth={2.5} />
                </div>
              </Marker>
            ))}
          </Map>
          {withGps.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="bg-black/70 backdrop-blur px-4 py-2 rounded-lg text-[12px] font-mono text-white/70">
...
function ContainerRow({ row, onOpen }: { row: Row; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="w-full text-left rounded-xl border border-[#37e6ff]/25 bg-[#37e6ff]/[.05] px-4 py-3 active:brightness-125 transition-all"
    >
      <div className="flex items-center gap-2 flex-wrap">
        <span className="font-mono text-[14px] font-bold text-[#37e6ff]">
          {s(row['container_number']) || 'No number'}
        </span>
        <span className="text-[10px] font-mono text-white/60 bg-white/[.08] px-1.5 py-0.5 rounded">
          {s(row['iso_type'])}
        </span>
        <span className="ml-auto text-[10px] font-mono px-1.5 py-0.5 rounded bg-[#37e6ff]/20 text-[#37e6ff] uppercase tracking-wider">
          {s(row['status']).replace('_', ' ')}
        </span>
      </div>
      <div className="text-[11px] text-white/60 mt-1 truncate">{s(row['booking_number'])} · {s(row['customer_name'])}</div>
      <div className="text-[10px] font-mono text-white/40 mt-1 flex items-center gap-3">
...
function UnclampForm({ row, onClose, onDone }: { row: Row; onClose: () => void; onDone: () => void }) {
  ...
  if (success) return (
    <div className="min-h-screen bg-[#0B111C] text-white flex flex-col items-center justify-center p-6">
      ...
      <div className="text-[12px] text-white/50 font-mono mt-1">Control room notified</div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0B111C] text-white">
      <header className="sticky top-0 z-10 bg-[#0B111C]/95 backdrop-blur border-b border-white/[.06] px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[13px] font-bold text-white truncate">Unclamp at port</div>
      ...
      <div className="p-4 space-y-3 pb-32">
        ...
        <div>
          <label className="block text-[11px] font-medium text-white/50 mb-1">Notes (optional)</label>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="e.g. seal intact, minor scratch on top-left corner"
            className="w-full min-h-[90px] px-3 py-2 rounded-lg bg-white/[.04] border border-white/[.08] text-white text-[14px] outline-none focus:border-[#37e6ff]/50"
          />
        </div>
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline gap-3">
      <span className="text-[10px] font-mono text-white/40 uppercase tracking-wider w-20 flex-shrink-0">{label}</span>
      <span className={`text-[13px] text-white/90 ${mono ? 'font-mono' : ''} truncate`}>{value}</span>
    </div>
  );
}
