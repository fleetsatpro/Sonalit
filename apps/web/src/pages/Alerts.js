...
import { IconBell, IconGauge, IconMapPinOff, IconAlertTriangle, IconDroplet, IconTool, IconWifi, IconShield, IconCheck, IconDownload, IconLoader2, IconMapPin, IconRoute, IconSearch, IconChevronDown, IconX, } from '@tabler/icons-react';
...
const gs = (s) => SEV[s?.toLowerCase()] ?? SEV['info'];
const TYPE_ICONS = {
...
const typeIcon = (t) => TYPE_ICONS[t?.toLowerCase()] ?? <IconBell size={11}/>;
// ── Helpers ───────────────────────────────────────────────────────────────────
const aStatus = (a) => a.resolved_at ? 'resolved' : a.acknowledged_at ? 'acknowledged' : 'open';
const aTitle = (a) => a.title ?? a.message ?? a.type ?? 'Alert';
const aTime = (a) => a.triggered_at ?? a.created_at ?? '';
const fmtT = (iso) => { if (!iso)
    return '—'; try {
...
function SevBadge({ sev }) {
    const s = gs(sev);
    return (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 9px', borderRadius: 2,
...
function ExpandPanel({ a, onAck, onRes, ackBusy, resBusy }) {
    ...
    const s = gs(a.severity);
    const st = aStatus(a);
    const loc = a.lat && a.lng ? `${a.lat.toFixed(4)}, ${a.lng.toFixed(4)}` : '—';
    const col = { padding: '16px 20px', borderRight: `1px solid ${LN}` };
    const hdr = (lbl) => (<div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: '.2em', textTransform: 'uppercase',
            color: s.c, marginBottom: 11, display: 'flex', alignItems: 'center', gap: 7 }}>
      {lbl}<div style={{ flex: 1, height: 1, background: `${s.c}30` }}/>
    </div>);
    const row = (k, v, vc) => (<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    ...
    const qbtn = (icon, lbl, fn, disabled, color) => (<button type="button" onClick={fn} disabled={disabled} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 11px', borderRadius: 3,
    ...
    return (<div style={{ background: BG, borderTop: `1px solid ${LN2}`, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
      <div style={col}>
        {hdr('Alert Intel')}
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        {st === 'resolved' && (<div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: GR, padding: '8px 11px', marginBottom: 4,
...
function AlertCard({ a, open, onToggle, onAck, onRes, ackBusy, resBusy }) {
    ...
    return (<div style={{ border: `1px solid ${open ? s.c : LN}`, background: open ? S2 : S1,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '130px 110px 1fr 140px 190px 100px',
            alignItems: 'center', padding: '13px 18px 13px 22px', gap: 14, cursor: 'pointer' }} onClick={onToggle}>
        ...
        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end', alignItems: 'center' }} onClick={e => e.stopPropagation()}>
          {st === 'open' && (<button type="button" onClick={onAck} disabled={ackBusy} title="Acknowledge" style={{ width: 28, height: 28, borderRadius: 3, background: S3, border: `1px solid ${LN}`,
          ...
          <button type="button" onClick={onToggle} style={{ width: 28, height: 28, borderRadius: 3, background: S3, border: `1px solid ${LN}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: T3 }}>
...
// ── KpiCard ───────────────────────────────────────────────────────────────────
function KpiCard({ label, val, color, sub, pct, onClick }) {
    return (<button type="button" onClick={onClick} style={{ padding: '16px 20px 14px', borderRight: `1px solid ${LN}`, position: 'relative',
...
export default function Alerts() {
    ...
    const { data, isLoading, isError } = useQuery({
        queryKey: ['alerts'],
        queryFn: async () => { const r = await api.get('/alerts', { params: { limit: 200 } }); return normalizeList(r.data); },
        enabled: !!orgId, placeholderData: (prev) => prev,
    });
    useEffect(() => { if (!orgId)
        return; return subscribe(`org#${orgId}`, e => { if (e.type === 'alert.new')
        void qc.invalidateQueries({ queryKey: ['alerts'] }); }); }, [orgId, qc]);
    useEffect(() => {
        const tick = () => {
    ...
    const ackMut = useMutation({ mutationFn: (id) => api.patch(`/alerts/${id}/acknowledge`), onSuccess: () => void qc.invalidateQueries({ queryKey: ['alerts'] }) });
    const resMut = useMutation({ mutationFn: (id) => api.patch(`/alerts/${id}/resolve`), onSuccess: () => void qc.invalidateQueries({ queryKey: ['alerts'] }) });
    const all = data?.data ?? [];
    const kpi = useMemo(() => ({
    ...
    const rows = useMemo(() => {
        let list = all.filter(a => {
            if (sevFilter !== 'all' && a.severity !== sevFilter)
                return false;
            if (staFilter !== 'all' && aStatus(a) !== staFilter)
                return false;
            if (search) {
                const q = search.toLowerCase();
                if (![aTitle(a), a.type, a.vehicle_reg ?? '', a.convoy_name ?? ''].join(' ').toLowerCase().includes(q))
                    return false;
            }
            return true;
        });
        if (sort === 'sev')
            list.sort((a, b) => gs(a.severity).ord - gs(b.severity).ord);
        if (sort === 'time')
            list.sort((a, b) => new Date(aTime(b)).getTime() - new Date(aTime(a)).getTime());
        if (sort === 'type')
    ...
    const ackAll = useCallback(async () => { for (const a of all.filter(x => aStatus(x) === 'open'))
        await ackMut.mutateAsync(a.id).catch(() => { }); }, [all, ackMut]);
    const pill = (id, lbl, active, fn) => (<button key={id} type="button" onClick={fn} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '.1em', padding: '7px 13px',
            borderRadius: 3, cursor: 'pointer', textTransform: 'uppercase', border: `1px solid ${active ? RE : LN}`,
            background: active ? RE : 'transparent', color: active ? '#fff' : T3, transition: 'all .13s',
            fontWeight: active ? 700 : 400, whiteSpace: 'nowrap' }}>
      {lbl}
    </button>);
    return (<div style={{ background: BG, color: T1, minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '22px 26px 0', borderBottom: `1px solid ${LN}` }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', paddingBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[['ack', 'Ack All', () => void ackAll(), false], ['exp', 'Export', () => exportCSV(rows), false]].map(([k, lbl, fn]) => (<button key={k} type="button" onClick={fn} style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.08em', padding: '9px 16px',
      ...
      {/* Critical banner */}
      {!bannerDismissed && criticalOpen.length > 0 && (<div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 26px',
          ...
          <button type="button" onClick={() => setBannerDismissed(true)} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: T3, cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 26px',
        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 180, maxWidth: 280 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="SEARCH ALERTS…" style={{ width: '100%', background: S2, border: `1px solid ${LN2}`, borderRadius: 4,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ display: 'flex', gap: 2 }}>
            {['all', 'critical', 'warning', 'info', 'low'].map((id, i) => pill(id, ['All', 'Critical', 'Warning', 'Info', 'Low'][i], sevFilter === id, () => setSevFilter(id)))}
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: '.16em', color: T3, textTransform: 'uppercase' }}>Status</div>
          <div style={{ display: 'flex', gap: 2 }}>
            {['all', 'open', 'acknowledged', 'resolved'].map((id, i) => pill(id, ['All', 'Open', 'Acked', 'Resolved'][i], staFilter === id, () => setStaFilter(id)))}
          </div>
        </div>
        <select value={sort} onChange={e => setSort(e.target.value)} style={{ background: S2, border: `1px solid ${LN2}`, borderRadius: 4, padding: '7px 10px',
      ...
      <div style={{ flex: 1, padding: '16px 26px' }}>
        {isLoading && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 64,
                color: T3, gap: 8, fontFamily: 'var(--font-mono)', fontSize: 10 }}>
            <IconLoader2 size={16} className="animate-spin"/>LOADING ALERTS…
          </div>)}
        {isError && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 64,
                color: RE, fontFamily: 'var(--font-mono)', fontSize: 10 }}>
            FAILED TO LOAD ALERTS — CHECK NETWORK CONNECTION
          </div>)}
        {!isLoading && !isError && rows.length === 0 && (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 64, gap: 16, textAlign: 'center' }}>
        ...
        {rows.map(a => (<AlertCard key={a.id} a={a} open={expanded.has(a.id)} onToggle={() => toggle(a.id)} onAck={() => ackMut.mutate(a.id)} onRes={() => resMut.mutate(a.id)} ackBusy={ackMut.isPending && ackMut.variables === a.id} resBusy={resMut.isPending && resMut.variables === a.id}/>))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 26px',
        ...
        <div style={{ display: 'flex', gap: 6, fontFamily: 'var(--font-mono)', fontSize: 9, color: T3 }}>
          {['Alert Rules', 'History'].map(lbl => (<div key={lbl} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px',
