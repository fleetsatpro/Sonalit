...
interface DriverRow {
  ...
  driver_score?: number | null; vehicle_registration?: string | null
  employee_id?: string | null; license_class?: string | null
...
const avColor = (id: string) => AV_PALETTE[id.charCodeAt(0) % AV_PALETTE.length]!
...
type LicStatus = 'expired' | 'expiring' | 'valid' | 'unknown'
function licStatus(expiry?: string | null): LicStatus {
  if (!expiry) return 'unknown'
  const exp = new Date(expiry).getTime()
  const now = Date.now()
  if (exp < now) return 'expired'
  if (exp < now + 60 * 86400_000) return 'expiring'
  return 'valid'
}
const LIC_CFG: Record<LicStatus, { col: string; label: string; icon: string }> = {
...
function scoreColor(s: number | null | undefined) {
  if (s == null) return { col: '#55556A', bg: 'rgba(255,255,255,.04)' }
  if (s >= 85) return { col: LIME,  bg: 'rgba(200,255,0,.12)' }
  if (s >= 70) return { col: CYAN,  bg: 'rgba(0,212,255,.12)' }
  if (s >= 55) return { col: AMBER, bg: 'rgba(255,170,0,.12)' }
  return { col: RED, bg: 'rgba(255,59,107,.12)' }
}

const STATUS_CFG: Record<string, { bg: string; col: string; border: string; dot?: boolean }> = {
...
function ScoreRing({ score }: { score: number | null | undefined }) {
  ...
  if (score == null) return <span style={{ fontFamily: MN, fontSize: 11, color: '#55556A' }}>—</span>
...
function SBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 100, fontSize: 11, fontWeight: 500, fontFamily: SANS, whiteSpace: 'nowrap', background: c.bg, color: c.col, border: `1px solid ${c.border}` }}>
...
function AddModal({ onClose }: { onClose: () => void }) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<AddForm>({ resolver: zodResolver(addSchema) })
  const mut = useMutation({
    mutationFn: (d: AddForm) => api.post('/drivers', { ...d, org_id: user?.org_id }),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ['drivers'] }); onClose() },
  })
  const inp = (lbl: string, key: keyof AddForm, extra?: object) => (
    <div>
      ...
      <td colSpan={8} style={{ padding: 0 }}>
        <div style={{ background: '#1A1A20', borderTop: '1px solid rgba(255,255,255,.06)', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
          <div style={{ padding: '18px 20px', borderRight: '1px solid rgba(255,255,255,.06)' }}>
            ...
            {([['Full name', d.name], ['Employee ID', d.employee_id ?? '—'], ['Joined', d.created_at ? new Date(d.created_at).toLocaleDateString('en-GB', { month:'short', year:'numeric' }) : '—'], ['License class', d.license_class ?? '—'], ['Phone', d.phone ?? '—']] as [string,string][]).map(([k, v]) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, padding: '4px 0', borderBottom: '1px solid rgba(255,255,255,.03)' }}>
          ...
          <div style={{ padding: '18px 20px', borderRight: '1px solid rgba(255,255,255,.06)' }}>
            ...
            {([['License #', d.license_number ?? '—'], ['Expiry', d.license_expiry ?? '—'], ['Status', <span key="s" style={{ color: licCfg.col, fontFamily: MN, fontSize: 11 }}>{licCfg.icon} {licCfg.label}</span>], ['Vehicle', d.vehicle_registration ?? 'Unassigned'], ['Driver status', <SBadge key="sb" status={d.status} />]] as [string, React.ReactNode][]).map(([k, v]) => (
              <div key={String(k)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, padding: '4px 0', borderBottom: '1px solid rgba(255,255,255,.03)' }}>
                <span style={{ color: '#55556A' }}>{k}</span>
                <span style={{ color: '#9898B0', fontFamily: MN, fontSize: 11 }}>{v}</span>
              </div>
            ))}
          ...
          <div style={{ padding: '18px 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {d.status !== 'suspended' ? (
                <button onClick={() => onStatusChange(d.id, 'suspended')} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(255,59,107,.08)', border: '1px solid rgba(255,59,107,.2)', borderRadius: 6, padding: '9px 16px', fontFamily: SANS, fontSize: 12, fontWeight: 500, color: RED, cursor: 'pointer' }}>
                  🚫 Suspend Driver
                </button>
              ) : (
                <button onClick={() => onStatusChange(d.id, 'active')} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(200,255,0,.08)', border: `1px solid rgba(200,255,0,.2)`, borderRadius: 6, padding: '9px 16px', fontFamily: SANS, fontSize: 12, fontWeight: 500, color: LIME, cursor: 'pointer' }}>
                  ✓ Reactivate Driver
                </button>
              )}
...
type FilterKey = 'all' | 'active' | 'on trip' | 'inactive' | 'suspended'
type SortKey = 'name' | 'score-d' | 'score-a'

export default function Drivers() {
  const qc = useQueryClient()
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<FilterKey>('all')
  const [sort, setSort] = useState<SortKey>('score-d')
  ...
  const kpi = useMemo(() => {
    ...
    const scores = all.map(d => d.driver_score).filter((s): s is number => s != null)
  ...
  const issueDrivers = useMemo(() => all.filter(d => {
    const s = licStatus(d.license_expiry); return s === 'expired' || s === 'expiring'
  }), [all])

  const rows = useMemo(() => {
    let list = all.filter(d => {
      if (filter === 'on trip') return d.status === 'active' && !!d.vehicle_registration
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sort === 'score-d') return (b.driver_score ?? -1) - (a.driver_score ?? -1)
  ...
  return (
    <div style={{ background: '#0C0C0E', minHeight: '100%', color: '#EEEEF5', fontFamily: SANS }}>
      ...
      {/* Alert strip */}
      {!alertDismissed && issueDrivers.length > 0 && (
        <div style={{ margin: '20px 28px 0', borderRadius: 8, border: '1px solid rgba(255,59,107,.25)', background: 'rgba(255,59,107,.07)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr) 1.4fr 1.4fr', gap: 1, background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 12, overflow: 'hidden', margin: '24px 28px 0' }}>
        {([
          ...
          <div key={k.label} style={{ background: '#1A1A20', padding: '20px 20px 18px', position: 'relative' }}>
        ...
        <div style={{ background: '#1A1A20', padding: '20px 22px 18px', position: 'relative' }}>
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 3 }}>
            {Array.from({ length: kpi.total }, (_, i) => (
              <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i < kpi.validLic ? LIME : RED }} />
            ))}
          </div>
        </div>
        <div style={{ background: '#1A1A20', padding: '20px 22px 18px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '20px 28px 0', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, email, license…"
            style={{ width: '100%', background: '#1A1A20', border: '1px solid rgba(255,255,255,.11)', borderRadius: 7, padding: '9px 11px 9px 34px', fontFamily: SANS, fontSize: 13, color: '#EEEEF5', outline: 'none' }}
            onFocus={e => (e.target.style.borderColor = LIME)} onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,.11)')} />
        </div>
        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          {(['all', 'active', 'on trip', 'inactive', 'suspended'] as FilterKey[]).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ fontFamily: SANS, fontSize: 11, fontWeight: 500, padding: '7px 13px', borderRadius: 100, cursor: 'pointer', border: `1px solid ${filter === f ? LIME : 'rgba(255,255,255,.11)'}`, background: filter === f ? LIME : 'transparent', color: filter === f ? '#0C0C0E' : '#55556A', whiteSpace: 'nowrap' }}>
              {f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
        <select value={sort} onChange={e => setSort(e.target.value as SortKey)} style={{ marginLeft: 'auto', background: '#1A1A20', border: '1px solid rgba(255,255,255,.11)', borderRadius: 7, padding: '8px 12px', fontFamily: MN, fontSize: 11, color: '#9898B0', outline: 'none', cursor: 'pointer' }}>
      ...
      <div style={{ padding: '16px 28px 0', overflowX: 'auto' }}>
        ...
        {!isLoading && (
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={8} style={{ padding: '60px 12px', textAlign: 'center', fontFamily: MN, fontSize: 10, color: '#55556A', letterSpacing: '.1em' }}>NO DRIVERS MATCH YOUR SEARCH</td></tr>
              )}
              {rows.map((d, idx) => {
