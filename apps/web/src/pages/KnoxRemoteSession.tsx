...
interface Telemetry { battery_pct?: number; signal_pct?: number; gps_locked?: boolean; app_version?: string; android_version?: string; knox_version?: string; device_id?: string; }
...
export default function KnoxRemoteSession() {
  ...
  function appendLog(type: string, label: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const startRes = await api.post<{ session_id: string }>(`/guardian/devices/${deviceId}/remote-session/start`, { triggered_by: 'manual' });
        if (cancelled) return;
        ...
        pc.ontrack = (event) => {
          if (videoRef.current && event.streams[0]) {
            ...
            if (!cancelled) { setSessionStatus('live'); appendLog('stream_live', 'Stream live'); }
          }
        };
        ...
        const unsub = subscribe<{ type: string; payload: unknown }>(`session:${session_id}`, (msg) => {
          if (msg.type === 'answer' && pc.signalingState !== 'stable')
            ...
            pc.addIceCandidate(new RTCIceCandidate(msg.payload as RTCIceCandidateInit)).catch(() => {});
        });
        ...
        setTimeout(() => {
          if (!cancelled && !videoRef.current?.srcObject) { setSessionStatus('error'); appendLog('error', 'Stream timeout — no device response'); }
        }, 15000);
        ...
        if (!cancelled) {
          ...
          if (axErr.response?.status === 400 && axErr.response?.data?.error === 'NOT_DEVICE_OWNER') appendLog('error', 'Device not enrolled as Knox Device Owner');
          else if (axErr.response?.status === 409) appendLog('error', 'Session already active on this device');
          else appendLog('error', 'Failed to start remote session');
  ...
  useEffect(() => {
    if (!deviceId) return;
    api.get<{ sessions?: Recording[] }>(`/guardian/devices/${deviceId}/remote-session/recordings`)
      .then(r => setRecordings(r.data?.sessions || [])).catch(() => {});
  }, [deviceId]);

  function handleOverlayClick(e: React.MouseEvent<HTMLDivElement>) {
    if (sessionStatus !== 'live' || !sessionRef.current || !deviceId) return;
  ...
  async function mdmAction(action: string, confirm?: boolean) {
    if (!sessionRef.current || !deviceId) return;
    if (confirm && !window.confirm(`Execute: ${action}?`)) return;
    try {
    ...
    } catch { setCmdMsg({ text: `Failed: ${action}`, ok: false }); }
    setTimeout(() => setCmdMsg(null), 3000);
  }

  async function endSession() {
    if (!sessionRef.current || !deviceId) { void navigate({ to: '/guardian' }); return; }
    try {
    ...
    } catch { void navigate({ to: '/guardian' }); }
  }

  async function takeScreenshot() {
    if (!videoRef.current || !sessionRef.current || !deviceId) return;
    ...
    } catch { appendLog('error', 'Screenshot failed'); }
  }

  const statusColor = { connecting: C.amber, live: C.cyan, ended: C.sub, error: C.red }[sessionStatus] ?? C.amber;
  const logColor: Record<string, string> = { session_started: C.purple, stream_live: C.green, touch: C.blue, key: C.amber, mdm_action: C.amber, screenshot: C.green, session_ended: C.red, error: C.red };
  ...
  return (
    <div style={{ padding: 16, background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif', fontSize: 12 }}>
      ...
      {((telemetry.battery_pct ?? 100) < 15 || (telemetry.signal_pct ?? 100) < 30) && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {([['Request Location', 'request_location', C.gold, false], ['Force Check-In', 'force_checkin', C.amber, false], ['Restart App', 'restart_app', C.green, false], ['Trigger Siren', 'trigger_siren', C.red, true], ['Lock Screen', 'lock_screen', C.blue, true], ['Clear App Data', 'clear_app_data', C.amber, true]] as [string, string, string, boolean][]).map(([label, action, color, confirm]) => (
                <button key={action} onClick={() => mdmAction(action, confirm)}
                  style={{ background: `${color}14`, border: `1px solid ${color}35`, borderRadius: 4, padding: '6px 10px', color, fontSize: 10, fontFamily: 'JetBrains Mono, monospace', cursor: 'pointer', textAlign: 'left', fontWeight: 600 }}>
                  {label}
                </button>
              ))}
              <button onClick={() => setWipeConfirm(true)}
                style={{ background: `${C.red}18`, border: `2px solid ${C.red}50`, borderRadius: 4, padding: '6px 10px', color: C.red, fontSize: 10, fontFamily: 'JetBrains Mono, monospace', cursor: 'pointer', textAlign: 'left', fontWeight: 800, letterSpacing: '0.06em' }}>
                REMOTE WIPE
              </button>
            </div>
          </div>
          ...
          <div style={{ position: 'relative', background: '#0a0a0a', borderRadius: 10, overflow: 'hidden', border: `2px solid ${sessionStatus === 'live' ? C.cyan : C.border}`, aspectRatio: '9/18', maxWidth: 360, margin: '0 auto', width: '100%' }}>
            ...
            {sessionStatus !== 'live' && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(8,12,20,0.9)' }}>
                ...
                {sessionStatus === 'error' && (
                  <button onClick={() => window.location.reload()} style={{ marginTop: 12, background: `${C.amber}20`, border: `1px solid ${C.amber}50`, color: C.amber, padding: '6px 16px', borderRadius: 4, cursor: 'pointer', fontSize: 10, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700 }}>RETRY</button>
                )}
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap' }}>
            {(['HOME', 'BACK', 'RECENTS', 'POWER', 'VOLUME_UP', 'VOLUME_DOWN'] as const).map(key => (
              <button key={key} onClick={() => { if (sessionRef.current && deviceId) api.post(`/guardian/devices/${deviceId}/remote-session/inject-key`, { key, session_id: sessionRef.current }).catch(() => {}); appendLog('key', `Key: ${key}`); }}
                style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 4, padding: '5px 12px', color: C.mid, fontSize: 10, fontFamily: 'JetBrains Mono, monospace', cursor: 'pointer', fontWeight: 600 }}>
                {key.replace('_', ' ')}
              </button>
            ))}
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: C.surf, border: `2px solid ${C.red}50`, borderRadius: 8, padding: 24, maxWidth: 400, width: '100%', margin: '0 16px' }}>
            ...
            <input value={wipeText} onChange={e => setWipeText(e.target.value)} placeholder="WIPE"
              style={{ width: '100%', background: C.panel, border: `1px solid ${C.red}40`, borderRadius: 4, padding: '8px 10px', fontSize: 12, color: C.red, outline: 'none', fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, boxSizing: 'border-box', marginBottom: 12 }} />
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { if (wipeText === 'WIPE') { setWipeConfirm(false); setWipeText(''); void mdmAction('remote_wipe'); } }} disabled={wipeText !== 'WIPE'}
                style={{ flex: 1, background: C.red, border: 'none', borderRadius: 4, padding: '8px', color: '#fff', cursor: wipeText === 'WIPE' ? 'pointer' : 'not-allowed', fontSize: 11, fontFamily: 'JetBrains Mono, monospace', fontWeight: 800, opacity: wipeText === 'WIPE' ? 1 : 0.5 }}>
                EXECUTE WIPE
              </button>
            </div>
          </div>
