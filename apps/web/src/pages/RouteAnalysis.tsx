...
import {
  Navigation, Loader2, AlertTriangle, ChevronDown, ChevronUp,
  Clock, Route, ShieldAlert, CheckCircle2,
} from 'lucide-react';
...
function RiskGauge({ score }: { score: number }): React.ReactElement {
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-2 bg-gray-700 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${riskBg(score)}`}
          style={{ width: `${score}%` }}
        />
      </div>
...
function FactorBadge({ factor }: { factor: string }): React.ReactElement {
  const isDangerous = ['armed_robbery', 'carjacking', 'civil_unrest'].includes(factor);
  return (
    <span className={`text-xs px-2 py-0.5 rounded border ${isDangerous ? 'bg-red-900/50 text-red-300 border-red-700/50' : 'bg-amber-900/40 text-amber-300 border-amber-700/40'}`}>
      {FACTOR_LABELS[factor] ?? factor}
    </span>
  );
}

function RouteCard({ route, recommended = false }: { route: RouteOption; recommended?: boolean }): React.ReactElement {
  const [open, setOpen] = useState(false);
  return (
    <div className={`rounded-lg border p-3 ${recommended ? 'border-green-700/50 bg-green-900/10' : 'border-gray-700 bg-gray-800/50'}`}>
      ...
      {route.notes && (
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-300 mt-1.5"
        >
      ...
      {open && route.notes && (
        <p className="text-xs text-gray-400 mt-1 leading-relaxed">{route.notes}</p>
      )}
    </div>
  );
}

function AnalysisCard({ analysis }: { analysis: RouteAnalysis }): React.ReactElement {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
      ...
      {analysis.ai_summary && (
        <p className="text-sm text-gray-300 leading-relaxed">{analysis.ai_summary}</p>
      )}
      ...
      {expanded && (
        <div className="space-y-2 pt-1">
          {analysis.recommended_route?.label && (
            <RouteCard route={analysis.recommended_route} recommended />
          )}
          {analysis.alternate_routes?.map((r, i) => (
...
const EMPTY_FORM: AnalyseForm = {
  origin_lat: '', origin_lng: '', dest_lat: '', dest_lng: '',
  departure_time: '', avoid_night: false,
};

export default function RouteAnalysis(): React.ReactElement {
  const [form, setForm] = useState<AnalyseForm>(EMPTY_FORM);
  const [latestResult, setLatestResult] = useState<RouteAnalysis | null>(null);

  const { data: history, refetch: refetchHistory } = useQuery<{ data: RouteAnalysis[]; total: number }>({
    queryKey: ['route-analyses'],
    queryFn: () => api.get<{ data: RouteAnalysis[]; total: number }>('/routes/analyses').then((r) => r.data),
  });

  const analyse = useMutation<{ data: RouteAnalysis }, Error>({
    mutationFn: () =>
      api.post<{ data: RouteAnalysis }>('/routes/analyse', {
  ...
  const field = (key: keyof AnalyseForm, label: string, type = 'text') => (
    <div>
      ...
      <input
        ...
        className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
        placeholder={type === 'number' ? '0.00000' : undefined}
      />
    </div>
  );

  const canSubmit =
    form.origin_lat && form.origin_lng && form.dest_lat && form.dest_lng &&
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
        >
      ...
      {!latestResult && !history?.data.length && (
