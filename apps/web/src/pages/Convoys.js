...
const riskColor = (p) => RISK_COLOR[p?.toUpperCase() ?? ''] ?? '#4e5a65';
const STATUS_STYLE = {
...
const statusStyle = (s) => STATUS_STYLE[s] ?? STATUS_STYLE['draft'];
function convoyProgress(c) {
...
function SBadge({ status }) {
    ...
    return (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: MN, fontSize: 8,
...
// ─── Detail Panel ─────────────────────────────────────────────────────────────
function DetailPanel({ id, onClose, onBroadcast, onEnd }) {
    ...
    return (<div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 360, background: '#0d1014',
      ...
      <div style={{ padding: '10px 18px', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', gap: 0, position: 'relative', flexShrink: 0 }}>
        ...
        {stages.map((s, i) => {
            ...
            return (<div key={s} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, position: 'relative' }}>
      ...
      <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
        {[
            ...
            ['Region', c?.region ?? '—'],
            ['Priority', c?.priority ?? '—'],
        ].map(([k, v]) => (<div key={String(k)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>
            <span style={{ fontFamily: SANS, fontSize: 11, color: '#4e5a65' }}>{k}</span>
            <span style={{ fontFamily: MN, fontSize: 10, color: '#8a95a0', textAlign: 'right' }}>{v}</span>
          </div>))}
      </div>

      {/* trucks / vehicles */}
      {c && (c.trucks?.length || c.vehicles?.length) ? (<div style={{ padding: '12px 18px', flexShrink: 0 }}>
          <div style={{ fontFamily: MN, fontSize: 7, letterSpacing: '.22em', color: '#4e5a65', marginBottom: 8 }}>VEHICLES</div>
          {(c.trucks ?? []).slice(0, 5).map(t => (<div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>
          ...
          {(c.vehicles ?? []).slice(0, c.trucks?.length ? 0 : 5).map(v => (<div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>
              <div style={{ width: 28, height: 28, background: '#161b20', border: '1px solid rgba(255,255,255,.08)', borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4e5a65', fontSize: 13, flexShrink: 0 }}>🚛</div>
              <div>
      ...
      <div style={{ padding: '12px 18px', display: 'flex', gap: 8, borderTop: '1px solid rgba(255,255,255,.08)', flexShrink: 0, background: '#0d1014', marginTop: 'auto' }}>
        ...
        {c?.status === 'active' && (<button onClick={onBroadcast} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: MN, fontSize: 9, letterSpacing: '.12em', padding: '7px 13px', borderRadius: 3, border: '1px solid #f97316', background: '#f97316', color: '#000', fontWeight: 700, cursor: 'pointer' }}>
            📡 BROADCAST
          </button>)}
        {c?.status === 'active' && (<button onClick={onEnd} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: MN, fontSize: 9, letterSpacing: '.12em', padding: '7px 13px', borderRadius: 3, border: '1px solid #a855f7', background: 'transparent', color: '#a855f7', fontWeight: 700, cursor: 'pointer' }}>
            🏁 END CONVOY
          </button>)}
      </div>
    </div>);
...
export default function Convoys() {
    ...
    const [broadcastConvoy, setBroadcastConvoy] = useState(null);
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => api.delete(`/convoys/${id}`),
        onSuccess: () => {
            void qc.invalidateQueries({ queryKey: ['convoys'] });
            void qc.invalidateQueries({ queryKey: ['convoy-reports-overview'] });
        },
    ...
    const dispatchMutation = useMutation({
        mutationFn: (id) => api.patch(`/convoys/${id}/status`, { status: 'active' }),
        onSuccess: () => {
            void qc.invalidateQueries({ queryKey: ['convoys'] });
            void qc.invalidateQueries({ queryKey: ['convoy-reports-overview'] });
        },
    ...
    const endMutation = useMutation({
        mutationFn: (id) => api.patch(`/convoys/${id}/status`, { status: 'completing' }),
        onSuccess: () => {
            void qc.invalidateQueries({ queryKey: ['convoys'] });
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', overflow: 'hidden' }}>
      <style>{`
      ...
      <div style={{ background: 'linear-gradient(180deg,#111519 0%,#0d1014 100%)', borderBottom: '1px solid rgba(255,255,255,.08)', padding: '16px 24px 0', flexShrink: 0 }}>
        ...
        ].map(sc => (<div key={sc.label} onClick={() => setFilter(sc.filter)} style={{ background: '#0d1014', padding: '12px 18px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'background .15s' }} onMouseEnter={e => (e.currentTarget.style.background = '#111519')} onMouseLeave={e => (e.currentTarget.style.background = '#0d1014')}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 24px', background: '#0d1014', borderBottom: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: 300 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search convoys, routes, regions…" style={{ width: '100%', background: '#111519', border: '1px solid rgba(255,255,255,.08)', borderRadius: 3, fontFamily: SANS, fontSize: 12, color: '#dde3ea', padding: '6px 10px 6px 28px', outline: 'none' }} onFocus={e => (e.target.style.borderColor = '#f97316')} onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,.08)')}/>
        </div>
        <div style={{ display: 'flex', gap: 2, background: '#111519', border: '1px solid rgba(255,255,255,.08)', borderRadius: 3, padding: 2 }}>
          {['all', 'active', 'completed', 'planned', 'cancelled'].map(f => (<button key={f} onClick={() => setFilter(f)} className={`cnv-tab${filter === f ? ' on' : ''}`} style={{ fontFamily: MN, fontSize: 8, letterSpacing: '.12em', padding: '4px 10px', borderRadius: 2, cursor: 'pointer', border: 'none', background: 'none', color: '#4e5a65', transition: 'all .12s' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative', display: 'flex' }}>
        <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,.08) transparent' }}>
          ...
          {!isLoading && !isError && (<table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['CONVOY ID / NAME', 'STATUS', 'ROUTE', 'PROGRESS', 'RISK', 'VEHICLES', 'START DATE', 'ETA', ''].map((h, i) => (<th key={i} style={{ fontFamily: MN, fontSize: 7, letterSpacing: '.2em', color: '#4e5a65', padding: '8px 14px', textAlign: 'left', background: '#0d1014', borderBottom: '1px solid rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 2, whiteSpace: 'nowrap', paddingLeft: i === 0 ? 24 : 14 }}>{h}</th>))}
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 && (<tr><td colSpan={9} style={{ padding: '60px 24px', textAlign: 'center', fontFamily: MN, fontSize: 10, color: '#4e5a65', letterSpacing: '.1em' }}>NO CONVOYS MATCH YOUR SEARCH</td></tr>)}
                {rows.map((c, idx) => {
                ...
                return (<tr key={c.id} onClick={() => setSelId(isSel ? null : c.id)} className={`cnv-tr${isSel ? ' sel-row' : ''}`} style={{ borderBottom: '1px solid rgba(255,255,255,.04)', cursor: 'pointer', position: 'relative', animationDelay: `${idx * 20}ms` }}>
                      ...
                      <td style={{ padding: '11px 14px 11px 0' }}>
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }} onClick={e => e.stopPropagation()}>
                          {c.status === 'planned' && (<button onClick={() => { if (window.confirm(`Dispatch "${c.name}"? This will mark it active.`)) {
                        setDispatchingId(c.id);
                        dispatchMutation.mutate(c.id);
                    } }} disabled={dispatchingId === c.id} style={{ height: 24, padding: '0 8px', display: 'flex', alignItems: 'center', gap: 4, borderRadius: 2, background: '#f97316', border: '1px solid #f97316', cursor: 'pointer', color: '#000', fontFamily: MN, fontSize: 8, fontWeight: 700, letterSpacing: '.1em', opacity: dispatchingId === c.id ? 0.5 : 1 }}>
                              ⚡ {dispatchingId === c.id ? '…' : 'DISPATCH'}
                            </button>)}
                          {c.status === 'active' && (<button title="Broadcast" onClick={() => setBroadcastConvoy({ id: c.id, name: c.name })} className="cnv-icobtn" style={{ width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 2, background: '#161b20', border: '1px solid rgba(249,115,22,.3)', cursor: 'pointer', color: '#f97316', fontSize: 11 }}>
                              📡
                            </button>)}
                          <Link to="/convoys/$id/edit" params={{ id: c.id }} className="cnv-icobtn" style={{ width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 2, background: '#161b20', border: '1px solid rgba(255,255,255,.08)', cursor: 'pointer', color: '#4e5a65', fontSize: 11, textDecoration: 'none' }}>✏</Link>
                          <button onClick={() => window.confirm(`Delete "${c.name}"?`) && deleteMutation.mutate(c.id)} className="cnv-icobtn" style={{ width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 2, background: '#161b20', border: '1px solid rgba(255,255,255,.08)', cursor: 'pointer', color: '#4e5a65', fontSize: 11 }}>🗑</button>
                        </div>
                      </td>
                    </tr>);
            })}
        ...
        {selId && (<>
            ...
            <DetailPanel id={selId} onClose={() => setSelId(null)} onBroadcast={() => { setBroadcastConvoy(selConvoy ? { id: selConvoy.id, name: selConvoy.name } : null); setSelId(null); }} onEnd={() => {
                if (!selId || !window.confirm('Mark this convoy as completing? CFOs will be asked to submit a handover form on their final EOD.'))
                    return;
                setEndingId(selId);
      ...
      {broadcastConvoy && (<BroadcastPanel convoyId={broadcastConvoy.id} convoyName={broadcastConvoy.name} onClose={() => setBroadcastConvoy(null)}/>)}
    </div>);
}
