...
export default function Devices() {
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get('/guardian/devices');
            const raw = res.data;
            return Array.isArray(raw) ? raw : (raw?.data ?? []);
        },
        refetchInterval: 30_000,
    });
    const enrolledCount = data?.filter((d) => d.status === 'enrolled' || d.status === 'active').length ?? 0;
    ...
    return (<div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        {data && (<div className="flex gap-3 text-xs ml-1">
      ...
      {isLoading && (<div className="text-slate-400 text-sm py-8 text-center">Loading devices…</div>)}
      {isError && (<div className="text-red-400 text-sm py-8 text-center">Failed to load devices.</div>)}
      ...
      {data && (<div className="bg-slate-800 border border-slate-700 rounded-lg overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((device) => (<tr key={device.id} className="border-t border-slate-700 hover:bg-slate-750">
                  ...
                  <td className="px-4 py-3">
                    {device.battery_level !== null ? (<div className="flex items-center gap-1.5">
                        <div className="w-14 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${device.battery_level > 20 ? 'bg-green-500' : 'bg-red-500'}`} style={{ width: `${device.battery_level}%` }}/>
                        </div>
