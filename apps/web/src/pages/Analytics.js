...
function StatCard({ label, value, icon, sub }) {
    return (<div className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center gap-3">
...
export default function Analytics() {
    ...
    const { data: convoyMetrics, isLoading: convoyLoading } = useQuery({
    ...
    const { data: fleetRegions, isLoading: fleetLoading } = useQuery({
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard label="Active Convoys" value={summaryLoading ? '—' : (summary?.activeConvoys ?? '—')} icon={<Truck size={18}/>}/>
        <StatCard label="Open Alerts" value={summaryLoading ? '—' : (summary?.openAlerts ?? '—')} icon={<AlertTriangle size={18}/>}/>
        <StatCard label="Fleet Utilisation" value={summaryLoading ? '—' : `${summary?.fleetUtilisation ?? '—'}`} sub="%" icon={<Activity size={18}/>}/>
        <StatCard label="On-Time Rate" value={summaryLoading ? '—' : `${summary?.onTimeRate ?? '—'}`} sub="%" icon={<CheckCircle size={18}/>}/>
        <StatCard label="Active Vehicles" value={summaryLoading ? '—' : (summary?.activeVehicles ?? '—')} sub={summary ? `/ ${summary.totalVehicles}` : undefined} icon={<Truck size={18}/>}/>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        {convoyLoading ? (<div className="h-48 flex items-center justify-center text-gray-400 text-sm">Loading…</div>) : (<ResponsiveContainer width="100%" height={220}>
            <LineChart data={(convoyMetrics ?? []).map((d) => ({ ...d, day: fmtDay(d.day) }))}>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#111827', border: '1px solid #1f2937', borderRadius: 6 }} labelStyle={{ color: '#e5e7eb' }}/>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        {fleetLoading ? (<div className="h-48 flex items-center justify-center text-gray-400 text-sm">Loading…</div>) : (<ResponsiveContainer width="100%" height={220}>
