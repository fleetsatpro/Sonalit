...
function ChatBubble({ message }) {
    ...
    return (<div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (<div className="w-8 h-8 rounded-full bg-orange-700/60 border border-orange-600/40 flex items-center justify-center shrink-0 mt-1">
      ...
      {isUser && (<div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center shrink-0 mt-1">
          <User size={14} className="text-gray-300"/>
        </div>)}
    </div>);
}
function DrawPanel({ onClose }) {
    ...
    const createMutation = useMutation({
        mutationFn: () => api.post('/geofences', { name, type: 'both', coordinates: points }),
        onSuccess: () => {
    ...
    return (<div className="flex flex-col h-full border-l border-gray-800" style={{ minWidth: 0 }}>
      ...
      <div className="relative flex-1">
        ...
        <div className="absolute bottom-3 left-3 right-3 z-10 flex flex-col gap-2">
          {points.length > 0 && !done && (<div className="flex gap-2">
              ...
              {points.length >= 3 && (<button onClick={() => setDone(true)} className="flex items-center gap-1 px-2 py-1.5 bg-orange-600 hover:bg-orange-500 text-white text-xs rounded-lg font-medium">
                  <CheckCheck size={11}/> Finish ({points.length} pts)
                </button>)}
              {points.length < 3 && (<span className="px-2 py-1.5 bg-gray-900/90 text-gray-400 text-xs rounded-lg border border-gray-700">
                  {points.length} pt{points.length !== 1 ? 's' : ''} — need 3+
                </span>)}
            </div>)}
          {points.length === 0 && (<span className="px-2 py-1.5 bg-gray-900/90 text-gray-400 text-xs rounded-lg border border-gray-700 text-center">
              Click map to place vertices
            </span>)}
        </div>
      </div>

      {done && (<div className="shrink-0 border-t border-gray-800 p-3 space-y-2">
          <input autoFocus className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500" placeholder="Geofence name…" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && name.trim() && createMutation.mutate()}/>
          <div className="flex gap-2">
            <button onClick={() => createMutation.mutate()} disabled={!name.trim() || createMutation.isPending} className="flex-1 py-1.5 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white text-xs font-medium rounded-lg transition-colors">
...
export default function Copilot() {
    ...
    useEffect(() => {
        if (/draw.*(geofence|zone|area)|create.*(geofence|zone)/i.test(input))
            setShowDraw(true);
    }, [input]);
    const handleSend = useCallback(async () => {
        ...
        try {
            const res = await api.post('/ai/dispatch', { command: text, history });
            const responseText = res.data?.response ?? 'No response received.';
            const assistantMsg = { id: `assistant-${Date.now()}`, role: 'assistant', content: responseText, timestamp: Date.now() };
            historyRef.current = [...historyRef.current, { role: 'user', content: text }, { role: 'assistant', content: responseText }].slice(-12);
            setMessages((prev) => [...prev, assistantMsg]);
            if (/draw|map|geofence|zone/i.test(responseText) && /geofence|zone|area/i.test(text))
                setShowDraw(true);
        }
        catch {
    ...
    return (<div className="flex h-full -m-4 md:-m-6 overflow-hidden">
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <div className="flex items-center gap-2 px-6 py-3 border-b border-gray-800 shrink-0" style={{ background: 'rgba(5,8,19,0.9)' }}>
          ...
          <button onClick={() => setShowDraw((v) => !v)} className={`ml-auto flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${showDraw ? 'bg-orange-600/20 border-orange-500/50 text-orange-300' : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-orange-300 hover:border-orange-500/40'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {messages.length === 1 && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
              {SUGGESTIONS.map((s) => (<button key={s} onClick={() => setInput(s)} className="text-left px-4 py-3 bg-gray-800/80 hover:bg-gray-700/80 border border-gray-700 rounded-xl text-sm text-gray-300 transition-colors">
                  {s}
                </button>))}
            </div>)}
          <div ref={bottomRef}/>
        ...
        <div className="border-t border-gray-800 px-4 py-3 shrink-0" style={{ background: 'rgba(5,8,19,0.9)' }}>
          <div className="flex gap-2">
            <textarea className="flex-1 bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-orange-500 resize-none" placeholder="Ask the Copilot anything… (Enter to send)" rows={1} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={isLoading}/>
            <button onClick={handleSend} disabled={!input.trim() || isLoading} className="p-2.5 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 rounded-xl transition-colors" aria-label="Send">
      ...
      {showDraw && (<div className="w-80 shrink-0 flex flex-col overflow-hidden">
