...
type ClaimStatus = 'draft' | 'submitted' | 'under_review' | 'approved' | 'rejected' | 'withdrawn';
...
function StatusIcon({ status }: { status: ClaimStatus }) {
  ...
  if (status === 'under_review') return <Clock className="w-3 h-3" />;
  if (status === 'submitted') return <AlertTriangle className="w-3 h-3" />;
  return null;
}
...
export default function ClaimsPage() {
  ...
  const pdfMut = useMutation({
    mutationFn: (id: string) => api.post(`/claims/${id}/generate-pdf`, {}),
  });
        ...
        {claimsQ.data && (
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                <tr key={c.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                  ...
                  <td className="p-3 flex items-center gap-2">
                    {STATUS_TRANSITIONS[c.status].map(next => (
                      <button key={next} onClick={() => statusMut.mutate({ id: c.id, status: next })}
              ...
              {claimsQ.data.data.length === 0 && (
                <tr><td colSpan={7} className="p-8 text-center text-gray-500">No claims yet</td></tr>
              )}
            </tbody>
          </table>
        )}
