...
export default function GPS() {
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        {/* SOS alert indicator */}
        {counts.sos > 0 && (<div style={{ marginLeft: 12, display: 'flex', alignItems: 'center', gap: 6, height: 28, padding: '0 12px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.35)', borderRadius: 4, animation: 'lf-sos-pulse 2s ease-in-out infinite' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 2, padding: '0 12px' }}>
          {['map', 'list'].map(t => (<button key={t} onClick={() => setMobileTab(t)} style={{ padding: '4px 12px', borderRadius: 5, background: mobileTab === t ? 'rgba(255,255,255,.07)' : 'none', border: 'none', color: mobileTab === t ? '#dfe0db' : '#7a7e8a', cursor: 'pointer', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 12, fontWeight: 500, letterSpacing: '.04em', textTransform: 'capitalize' }}>
      ...
      {/* ── Status strip ── */}
      <StatusStrip counts={counts} active={statusFilter} onFilter={f => setStatusFilter(f)}/>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ display: (!isMobile || mobileTab === 'list') ? 'flex' : 'none', overflow: 'hidden', flexShrink: 0, ...(isMobile ? { width: '100%' } : {}) }}>
          <VehiclePanel groups={filteredGroups} selectedId={selected?.id ?? null} collapsed={isMobile ? false : !panelOpen} onToggleCollapse={() => setPanelOpen(o => !o)} onSelect={v => { handleSelect(v); if (isMobile)
        setMobileTab('map'); }}/>
        </div>
        ...
        <div style={{ flex: 1, position: 'relative', display: (!isMobile || mobileTab === 'map') ? 'flex' : 'none', minWidth: 0 }}>
          <FleetMap vehicles={allVehicles} selectedId={selected?.id ?? null} onSelect={handleSelect}/>
