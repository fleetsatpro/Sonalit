...
interface CustodyMeta {
  chain_valid: boolean;
  event_count: number;
}

export default function PortalCustody(): React.ReactElement {
  ...
  useEffect(() => {
    fetch(`${API}/portal/convoy/${encodeURIComponent(convoy_id)}/custody`, { credentials: 'include' })
      .then(async res => {
        ...
        return res.json() as Promise<{ data: CustodyEvent[]; meta: CustodyMeta }>;
      })
      .then(json => {
        if (json) { setEvents(json.data); setMeta(json.meta); }
      })
  ...
  return (
    <PortalShell>
      <header
        className="sticky top-0 z-40 border-b border-white/[0.06] px-4 py-3 flex items-center gap-3"
        style={{ background: 'rgba(7,11,22,0.95)', backdropFilter: 'blur(16px)' }}
      >
        <button
          onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/track', params: { convoy_id } })}
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5 pb-16">
        ...
        {meta && (
          <div
            className={`rounded-xl border p-4 mb-5 portal-reveal flex items-start gap-3 ${
              meta.chain_valid
        ...
        {loading && (
          <div className="space-y-3">
            {[0, 1, 2, 3].map(i => (
              <div
                key={i}
                className="h-20 rounded-xl border border-white/[0.07] animate-pulse"
                style={{ background: 'var(--p-surface)', animationDelay: `${i * 60}ms` }}
              />
            ))}
          </div>
        )}
        ...
        {!loading && !errorMsg && events.length > 0 && (
          <div
            className="rounded-xl border border-white/[0.07] p-4 portal-reveal portal-reveal-2"
            style={{ background: 'linear-gradient(145deg,var(--p-surface),var(--p-surface2))' }}
          >
            {events.map((evt, i) => (
              <div key={evt.event_id} className="hash-reveal" style={{ animationDelay: `${i * 40}ms` }}>
                <LedgerEvent
                  seq={evt.seq}
                  kind={evt.kind}
        ...
        {/* Hash explanation */}
        {events.length > 0 && (
          <div className="mt-4 rounded-xl border border-white/[0.05] p-3 portal-reveal portal-reveal-5">
