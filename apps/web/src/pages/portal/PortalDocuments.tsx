...
export default function PortalDocuments(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/portal/convoy/${convoy_id}/documents`, { credentials: 'include' })
      .then(async (res) => {
        ...
        return res.json() as Promise<{ data: PortalDocument[] }>;
      })
      .then(json => { if (json) setDocs(json.data); })
  ...
  return (
    <div className="portal-root" style={{ background: 'var(--p-bg)' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-12">
        ...
        {!loading && !errorMsg && docs.length > 0 && (
          <div className="rounded-xl border border-white/[0.07] overflow-hidden" style={CARD}>
            {docs.map((doc, idx) => {
              const typeColor = DOC_TYPE_COLORS[doc.type] ?? 'text-gray-400 border-gray-600';
              return (
                <div
                  key={doc.id}
                  className={`flex items-center gap-4 px-5 py-4 ${idx < docs.length - 1 ? 'border-b border-white/[0.05]' : ''}`}
                >
