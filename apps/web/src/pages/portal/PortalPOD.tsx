...
function PortalHeader({ onBack }: { onBack: () => void }): React.ReactElement {
  return (
...
function PODCard({ pod }: { pod: POD }): React.ReactElement {
  return (
    <div className="rounded-xl border border-white/[0.07] overflow-hidden mb-6" style={CARD}>
      {/* Header */}
      <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-white/[0.06]">
        ...
        {pod.pod_pdf_url && (
          <a
            href={pod.pod_pdf_url}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 text-xs text-orange-400 hover:text-orange-300 transition-colors shrink-0"
          >
            <Download size={13} />
            Download PDF
          </a>
      ...
      {pod.signature_url && (
        <div className="px-5 py-4 border-b border-white/[0.06]">
          ...
          <img
            ...
            className="max-h-24 rounded border border-white/[0.10] bg-white/[0.03]"
          />
        </div>
      )}
            ...
            {pod.photo_urls.map((url, idx) => (
              <a key={idx} href={url} target="_blank" rel="noopener noreferrer">
                <img
                  ...
                  className="w-24 h-24 object-cover rounded-lg border border-white/[0.10] hover:border-orange-500/40 transition-colors"
                />
              </a>
            ))}
          </div>
...
export default function PortalPOD(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/portal/convoy/${convoy_id}/pod`, { credentials: 'include' })
      .then(async (res) => {
        ...
        return res.json() as Promise<{ data: POD[] }>;
      })
      .then((json) => { if (json) setPods(json.data); })
  ...
  return (
    <div className="portal-root" style={{ background: 'var(--p-bg)' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-12">
        ...
        {!loading && !errorMsg && pods.length === 0 && (
