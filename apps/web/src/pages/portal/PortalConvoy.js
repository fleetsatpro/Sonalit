...
import { PortalShell, Badge, MapShell, TruckRow, EscortPanel, fmtDateTime, } from '../../components/portal/PortalPrimitives.js';
const API = import.meta.env['VITE_API_BASE_URL'] ?? '/api/v1';
// ── Cluster map ───────────────────────────────────────────────────────────────
function ConvoyClusterMap({ vehicles }) {
    ...
    useEffect(() => {
        ...
        const center = first ? [first.current_lng, first.current_lat] : [28, -26];
    ...
    return (<MapShell style={{ height: 220 }} className="mb-4 portal-reveal portal-reveal-2">
      <div ref={containerRef} style={{ width: '100%', height: '100%' }}/>
    </MapShell>);
}
// ── Full roster ───────────────────────────────────────────────────────────────
function FullRoster({ vehicles, coloadCount }) {
    ...
    return (<div className="rounded-xl border border-white/[0.07] overflow-hidden portal-reveal portal-reveal-3" style={{ background: 'linear-gradient(145deg,var(--p-surface),var(--p-surface2))' }}>
      ...
      <div className="p-3 flex flex-col gap-2">
        ...
        {coloadCount > 0 && (<div className="mt-3 flex items-start gap-2 p-3 rounded-lg bg-white/[0.02] border border-white/[0.05]">
...
// ── Main ─────────────────────────────────────────────────────────────────────
export default function PortalConvoy() {
    ...
    return (<PortalShell>
      <header className="sticky top-0 z-40 border-b border-white/[0.06] px-4 py-3 flex items-center gap-3" style={{ background: 'rgba(7,11,22,0.95)', backdropFilter: 'blur(16px)' }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {overview && (<p className="text-xs text-white/30">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5 pb-16">
        {loading && (<div className="space-y-3">
            {[0, 1].map(i => (<div key={i} className="h-44 rounded-xl border border-white/[0.07] animate-pulse" style={{ background: 'var(--p-surface)', animationDelay: `${i * 80}ms` }}/>))}
          </div>)}
        ...
        {overview && (<>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4 portal-reveal">
              <div className="rounded-xl border border-white/[0.07] p-3 text-center" style={{ background: 'var(--p-surface2)' }}>
            ...
            <div className="rounded-xl border border-white/[0.07] px-4 py-3 mb-4 grid grid-cols-2 gap-4 portal-reveal portal-reveal-1" style={{ background: 'var(--p-surface2)' }}>
              <div>
            ...
            {/* Cluster map */}
            {overview.vehicles.some(v => v.current_lat != null) && (<ConvoyClusterMap vehicles={overview.vehicles}/>)}

            {/* Full roster with co-load privacy */}
            <FullRoster vehicles={overview.vehicles} coloadCount={overview.coload_count}/>

            {/* Escorts */}
            {overview.escorts.length > 0 && (<div className="mt-4 rounded-xl border border-white/[0.07] overflow-hidden portal-reveal portal-reveal-4" style={{ background: 'var(--p-surface2)' }}>
            ...
            <div className="mt-5 flex flex-wrap gap-3 portal-reveal portal-reveal-5">
              <button onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/security', params: { convoy_id } })} className="flex items-center gap-1.5 text-xs font-semibold border px-3 py-2 rounded-lg transition-all" style={{ color: '#fca5a5', borderColor: 'rgba(239,68,68,0.30)', background: 'rgba(239,68,68,0.08)' }}>
                <Shield size={12}/> Live Security
              </button>
              <button onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/exceptions', params: { convoy_id } })} className="flex items-center gap-1.5 text-xs text-white/50 hover:text-red-400 border border-white/[0.07] hover:border-red-500/30 px-3 py-2 rounded-lg transition-all">
                <AlertTriangle size={12}/> Exceptions
              </button>
              <button onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/sensors', params: { convoy_id } })} className="text-xs text-white/50 hover:text-white/80 border border-white/[0.07] hover:border-white/20 px-3 py-2 rounded-lg transition-all">
                Sensors
              </button>
              <button onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/replay', params: { convoy_id } })} className="text-xs text-white/50 hover:text-white/80 border border-white/[0.07] hover:border-white/20 px-3 py-2 rounded-lg transition-all">
                Replay
              </button>
              <button onClick={() => void navigate({ to: '/portal/convoy/$convoy_id/custody', params: { convoy_id } })} className="text-xs text-white/50 hover:text-white/80 border border-white/[0.07] hover:border-white/20 px-3 py-2 rounded-lg transition-all">
                Custody Ledger
              </button>
            </div>
          </>)}
