...
function SecurityMapOverlay({ level, incidentType }: { level: SecurityLevel; incidentType: string | null }) {
  const ringRef = useRef<SVGCircleElement>(null);

  useEffect(() => { ensureMapStyle(); }, []);

  const truckClass =
    level === 'critical' ? 'security-truck-drift' :
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden portal-reveal portal-reveal-2"
      style={{ background: 'var(--p-surface2)', border: '1px solid rgba(255,255,255,0.07)', height: 220 }}>
      ...
      <svg viewBox="0 0 400 220" className="absolute inset-0 w-full h-full">
        ...
        {/* Main route */}
        <path d="M30,160 Q120,150 200,110 Q280,70 370,60"
          stroke="rgba(249,115,22,0.35)" strokeWidth="3" fill="none" strokeLinecap="round"
          strokeDasharray="6 4" />

        {/* Route highlight */}
        <path d="M30,160 Q120,150 200,110 Q280,70 370,60"
          stroke="rgba(249,115,22,0.08)" strokeWidth="10" fill="none" strokeLinecap="round" />
        ...
        {level === 'critical' && (
          <>
            <circle ref={ringRef} cx="200" cy="110" r="18" fill="none"
              stroke={ringColor} strokeWidth="2" className="security-sos-ring"
              style={{ transformOrigin: '200px 110px' }} />
            <circle cx="200" cy="110" r="18" fill="none"
              stroke={ringColor} strokeWidth="1.5" className="security-sos-ring"
              style={{ animationDelay: '0.7s', transformOrigin: '200px 110px' }} />
          </>
        )}
        {/* Warning ring */}
        {level === 'warning' && (
          <circle cx="200" cy="110" r="20" fill="none"
            stroke={ringColor} strokeWidth="1.5" opacity="0.5"
            strokeDasharray="4 3" />
        )}
        ...
        <g className={truckClass} style={{ transformOrigin: '200px 110px' }}>
          {/* Truck body */}
          <rect x="191" y="103" width="18" height="12" rx="2"
            fill={level === 'critical' ? '#ef4444' : level === 'warning' ? '#f5b042' : '#22c55e'}
            opacity="0.9" />
          {/* Cabin */}
          <rect x="191" y="105" width="7" height="8" rx="1.5"
          ...
          {/* Heading arrow */}
          <polygon points="210,107 214,110 210,113"
            fill={level === 'critical' ? '#ef4444' : level === 'warning' ? '#f5b042' : '#22c55e'}
            opacity="0.7" />
        </g>
...
export default function PortalSecurity(): React.ReactElement {
  const params = useParams({ strict: false }) as { convoy_id?: string };
  const convoy_id = params.convoy_id ?? '';
  const navigate = useNavigate();

  const [level, setLevel] = useState<SecurityLevel>('secure');
  const [activeType, setActiveType] = useState<string | null>(null);

  // Subscribe to real-time to track map state separately from status bar
  useEffect(() => {
    if (!convoy_id) return;
    return subscribe<IncidentRtEvent>(`portal#${convoy_id}`, evt => {
      if (evt.type === 'security') {
        setLevel(evt.level as SecurityLevel);
  ...
  return (
    <PortalShell>
      <header className="sticky top-0 z-20 flex items-center gap-3 px-4 py-3 border-b border-white/[0.06]"
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5 pb-16 space-y-4">
        ...
        <div className="portal-reveal portal-reveal-3">
          ...
          <IncidentFeed convoyId={convoy_id} onLevelChange={l => setLevel(l as SecurityLevel)} />
        </div>
