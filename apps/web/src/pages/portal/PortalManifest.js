...
const CARD_STYLE = { background: 'var(--p-surface)' };
...
function HandlingBadge({ handling }) {
    ...
    return (<span className={`text-xs px-2 py-0.5 rounded-full border ${colors}`}>
...
export default function PortalManifest() {
    ...
    return (<div className="portal-root" style={{ background: 'var(--p-bg)' }}>
      ...
      <div className="border-b border-white/[0.06] px-4 sm:px-6 py-4 flex items-center gap-3">
        <button onClick={() => void navigate({ to: '/portal/dashboard' })} className="text-gray-400 hover:text-white transition-colors" aria-label="Back to dashboard">
          <ArrowLeft size={18}/>
        </button>
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6 pb-12">
        ...
        {!loading && !errorMsg && manifest && (<>
            {/* Summary bar */}
            {manifest.data.length > 0 && (<div className="flex flex-wrap items-center gap-4 mb-6 text-sm text-white/50">
            ...
            {manifest.data.length === 0 && (<div className="flex flex-col items-center justify-center py-20">
            ...
            {manifest.data.map((shipment) => (<div key={shipment.shipment_id} className="mb-8">
                ...
                <div className="rounded-xl border border-white/[0.07] px-5 py-4 mb-3" style={CARD_STYLE}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {shipment.cargo_description && (<p className="text-xs text-white/50 mt-0.5">{shipment.cargo_description}</p>)}
                    </div>
                    <div className="text-right shrink-0 space-y-0.5">
                      {shipment.total_weight_kg != null && (<p className="text-xs text-white/50">
                          Weight: <span className="text-white/70">{fmtWeight(shipment.total_weight_kg)}</span>
                        </p>)}
                      {manifest.show_value && shipment.declared_value != null && (<p className="text-xs text-white/50">
                          Value: <span className="text-white/70">{fmtValue(shipment.declared_value, null)}</span>
                        </p>)}
                      {shipment.customs_ref && (<p className="text-xs text-white/50">
                ...
                {/* Items table */}
                {shipment.items.length > 0 ? (<div className="rounded-xl border border-white/[0.07] overflow-hidden" style={CARD_STYLE}>
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-white/[0.06]">
                          ...
                          {manifest.show_value && (<th className="text-right px-3 py-3 text-white/40 font-medium w-28">Value</th>)}
                          <th className="text-right px-4 py-3 text-white/40 font-medium w-28">Handling</th>
                        </tr>
                      </thead>
                      <tbody>
                        {shipment.items.map((item, idx) => (<tr key={item.id} className={idx < shipment.items.length - 1 ? 'border-b border-white/[0.04]' : ''}>
                            ...
                            {manifest.show_value && (<td className="px-3 py-3 text-white/60 text-right">
                  ...
                  </div>) : (<p className="text-xs text-white/30 px-1">No line items on this shipment's manifest.</p>)}
              </div>))}
          </>)}
      </div>
    </div>);
