...
export default function PortalReplay() {
    ...
    useEffect(() => {
        if (!mapRef.current || points.length === 0 || mapInstance.current)
            return;
    ...
    useEffect(() => {
        ...
        if (!map || !map.isStyleLoaded() || points.length === 0)
            return;
        const pt = points[cursor];
        if (!pt)
            return;
        markerRef.current?.setLngLat([pt.lng, pt.lat]);
        const src = map.getSource('traversed');
        src?.setData({
    ...
    const pt = points[cursor];
    return (<div className="portal-root flex flex-col">
      ...
      {!loading && !errorMsg && points.length === 0 && (<div className="flex-1 flex flex-col items-center justify-center py-20">
      ...
      {!loading && !errorMsg && points.length > 0 && (<>
          ...
          <div className="shrink-0 border-t border-white/[0.06] px-4 py-4" style={CARD}>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setPlaying(p => !p)} className="flex items-center justify-center w-9 h-9 rounded-full bg-orange-500 hover:bg-orange-400 transition-colors shrink-0" aria-label={playing ? 'Pause' : 'Play'}>
                {playing ? <Pause size={14} className="text-white"/> : <Play size={14} className="text-white ml-0.5"/>}
              </button>
              <input type="range" min={0} max={points.length - 1} value={cursor} onChange={e => { setPlaying(false); setCursor(parseInt(e.target.value)); }} className="flex-1 accent-orange-500" style={{ accentColor: '#f97316' }}/>
            </div>
          </div>
        </>)}
    </div>);
