...
function StatCard({ label, unit, s }) {
    if (!s)
        return (<div className="rounded-lg border border-white/[0.07] px-4 py-3" style={CARD}>
...
export default function PortalSensors() {
    ...
    const tempVals = readings.map(r => r.temperature_c).filter(x => x != null);
    const humVals = readings.map(r => r.humidity_pct).filter(x => x != null);
    const shockVals = readings.map(r => r.shock_g).filter(x => x != null);
    ...
    return (<div className="portal-root" style={{ background: 'var(--p-bg)' }}>
      <div className="border-b border-white/[0.06] px-4 sm:px-6 py-4 flex items-center gap-3">
        ...
        <div className="ml-auto flex gap-1">
          {[6, 24, 72].map(h => (<button key={h} onClick={() => setHours(h)} className={`text-xs px-2.5 py-1 rounded-md border transition-colors ${hours === h
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6 pb-12">
        ...
        {!loading && !errorMsg && !hasTemp && !hasHum && !hasShock && (<div className="flex flex-col items-center justify-center py-20">
        ...
        {!loading && !errorMsg && (hasTemp || hasHum || hasShock) && (<>
            ...
            {/* Temperature chart */}
            {hasTemp && (<div className="rounded-xl border border-white/[0.07] p-5 mb-4" style={CARD}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 4, left: -16 }}>
                    ...
                    <Tooltip contentStyle={{ background: '#0d1426', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11 }} labelStyle={{ color: 'rgba(255,255,255,0.5)' }} itemStyle={{ color: '#f97316' }}/>
            ...
            {/* Humidity chart */}
            {hasHum && (<div className="rounded-xl border border-white/[0.07] p-5 mb-4" style={CARD}>
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 4, left: -16 }}>
                    ...
                    <Tooltip contentStyle={{ background: '#0d1426', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11 }} itemStyle={{ color: '#60a5fa' }}/>
            ...
            {/* Shock events */}
            {hasShock && (<div className="rounded-xl border border-white/[0.07] p-5" style={CARD}>
                ...
                <ResponsiveContainer width="100%" height={120}>
                  <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 4, left: -16 }}>
                    ...
                    <XAxis dataKey="t" tick={{ fontSize: 10, fill: 'rgba(255,255,255,0.3)' }} interval="preserveStartEnd"/>
                    <YAxis tick={{ fontSize: 10, fill: 'rgba(255,255,255,0.3)' }}/>
                    <Tooltip contentStyle={{ background: '#0d1426', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11 }} itemStyle={{ color: '#f87171' }}/>
