...
export default function Login(): React.ReactElement {
  ...
  // Canvas background
  useEffect(() => {
    const canvas = document.getElementById('login-bg-canvas') as HTMLCanvasElement | null;
    if (!canvas) return;
    ...
    type Shooter = { x: number; y: number; vx: number; vy: number; len: number; life: number; decay: number; col: string };
    let shooters: Shooter[] = [];
    let shooterInt: ReturnType<typeof setInterval> | null = null;
    if (!isLowEnd) {
      shooterInt = setInterval(() => {
        if (Math.random() < 0.35) shooters.push({
    ...
    let ga = 0, t = 0, rafId: number;
    function loop() {
      ctx.clearRect(0, 0, W, H);
      if (t < 200) ga = Math.min(0.03, ga + 0.0002);
      ctx.globalAlpha = ga; ctx.strokeStyle = 'rgba(60,100,200,1)'; ctx.lineWidth = 0.4;
      ...
      stars.forEach((s) => {
        ...
        ctx.globalAlpha = s.op * (0.5 + 0.5 * Math.sin(s.ph)); ctx.fillStyle = s.col; ctx.fill();
      });
      ctx.globalAlpha = 1;
      shooters = shooters.filter((s) => {
        ...
        gr.addColorStop(0, s.col + s.life + ')'); gr.addColorStop(1, s.col + '0)');
        ctx.beginPath(); ctx.moveTo(s.x, s.y); ctx.lineTo(s.x - s.vx * (s.len / sp), s.y - s.vy * (s.len / sp));
        ctx.strokeStyle = gr; ctx.lineWidth = 1.4 * s.life; ctx.stroke();
        return true;
      });
      warps.forEach((w) => {
        w.life += 0.016 * w.spd;
        if (w.life > w.maxLife) { w.life = 0; w.x = Math.random() * W; w.y = Math.random() * H; w.angle = Math.random() * Math.PI * 2; }
        w.len = Math.min(w.maxLen, (w.life / w.maxLife) * w.maxLen);
        const op = Math.sin((w.life / w.maxLife) * Math.PI) * 0.18;
        ctx.beginPath(); ctx.moveTo(w.x, w.y); ctx.lineTo(w.x + Math.cos(w.angle) * w.len, w.y + Math.sin(w.angle) * w.len);
        ctx.strokeStyle = `rgba(${w.col},${op})`; ctx.lineWidth = 0.7; ctx.stroke();
      });
      particles.forEach((p) => {
        p.x += p.vx; p.y += p.vy;
        ...
        if (!isLowEnd) { ctx.shadowColor = `rgba(${p.col},.45)`; ctx.shadowBlur = 7; }
        ctx.fill(); ctx.shadowBlur = 0;
      });
      t++; rafId = requestAnimationFrame(loop);
    }
    if (!prefersRM) rafId = requestAnimationFrame(loop);
    return () => { cancelAnimationFrame(rafId!); if (shooterInt) clearInterval(shooterInt); window.removeEventListener('resize', onResize); };
  }, []);

  // Logo letter colour animation (card)
  useEffect(() => {
    const letters = Array.from(logoTextRef.current?.querySelectorAll<HTMLSpanElement>('.logo-lt') ?? []);
    ...
    function lerp(a: string, b: string, t: number) {
      const h = (c: string) => [parseInt(c.slice(1,3),16), parseInt(c.slice(3,5),16), parseInt(c.slice(5,7),16)] as const;
    ...
    function run() {
      state.forEach((s) => {
        s.prog += s.spd; s.op += s.opSpd * s.opDir;
        if (s.op >= 1) { s.op = 1; s.opDir = -1; }
        if (s.op <= 0.3) { s.op = 0.3; s.opDir = 1; }
        if (s.prog >= 1) {
          s.prog = 0; s.fromA = s.toA;
          let next: string[]; do { next = PAL[Math.floor(Math.random()*PAL.length)]!; } while (next === s.fromA);
          s.toA = next; s.spd = 0.004 + Math.random()*0.006;
        }
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      function countUp(id: string, end: number, dur: number, fmt: (v: number) => string) {
        const el = document.getElementById(id); if (!el) return;
        const s = performance.now();
        (function u(n: number) {
          const p = Math.min(1,(n-s)/dur), e = 1-Math.pow(1-p,4);
          el.textContent = fmt(Math.round(e*end)); if (p<1) requestAnimationFrame(u);
        })(performance.now());
      }
      countUp('mS', 4281, 2000, (v) => v.toLocaleString());
      countUp('mC', 142, 2000, (v) => String(v));
      const rEl = document.getElementById('mR'); if (!rEl) return;
      const rs = performance.now();
      (function u(n: number) {
        const p = Math.min(1,(n-rs)/2000), e = 1-Math.pow(1-p,4);
        rEl.textContent = (e*98.4).toFixed(1)+'%'; if (p<1) requestAnimationFrame(u);
      })(performance.now());
    }, 3600);
  ...
  const passkeyMutation = useMutation<AuthResponse, Error, string>({
    mutationFn: async (email) => {
      const opts = (await api.get<WebAuthnOptions>('/auth/webauthn/authenticate-options', { params: { email } })).data;
      const credential = await navigator.credentials.get({
        publicKey: { challenge: base64UrlToBuffer(opts.challenge as string), timeout: 60000, userVerification: 'preferred', ...(opts as Omit<PublicKeyCredentialRequestOptions, 'challenge'>) },
      });
      ...
      return (await api.post<AuthResponse>('/auth/webauthn/authenticate', {
  ...
  const onPasskeySubmit = () => {
    if (!passkeyEmail.trim()) { setAlert({ type: 'error', msg: 'Enter your email first' }); return; }
    setAlert(null);
    ...
    <>
        ...
        <div className="card" ref={cardRef} role="region" aria-label="Sign in to Sonalit">
          ...
          <div className="card-logo" aria-hidden="true">
            ...
            <div className="logo-text" ref={logoTextRef}>
              {'SONALIT'.split('').map((ch, i) => (
                <span key={i} className="logo-lt" data-i={i}>{ch}</span>
              ))}
            </div>
          </div>
          ...
          {alert && (
            <div className={`alert alert-${alert.type} show`} role="alert" aria-live="polite" aria-atomic="true">
              {alert.type === 'error'
          ...
          {mode === 'password' && (
            <form onSubmit={handleSubmit(onPasswordSubmit)} noValidate>
              <div className="fg">
                ...
                <div className="iw">
                  <input type="email" id="lEmailInput" className={`fi${errors.email?' invalid':''}`}
                    placeholder="you@company.com" autoComplete="email" autoCapitalize="none" spellCheck={false}
                    aria-required="true" aria-invalid={!!errors.email} {...register('email')} />
              ...
              <div className="fg">
                ...
                <div className="iw">
                  <input type={showPassword?'text':'password'} id="lPasswordInput" className={`fi fi-pw${errors.password?' invalid':''}`}
                    placeholder="Enter your password" autoComplete="current-password"
                    aria-required="true" aria-invalid={!!errors.password} {...register('password')} />
                  <span className="ig" aria-hidden="true" />
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', margin:'-2px 0 16px' }}>
                <div className="check-row">
                  <div className={`check-box${remembered?' checked':''}`} role="checkbox" aria-checked={remembered}
                    tabIndex={0} aria-label="Remember me for 30 days"
                    onClick={() => setRemembered((r) => !r)}
                    onKeyDown={(e) => (e.key===' '||e.key==='Enter') && setRemembered((r) => !r)}
                  >
              ...
              <button type="submit" className={`btn-login${isSubmitting?' loading':''}`}
                disabled={isSubmitting || cooldownSecs > 0}
              >
              ...
              {cooldownSecs > 0 && (
                <div className="rate-notice show" aria-live="polite">
          ...
          {mode === 'passkey' && (
            <div>
              <div className="fg">
                ...
                <div className="iw">
                  <input type="email" id="lPasskeyEmail" className="fi" placeholder="you@company.com"
                    autoComplete="email" value={passkeyEmail}
                    onChange={(e) => { setPasskeyEmail(e.target.value); setAlert(null); }} />
                  <span className="ig" aria-hidden="true" />
                </div>
              </div>
              <button className={`btn-login${passkeyMutation.isPending?' loading':''}`}
                onClick={onPasskeySubmit} disabled={passkeyMutation.isPending}
              >
                <span className="btn-inner">
                  <span className="btn-spinner" aria-hidden="true" />
      ...
      <div className="login-ticker" aria-label="Live logistics feed" aria-live="off">
        ...
        <div className="ticker-track" aria-hidden="true">
          {[...TICKER_ROUTES, ...TICKER_ROUTES].map((r, i) => (
            <span key={i} className="tick"><span className={`tdot ${r.c}`} />{r.t}</span>
          ))}
        </div>
      </div>
    </>
