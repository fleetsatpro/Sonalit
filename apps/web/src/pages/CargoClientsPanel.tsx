...
function CreateClientModal({ onClose, onCreated }: { onClose: () => void; onCreated: (c: CargoClient) => void }) {
  ...
  const mutation = useMutation({
    ...
    onError: (e: unknown) => setErr((e as { response?: { data?: { error?: string } } }).response?.data?.error ?? 'Failed'),
  });
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-white/10 p-6" style={{ background: 'rgba(13,20,38,0.98)' }}>
        ...
        <div className="space-y-3">
          {(['email', 'name', 'company'] as const).map(f => (
            <div key={f}>
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={() => mutation.mutate()} disabled={!form.email || !form.name || mutation.isPending}
            className="flex-1 rounded-lg bg-orange-600 hover:bg-orange-500 disabled:opacity-50 py-2 text-sm font-semibold text-white flex items-center justify-center gap-2">
...
function LinkConvoyModal({ client, convoys, onClose, onLinked }: {
  client: CargoClient; convoys: Convoy[]; onClose: () => void; onLinked: () => void;
}) {
      ...
      <div className="w-full max-w-md rounded-xl border border-white/10 p-6" style={{ background: 'rgba(13,20,38,0.98)' }}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <select value={convoyId} onChange={e => setConvoyId(e.target.value)}
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
        ...
        <div className="flex gap-3 mt-5">
          <button onClick={onClose} className="flex-1 rounded-lg border border-white/10 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
          <button onClick={() => mutation.mutate()} disabled={!convoyId || mutation.isPending}
            className="flex-1 rounded-lg bg-orange-600 hover:bg-orange-500 disabled:opacity-50 py-2 text-sm font-semibold text-white flex items-center justify-center gap-2">
...
export function CargoClientsPanel({ convoys }: { convoys: Convoy[] }): React.ReactElement {
  ...
  const qc = useQueryClient();
  const { data, isLoading } = useQuery<{ data: CargoClient[] }>({
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowCreate(true)}
          className="flex items-center gap-1.5 rounded-lg bg-orange-600 hover:bg-orange-500 px-3 py-1.5 text-xs font-semibold text-white">
      ...
      <div className="rounded-xl border border-white/[0.07] overflow-hidden" style={{ background: 'rgba(13,20,38,0.5)' }}>
        ...
        {clients.map((c, i) => (
          <div key={c.id} className={i > 0 ? 'border-t border-white/[0.05]' : ''}>
      ...
      {showCreate && (
        <CreateClientModal onClose={() => setShowCreate(false)} onCreated={() => {
          setShowCreate(false);
