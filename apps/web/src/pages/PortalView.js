...
import { AlertTriangle, CheckCircle2, Circle, Clock, Download, Loader2, Package, Share2, Truck, } from 'lucide-react';
const PortalMap = lazy(() => import('../components/PortalMap'));
// ── Constants ──────────────────────────────────────────────────────────────────
const API_BASE = import.meta.env['VITE_API_BASE_URL'] ?? '/api/v1';
const WS_URL = import.meta.env['VITE_CENTRIFUGO_URL']
...
const CARD_BG = { background: 'rgba(13,20,38,0.7)' };
...
export default function PortalView() {
    ...
    const portalFetch = useCallback((path) => fetch(`${API_BASE}/portal${path}`, {
        headers: { Authorization: `Bearer ${urlToken}` },
    }).then(r => {
        if (!r.ok)
            return r.json().then((b) => Promise.reject(new Error(b.error ?? `Error ${r.status}`)));
        return r.json();
    }), [urlToken]);
    ...
    useEffect(() => {
        ...
        Promise.all([
            portalFetch('/convoy'),
            portalFetch('/convoy/location').catch(() => null),
            portalFetch('/convoy/eta').catch(() => null),
            portalFetch('/convoy/checkpoints').catch(() => null),
            portalFetch('/convoy/seals').catch(() => null),
        ...
        void (async () => {
            try {
                const c = new Centrifuge(WS_URL, {
                    getToken: async () => {
                        const r = await portalFetch('/centrifuge-token');
                        return r.data.token;
                    },
                });
                if (!active)
    ...
    const noSignal = !!convoy?.departed_at && !convoy?.arrived_at && (!location?.last_ping_at ||
        Date.now() - new Date(location.last_ping_at).getTime() > 30 * 60 * 1000);
    // ── Render ─────────────────────────────────────────────────────────────────
    return (<div className="min-h-screen" style={{ background: '#050813' }}>
      ...
      <div className="border-b border-white/[0.06] px-4 sm:px-6 py-4 flex items-center justify-between">
        ...
        {convoy && (<button onClick={() => void navigator.clipboard?.writeText(window.location.href)} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white transition-colors px-3 py-1.5 rounded-lg border border-white/[0.06] hover:border-white/[0.15]" title="Copy tracking link">
            <Share2 size={13}/> Share
          </button>)}
      </div>

      {/* Loading */}
      {loading && (<div className="flex items-center justify-center py-24 text-gray-400">
          <Loader2 size={18} className="animate-spin mr-2"/> Loading shipment…
        </div>)}

      {/* Error */}
      {!loading && error && (<div className="max-w-lg mx-auto px-4 py-12">
      ...
      {/* Main content */}
      {!loading && convoy && badge && (<div className="max-w-2xl mx-auto px-4 pb-12 pt-4 space-y-4">

          {/* Title row */}
          <div className="flex items-start justify-between gap-3 pt-2">
            <div>
              <h1 className="text-xl font-bold text-white">{convoy.reference}</h1>
              {convoy.cargo_owner_ref && (<p className="text-xs text-gray-500 mt-0.5">{convoy.cargo_owner_ref}</p>)}
            </div>
          ...
          {noSignal && (<div className="flex items-center gap-2 rounded-xl border border-amber-700/50 bg-amber-900/20 px-4 py-3">
          ...
          {/* Hero map */}
          <div className="rounded-xl overflow-hidden border border-white/[0.07]" style={{ height: 'clamp(260px, 55vw, 460px)' }}>
            <Suspense fallback={<div className="flex items-center justify-center h-full text-gray-500 text-sm gap-2" style={{ background: '#060e1a' }}>
                <Loader2 size={16} className="animate-spin"/> Loading map…
              </div>}>
              <PortalMap currentLocation={location?.current_location ?? null} heading={location?.heading ?? null} trail={location?.trail ?? []} routeLine={location?.route_line ?? null} origin={convoy.origin ?? ''} destination={convoy.destination ?? ''} status={convoy.status} animatePosition={!reducedMotion} originCoords={location?.origin_coords ?? null} destinationCoords={location?.destination_coords ?? null} noSignal={noSignal}/>
            </Suspense>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className={CARD} style={CARD_BG}>
              ...
              {eta?.status === 'delayed' && (<p className="text-xs text-red-400 mt-0.5">Delayed</p>)}
              {eta?.status === 'arrived' && (<p className="text-xs text-blue-400 mt-0.5">Arrived</p>)}
            </div>

            <div className={CARD} style={CARD_BG}>
              <p className="text-xs text-gray-500 mb-2">Progress</p>
              <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div className="h-1.5 rounded-full bg-orange-500 transition-all duration-500" style={{ width: `${eta?.progress_pct ?? 0}%` }}/>
              </div>
              <p className="text-xs text-white mt-1.5">
                {eta?.progress_pct != null ? `${eta.progress_pct}%` : '—'}
                {eta?.remaining_km != null && (<span className="text-gray-500"> · {Math.round(eta.remaining_km)} km left</span>)}
              </p>
            </div>
            ...
            <div className={CARD} style={CARD_BG}>
              ...
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full shrink-0 ${location?.last_ping_at ? 'bg-green-400 animate-pulse' : 'bg-gray-600'}`}/>
            ...
            <div className={CARD} style={CARD_BG}>
              ...
              {eta?.avg_speed_kmh != null && (<p className="text-xs text-gray-500">avg {Math.round(eta.avg_speed_kmh)} km/h</p>)}
            </div>
          </div>
            ...
            <div className="flex items-stretch gap-3">
              ...
              <div className="flex flex-col gap-3 flex-1">
                <div>
                  ...
                  {convoy.departed_at && (<p className="text-xs text-gray-500">Departed {fmtTime(convoy.departed_at)}</p>)}
                </div>
                <div>
                  <p className="text-xs text-gray-500">Destination</p>
                  <p className="text-sm text-white font-medium">{convoy.destination || '—'}</p>
                  {convoy.arrived_at && (<p className="text-xs text-gray-500">Arrived {fmtTime(convoy.arrived_at)}</p>)}
                </div>
              </div>
          ...
          {/* Checkpoint timeline */}
          {checkpoints.length > 0 && (<div className={CARD} style={CARD_BG}>
              <h2 className={SECTION_TITLE}>Checkpoints</h2>
              <div className="space-y-3">
                {checkpoints.map((cp, i) => {
                    const isNext = !cp.reached && (i === 0 || (checkpoints[i - 1]?.reached ?? false));
                    return (<div key={cp.order} className="flex items-start gap-3">
                      {cp.reached
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {cp.reached_at && (<p className="text-xs text-gray-500 mt-0.5">{fmtTime(cp.reached_at)}</p>)}
                        {isNext && !cp.reached && (<p className="text-xs text-orange-400/70 mt-0.5">Next checkpoint</p>)}
                      </div>
                    </div>);
                })}
              </div>
          ...
          {/* Seal status card */}
          {seals && (<div className={CARD} style={CARD_BG}>
              <div className="flex items-center justify-between mb-4">
                <h2 className={SECTION_TITLE.replace(' mb-4', '')}>Seal Status</h2>
                <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${seals.seal_status === 'intact'
              ...
              {seals.trucks.length > 0 && (<div className="space-y-0 divide-y divide-white/[0.05]">
                  {seals.trucks.map(t => (<div key={t.truck_id} className="flex items-center justify-between py-2.5">
                      <div className="min-w-0">
                        <p className="text-sm text-white">Truck {t.position}</p>
                        {t.driver_name && (<p className="text-xs text-gray-500 truncate">{t.driver_name}</p>)}
                      </div>
          ...
          {/* PDF download */}
          <button onClick={() => void downloadPdf()} disabled={pdfLoading} className="w-full flex items-center justify-center gap-2 rounded-xl border border-orange-500/30 bg-orange-600/10 hover:bg-orange-600/20 disabled:opacity-50 px-4 py-3 text-sm font-semibold text-orange-400 transition-colors">
            {pdfLoading
