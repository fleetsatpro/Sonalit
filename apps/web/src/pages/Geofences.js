...
const RC = { safe: GRN, medium: AMB, warning: AMB, high: ORG, critical: RED };
const TC = { free: GRN, light: '#9eff44', moderate: AMB, heavy: ORG, severe: RED };
const REGIONS = {
...
const CORRIDORS = [
    { id: 'northAtlantic', name: 'North Atlantic Lane', region: 'europe', risk: 'critical', path: [[40.7, -74], [45.2, -50.5], [49.8, -25], [51.2, -8.5], [50.9, -1]] },
    { id: 'suez', name: 'Suez Canal Route', region: 'middleeast', risk: 'high', path: [[35.9, -5.5], [35.5, 5], [33.2, 20.5], [31.3, 32.5], [29.9, 34.7]] },
    { id: 'panama', name: 'Panama Canal Route', region: 'samerica', risk: 'high', path: [[25, -80], [20, -78.5], [12, -79.5], [9, -79.5], [8, -79]] },
    { id: 'singapore', name: 'Singapore Strait', region: 'asia', risk: 'warning', path: [[1.3, 103.8], [1.2, 104.6], [2.5, 107.2]] },
    { id: 'shanghai', name: 'Shanghai Approach', region: 'asia', risk: 'medium', path: [[28.5, 122], [30.5, 122.8], [31.4, 121.9]] },
    { id: 'rotterdam', name: 'Rotterdam–Hamburg', region: 'europe', risk: 'medium', path: [[51.92, 4.48], [52.37, 4.9], [52.55, 6.05], [53.55, 9.98]] },
    { id: 'dubai', name: 'Dubai Gulf', region: 'middleeast', risk: 'safe', path: [[22, 60], [24, 57.5], [25.3, 55.3]] },
    { id: 'laPacific', name: 'LA Pacific', region: 'namerica', risk: 'warning', path: [[33.7, -118.2], [30, -121], [25, -127]] },
    { id: 'mumbai', name: 'Mumbai Approach', region: 'asia', risk: 'medium', path: [[18, 70], [19, 72], [19, 72.8]] },
    { id: 'southChina', name: 'South China Sea', region: 'asia', risk: 'high', path: [[22.3, 114.2], [18, 116], [14, 115], [10, 112], [6, 108], [1.3, 105]] },
    { id: 'europe', name: 'Europe Freight Grid', region: 'europe', risk: 'medium', path: [[48.85, 2.35], [49.5, 4.5], [50.5, 8], [51, 13], [52, 20.5]] },
    { id: 'sahara', name: 'Trans-Sahara Corridor', region: 'africa', risk: 'medium', path: [[30, -5], [25, 2], [18, 10], [10, 13]] },
    { id: 'cape', name: 'Cape Town Corridor', region: 'africa', risk: 'safe', path: [[-33.9, 18.4], [-30, 25], [-26, 30]] },
];
const PREDICTIONS = [
    { title: 'Port Congestion', prob: 81, trend: 'up', detail: 'Elevated berth utilization at key hubs.' },
    { title: 'Border Delay', prob: 74, trend: 'flat', detail: 'Customs processing slowing at major ports.' },
    { title: 'Weather Disruption', prob: 69, trend: 'down', detail: 'North Atlantic storm system weakening.' },
    { title: 'Security Incident', prob: 58, trend: 'up', detail: 'Anomalous clustering near Gulf of Aden.' },
];
function sevToRisk(sev) { return sev === 'critical' ? 'critical' : sev === 'high' ? 'high' : sev === 'medium' ? 'medium' : 'warning'; }
function fmtAge(iso) { const d = Math.floor((Date.now() - new Date(iso).getTime()) / 60000); return d < 1 ? 'now' : d < 60 ? `${d}m` : d < 1440 ? `${Math.floor(d / 60)}h` : `${Math.floor(d / 1440)}d`; }
function coordsToLL(c) {
    if (typeof c === 'string') {
        try {
            return coordsToLL(JSON.parse(c));
        }
        catch {
            return null;
        }
    ...
    if (c !== null && typeof c === 'object' && !Array.isArray(c)) {
        ...
        if (Array.isArray(o['coordinates']))
            return coordsToLL(o['coordinates']);
        return null;
    }
    if (!Array.isArray(c) || c.length < 2)
        return null;
    if (Array.isArray(c[0])) {
        return c.map(p => [p[1] ?? 0, p[0] ?? 0]);
    }
    return null;
}
export default function Geofences() {
    ...
    const [selCorridorId, setSelCorridorId] = useState(null);
    ...
    const { data: vData } = useQuery({ queryKey: ['vehicles-intel'], queryFn: () => api.get('/vehicles').then(r => r.data), refetchInterval: 30_000 });
    const { data: aData } = useQuery({ queryKey: ['alerts-intel'], queryFn: () => api.get('/alerts').then(r => r.data), refetchInterval: 30_000 });
    const { data: gData } = useQuery({ queryKey: ['geofences'], queryFn: () => api.get('/geofences').then(r => ({ data: Array.isArray(r.data) ? r.data : r.data?.data ?? [] })) });
    ...
    const corridors = useMemo(() => CORRIDORS.filter(c => region === 'global' || c.region === region).map(c => ({ ...c, segs: c.path.slice(0, -1).map(() => pick(SEG_TYPES)), health: ({ safe: 92, medium: 70, warning: 55, high: 35, critical: 18 }[c.risk] || 50) })), [region]);
    const kpi = useMemo(() => ({
    ...
    useEffect(() => {
        ...
        const t = setInterval(() => {
            ...
            if (mi < msgs.length - 1 && pct > mi * (100 / msgs.length)) {
                mi++;
            }
            if (pct >= 100) {
                clearInterval(t);
    ...
    // Map init
    useEffect(() => {
        if (booting || !mapEl.current || map.current)
            return;
    ...
    useEffect(() => {
        ...
        if (!m)
            return;
        const draw = () => {
            ['gf-corr-glow', 'gf-corr-line', 'gf-corr-seg', 'gf-zones-fill', 'gf-zones-line'].forEach(id => { if (m.getLayer(id))
                m.removeLayer(id); });
            ['gf-corr-src', 'gf-zones-src'].forEach(id => { if (m.getSource(id))
                m.removeSource(id); });
            if (!layers.corridors)
                return;
            const lines = corridors.map(c => ({ type: 'Feature', properties: { risk: c.risk, id: c.id }, geometry: { type: 'LineString', coordinates: c.path.map(([lat, lng]) => [lng, lat]) } }));
            ...
            const polys = geofences.filter(g => !['corridor', 'linear', 'route'].includes((g.type || '').toLowerCase())).flatMap(g => {
                const pts = coordsToLL(g.coordinates);
                if (!pts || pts.length < 3)
                    return [];
                const closed = [...pts, pts[0]];
                const ring = closed.map(([lat, lng]) => [lng, lat]);
                return [{ type: 'Feature', properties: { name: g.name, active: g.active }, geometry: { type: 'Polygon', coordinates: [ring] } }];
            });
            if (polys.length) {
    ...
    const applyRegion = useCallback((r) => {
        setRegion(r);
        setRegionMenuOpen(false);
        const b = REGIONS[r]?.bounds;
        if (b && map.current)
            map.current.fitBounds(b, { animate: true, duration: 600 });
    }, []);
    const showToast = useCallback((msg) => {
        setToast({ msg, show: true });
        setTimeout(() => setToast(t => ({ ...t, show: false })), 7000);
    }, []);
    const fetchAI = useCallback(async () => {
        ...
        try {
            ...
            setAiText(`${highRisk > 2 ? '⚠' : '●'} ${highRisk} high-risk corridors active. Threat index ${kpi.threat}/100 · ${kpi.anomalies} anomalies. ${highRisk > 2 ? 'Activate contingency routing immediately.' : 'Standard monitoring in effect.'}`);
        }
        setAiLoading(false);
    }, [corridors, region, kpi]);
    const cmdVehicle = useCallback((action) => {
        if (!selVehicle)
            return;
        ...
        showToast(msgs[action] || 'Command sent');
    }, [selVehicle, showToast]);
    const runConsoleCmd = useCallback((raw) => {
        const addLine = (t, c) => setConsoleLines(l => [...l.slice(-29), { t, c }]);
        addLine('> ' + raw, 'info');
        const cmd = raw.toLowerCase().trim();
        if (cmd === 'help') {
            ['help — show commands', 'status — situation report', 'region <name> — switch region', 'asset count — count active assets', 'ai refresh — refresh assessment', 'clear — clear console'].forEach(l => addLine(l, 'info'));
        }
        else if (cmd === 'status')
            addLine(`REGION: ${REGIONS[region]?.name} | THREAT: ${kpi.threat}/100 | ASSETS: ${kpi.assets} | ANOMALIES: ${kpi.anomalies}`, 'ok');
        else if (cmd.startsWith('region ')) {
            const r = cmd.split(' ')[1] ?? '';
            if (r && REGIONS[r]) {
                applyRegion(r);
                addLine('Region: ' + REGIONS[r].name, 'ok');
            }
            else
                addLine('Unknown region', 'err');
        }
        else if (cmd === 'asset count')
            addLine(`Total: ${kpi.assets} | Offline: ${vehicles.filter(v => v.status === 'offline').length}`, 'ok');
        else if (cmd === 'ai refresh') {
            void fetchAI();
            addLine('AI refresh triggered', 'ok');
        }
        else if (cmd === 'clear')
            setConsoleLines([]);
        else
    ...
    const S = (obj) => obj;
    if (booting)
        return (<div style={S({ position: 'fixed', inset: 0, zIndex: 9999, background: V, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 })}>
    ...
    const secH = (label, badge) => (<div style={S({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 2px 6px', fontFamily: "'JetBrains Mono',monospace", fontSize: 8, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: MUTED, borderBottom: `1px solid ${BD}`, marginBottom: 8 })}>
      <span>{label}</span>
      {badge != null && <span style={S({ background: RAISED, border: `1px solid ${BD}`, borderRadius: 10, padding: '1px 7px', fontSize: 8 })}>{badge}</span>}
    </div>);
    const threatSevColor = (s) => s === 'critical' ? RED : s === 'high' ? ORG : AMB;
    return (<div id="gic-wrap" style={S({ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 56px)', background: V, color: TEXT, overflow: 'hidden' })}>
      <style>{css}</style>
      ...
      <header style={S({ flexShrink: 0, height: 56, display: 'flex', alignItems: 'stretch', background: 'linear-gradient(180deg,rgba(5,14,22,.99),rgba(3,10,18,.97))', borderBottom: `1px solid rgba(0,200,255,0.07)`, position: 'relative', zIndex: 1100 })}>
        ...
        {/* KPIs */}
        {[{ l: 'Threat Index', v: kpi.threat, c: kpi.threat > 70 ? RED : kpi.threat > 40 ? AMB : CYAN, s: '/100' }, { l: 'Active Assets', v: kpi.assets, c: CYAN, s: 'tracking' }, { l: 'Geofences', v: kpi.geofences, c: GRN, s: 'zones' }, { l: 'Anomalies', v: kpi.anomalies, c: kpi.anomalies > 10 ? RED : AMB, s: 'detected' }].map(({ l, v, c, s }) => (<div key={l} style={S({ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 16px', borderRight: `1px solid ${BD}`, minWidth: 82 })}>
        ...
        <div style={S({ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px' })}>
          ...
          <div style={S({ position: 'relative', maxWidth: 260, flex: 1 })}>
            ...
            {searchResults.length > 0 && search.length >= 2 && (<div style={S({ position: 'absolute', top: 'calc(100%+4px)', left: 0, right: 0, background: OVERLAY, border: `1px solid ${BD}`, borderRadius: 6, zIndex: 2000, overflow: 'hidden' })}>
                {searchResults.map((h, i) => (<div key={i} onClick={() => { const v = vehicles.find(x => x.registration === h.label); if (v) {
                setSelVehicle(v);
                setSearch('');
                setRightOpen(true);
            }
            else
                setSearch(''); }} style={S({ padding: '7px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontSize: 11 })}>
          ...
          <div style={S({ position: 'relative' })}>
            ...
            {regionMenuOpen && (<div style={S({ position: 'absolute', top: 'calc(100%+6px)', left: 0, zIndex: 1300, background: OVERLAY, border: `1px solid ${BD}`, borderRadius: 8, padding: 4, minWidth: 160 })}>
                {Object.entries(REGIONS).map(([k, { name }]) => (<div key={k} onClick={() => applyRegion(k)} style={S({ padding: '7px 12px', borderRadius: 4, color: k === region ? CYAN : SUB, background: k === region ? 'rgba(0,200,255,0.07)' : 'transparent', fontFamily: "'JetBrains Mono',monospace", fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', cursor: 'pointer' })}>
          ...
          <div style={S({ display: 'flex', gap: 2, padding: 2, background: 'rgba(255,255,255,.015)', border: `1px solid ${BD}`, borderRadius: 8 })}>
            {[['corridors', 'Corridors'], ['heatmap', 'Heatmap'], ['threats', 'Threats'], ['weather', 'Weather']].map(([k, l]) => (<button key={k} onClick={() => setLayers(ls => ({ ...ls, [k]: !ls[k] }))} style={S({ height: 24, padding: '0 9px', borderRadius: 4, fontFamily: "'JetBrains Mono',monospace", fontSize: 8, letterSpacing: '0.06em', textTransform: 'uppercase', color: layers[k] ? CYAN : MUTED, background: layers[k] ? 'rgba(0,200,255,0.07)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 })}>
        ...
        <div style={S({ display: 'flex', alignItems: 'stretch', borderLeft: `1px solid ${BD}`, flexShrink: 0 })}>
          {[['⊞', 'Dashboard', () => setDashOpen(true)], ['⏮', 'Time Machine', () => setTmOpen((o) => !o)], ['⌨', 'Console', () => setConsoleOpen((o) => !o)]].map(([ico, lbl, fn]) => (<button key={lbl} onClick={fn} style={S({ display: 'flex', alignItems: 'center', gap: 5, height: '100%', padding: '0 12px', borderRight: `1px solid ${BD}`, fontFamily: "'JetBrains Mono',monospace", fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: MUTED, cursor: 'pointer' })}>
              {ico} {lbl}
            </button>))}
      ...
      <div style={S({ flex: 1, position: 'relative', overflow: 'hidden' })}>
        ...
        {/* Drawer tabs */}
        {[{ side: 'left', open: leftOpen, fn: () => setLeftOpen(o => !o), lbl: 'Threats' }, { side: 'right', open: rightOpen, fn: () => setRightOpen(o => !o), lbl: 'Mission' }].map(({ side, open, fn, lbl }) => (!open && <div key={side} onClick={fn} style={S({ position: 'absolute', top: '50%', transform: 'translateY(-50%)', zIndex: 800, [side]: 0, cursor: 'pointer' })}>
        ...
        {drawer('left', leftOpen, 'Threat Register', () => setLeftOpen(false), (<>
            ...
            {threats.slice(0, 12).map(t => (<div key={t.id} style={S({ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 10px', borderRadius: 4, background: RAISED, border: `1px solid ${BD}`, marginBottom: 5, cursor: 'pointer' })}>
            ...
            {[...geofences.map(g => ({ name: g.name, type: g.type || 'zone', risk: 'medium', id: g.id })), ...customZones.map(z => ({ name: z.name, type: 'custom', risk: z.risk, id: z.id }))].map(g => (<div key={g.id} onClick={() => { const c = corridors.find(x => x.id === g.id); if (c) {
                setSelCorridorId(g.id);
                setCorridorPanelOpen(true);
            } }} style={S({ display: 'flex', alignItems: 'center', padding: '7px 10px', borderRadius: 4, background: RAISED, border: `1px solid ${BD}`, marginBottom: 4, cursor: 'pointer' })}>
        ...
        {drawer('right', rightOpen, 'Mission Control', () => setRightOpen(false), (<>
            {selVehicle ? (<div style={S({ background: RAISED, border: `1px solid ${BD}`, borderRadius: 8, overflow: 'hidden', marginBottom: 12 })}>
                ...
                <div style={S({ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 7 })}>
                  {[[`Speed`, selVehicle.speed != null ? Math.round(selVehicle.speed) + ' km/h' : '—'], [`Fuel`, selVehicle.fuel_level != null ? Math.round(selVehicle.fuel_level) + '%' : '—'], [`Driver`, selVehicle.driver_name || '—'], [`Region`, selVehicle.region || '—']].map(([l, v]) => (<div key={l} style={S({ display: 'flex', justifyContent: 'space-between', fontFamily: "'JetBrains Mono',monospace", fontSize: 10 })}><span style={{ color: MUTED }}>{l}</span><span style={{ fontWeight: 500 }}>{v}</span></div>))}
                </div>
                <div style={S({ display: 'flex', gap: 6, padding: '10px 14px', borderTop: `1px solid rgba(0,200,255,.07)` })}>
                  {[['Priority', 'primary', () => cmdVehicle('prioritise')], ['Hold', 'secondary', () => cmdVehicle('hold')], ['Recall', 'secondary', () => cmdVehicle('recall')], ['Divert', 'danger', () => cmdVehicle('divert')]].map(([l, cls, fn]) => (<button key={l} onClick={fn} style={S({ flex: 1, height: 28, borderRadius: 4, fontFamily: "'JetBrains Mono',monospace", fontSize: 8, letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 600, cursor: 'pointer', background: cls === 'primary' ? CYAN : cls === 'danger' ? 'rgba(240,40,85,.08)' : SURF, border: cls === 'danger' ? `1px solid rgba(240,40,85,.28)` : cls === 'secondary' ? `1px solid ${BD}` : 'none', color: cls === 'primary' ? V : cls === 'danger' ? RED : SUB })}>{l}</button>))}
                </div>
              </div>) : (<div style={S({ fontSize: 11, color: MUTED, padding: 12, background: RAISED, border: `1px solid ${BD}`, borderRadius: 4, lineHeight: 1.7, marginBottom: 10 })}>
            ...
            <div className="gic-scroll" style={S({ maxHeight: 180, overflowY: 'auto' })}>
              {[...vehicles].sort((a, b) => { const o = { offline: 0, maintenance: 1, active: 2 }; return (o[a.status] ?? 3) - (o[b.status] ?? 3); }).slice(0, 20).map(v => {
                const col = v.status === 'offline' ? MUTED : v.status === 'maintenance' ? ORG : v.fuel_level != null && v.fuel_level < 25 ? RED : GRN;
                return (<div key={v.id} onClick={() => { setSelVehicle(v); if (v.latitude && v.longitude && map.current)
                    map.current.flyTo({ center: [v.longitude, v.latitude], zoom: 12, duration: 800 }); }} style={S({ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 4, background: selVehicle?.id === v.id ? 'rgba(0,200,255,.07)' : RAISED, border: `1px solid ${selVehicle?.id === v.id ? BDACC : BD}`, marginBottom: 4, cursor: 'pointer' })}>
            ...
            {PREDICTIONS.map(p => {
                ...
                return (<div key={p.title} title={p.detail} style={S({ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 4, background: RAISED, border: `1px solid ${BD}`, marginBottom: 5 })}>
            ...
            {secH('AI Situation Assessment', <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 7, color: VIO, letterSpacing: '0.08em' }}>CLAUDE AI</span>)}
        ...
        <div style={S({ position: 'absolute', bottom: 16, right: 16, zIndex: 600, background: OVERLAY, backdropFilter: 'blur(20px)', border: `1px solid ${BD}`, borderRadius: 8, minWidth: 230, overflow: 'hidden' })}>
          ...
          <div style={S({ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' })}>
            {[['Top Threat', threats[0]?.title?.slice(0, 18) || '—', TEXT], ['Severity', (threats[0]?.sev || '—').toUpperCase(), threats[0] ? threatSevColor(threats[0].sev) : MUTED], ['Confidence', '92%', GRN]].map(([l, v, c]) => (<div key={l} style={S({ padding: '9px 11px', borderRight: `1px solid rgba(0,200,255,.07)`, display: 'flex', flexDirection: 'column', gap: 3 })}>
                ...
                <span style={S({ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, fontWeight: 700, color: c })}>{v}</span>
              </div>))}
          </div>
        </div>
          ...
          {selCorr && <>
            ...
            <div className="gic-scroll" style={S({ flex: 1, overflowY: 'auto', padding: '12px 14px' })}>
              ...
              <div style={S({ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 12 })}>
                {[['Health', selCorr.health + '%', selCorr.health > 60 ? GRN : selCorr.health > 35 ? AMB : RED], ['Segments', selCorr.segs.length, TEXT], ['Severe', selCorr.segs.filter((s) => s === 'severe' || s === 'heavy').length, RED], ['Assets', activeVehicles.filter(v => v.region === selCorr.region).length, CYAN]].map(([l, v, c]) => (<div key={l} style={S({ background: SURF, border: `1px solid ${BD}`, borderRadius: 4, padding: '10px' })}>
                    ...
                    <div style={S({ fontFamily: "'JetBrains Mono',monospace", fontSize: 16, fontWeight: 700, color: c })}>{v}</div>
                  </div>))}
              </div>
              ...
              <div style={S({ display: 'flex', gap: 6 })}>
                {[['Escalate', 'primary', () => { showToast(`ESCALATED: ${selCorr.name}`); }], ['Reroute', 'secondary', () => showToast(`Traffic rerouted from ${selCorr.name}`)], ['Close', 'danger', () => { showToast(`CLOSURE: ${selCorr.name} closed`); }]].map(([l, cls, fn]) => (<button key={l} onClick={fn} style={S({ flex: 1, height: 28, borderRadius: 4, fontFamily: "'JetBrains Mono',monospace", fontSize: 8, letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 600, cursor: 'pointer', background: cls === 'primary' ? CYAN : cls === 'danger' ? 'rgba(240,40,85,.08)' : SURF, border: cls === 'danger' ? `1px solid rgba(240,40,85,.28)` : cls === 'secondary' ? `1px solid ${BD}` : 'none', color: cls === 'primary' ? V : cls === 'danger' ? RED : SUB })}>{l}</button>))}
              </div>
            </div>
          </>}
        </div>
        ...
        {consoleOpen && <div style={S({ position: 'absolute', bottom: 80, left: '50%', transform: 'translateX(-50%)', zIndex: 2100, width: 460, background: 'rgba(1,4,7,.98)', border: `1px solid ${BDACC}`, borderRadius: 8, overflow: 'hidden' })}>
          ...
          <input autoFocus value={consoleInput} onChange={e => setConsoleInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && consoleInput.trim()) {
            runConsoleCmd(consoleInput.trim());
            setConsoleInput('');
        } if (e.key === 'Escape')
            setConsoleOpen(false); }} placeholder="> _" style={S({ width: '100%', padding: '10px 14px', background: 'transparent', border: 'none', outline: 'none', color: CYAN, fontFamily: "'JetBrains Mono',monospace", fontSize: 13, fontWeight: 500, borderTop: `1px solid rgba(0,200,255,.07)` })}/>
        </div>}
      ...
      <footer style={S({ flexShrink: 0, height: 52, display: 'flex', alignItems: 'stretch', background: SURF, borderTop: `1px solid rgba(0,200,255,.07)`, zIndex: 600 })}>
        ...
        <div className="gic-scroll" style={S({ flex: 1, overflowX: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px', overflowY: 'hidden' })}>
          ...
          {tlEvents.map(e => {
            ...
            return (<div key={e.id} style={S({ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 7, padding: '5px 12px', borderRadius: 20, background: RAISED, border: `1px solid ${BD}`, cursor: 'pointer', whiteSpace: 'nowrap' })}>
      ...
      {/* Dashboard overlay */}
      {dashOpen && <div style={S({ position: 'fixed', inset: 0, zIndex: 2200, background: 'rgba(0,0,0,.8)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center' })} onClick={e => { if (e.target === e.currentTarget)
            setDashOpen(false); }}>
        <div style={S({ background: OVERLAY, border: `1px solid ${BD}`, borderRadius: 12, padding: 28, width: 720, maxWidth: '96vw', maxHeight: '90vh', overflowY: 'auto' })}>
          ...
          <div style={S({ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 22 })}>
            {[{ l: 'Threat Index', v: kpi.threat, c: kpi.threat > 70 ? RED : AMB }, { l: 'Active Assets', v: kpi.assets, c: CYAN }, { l: 'High-Risk Corridors', v: corridors.filter(c => c.risk === 'critical' || c.risk === 'high').length, c: ORG }, { l: 'Anomalies', v: kpi.anomalies, c: kpi.anomalies > 5 ? RED : AMB }].map(({ l, v, c }) => (<div key={l} style={S({ background: RAISED, border: `1px solid ${BD}`, borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', gap: 6 })}>
          ...
          {threats.slice(0, 5).map(t => (<div key={t.id} style={S({ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 10px', borderRadius: 4, background: RAISED, border: `1px solid ${BD}`, marginBottom: 5 })}>
