...
export default function UpdateAvailableToast() {
      ...
      registration.addEventListener('updatefound', () => {
        ...
        newSW.addEventListener('statechange', () => {
          ...
          if (newSW.state === 'installed' && navigator.serviceWorker.controller) {
            if (!cancelled) setWaitingSW(newSW);
          }
        });
      });
    });
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed bottom-4 right-4 z-50 flex items-center gap-3 rounded-lg bg-slate-800 px-4 py-3 shadow-lg text-white text-sm"
    >
