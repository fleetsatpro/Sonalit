...
const EventsTicker = React.memo(function EventsTicker() {
    ...
    const getColor = (severity) => SEVERITY_COLOR[severity] ?? SEVERITY_COLOR['default'];
    return (<div style={{
            position: 'sticky',
            top: 'var(--d-top-h)',
            zIndex: 190,
      ...
      <div ref={containerRef} style={{
        ...
        {doubled.map((evt, i) => (<span key={`${evt.id}-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 24px', flexShrink: 0 }}>
            <span style={{
