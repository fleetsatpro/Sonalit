...
function KPICardComp({ card, value }) {
    ...
    return (<button onClick={() => nav({ to: card.path })} className='d-clip' style={{
            flex: '1 1 140px',
...
const KPIStrip = React.memo(function KPIStrip() {
    ...
    if (!kpi) {
        return (<div style={{ padding: '16px 16px 0', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {CARDS.map(c => (<div key={c.key} className='d-clip' style={{
    ...
    return (<div style={{ padding: '16px 16px 0', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      {CARDS.map(card => (<KPICardComp key={card.key} card={card} value={values[card.key] ?? 0}/>))}
    </div>);
});
