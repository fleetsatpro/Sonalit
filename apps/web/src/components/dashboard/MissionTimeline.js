...
const MissionTimeline = React.memo(function MissionTimeline() {
    ...
    const { data, isError, refetch } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/timeline'); return r.data.data ?? []; },
        staleTime: 60000,
    });
    useEffect(() => {
    ...
    if (isError)
        return <DataError section='Mission Timeline' onRetry={refetch}/>;
    const now = new Date();
    return (<div className='d-section-reveal d-card' style={{ padding: 16 }}>
      ...
      <div ref={scrollRef} className='d-hscroll' style={{ paddingBottom: 8, alignItems: 'flex-end', gap: 0 }}>
        ...
        {(data ?? []).map((evt, i) => {
            ...
            return (<div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, marginRight: 8, opacity: isPast ? .4 : 1 }}>
        ...
        {(!data || data.length === 0) && (<div style={{ color: 'var(--d-t3)', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, padding: '16px 0' }}>No scheduled events in next 24h</div>)}
      </div>
    </div>);
});
function SH() {
