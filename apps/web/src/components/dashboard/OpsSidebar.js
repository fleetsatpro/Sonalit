...
const SEV_COLOR = { critical: 'var(--d-fire)', high: 'var(--d-fire)', alert: 'var(--d-fire)', incident: 'var(--d-warn)', medium: 'var(--d-warn)', convoy: 'var(--d-sig)', panic: 'var(--d-fire)', low: 'var(--d-ok)' };
...
function DonutGauge({ label, value, color }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
      <svg viewBox='0 0 60 60' width={52} height={52}>
        ...
        <circle ref={circleRef} cx={30} cy={30} r={24} fill='none' stroke={color} strokeWidth={6} strokeDasharray={ARC} strokeDashoffset={ARC} strokeLinecap='round' style={{ transform: 'rotate(-90deg)', transformOrigin: '30px 30px', willChange: 'stroke-dashoffset' }}/>
...
const OpsSidebar = React.memo(function OpsSidebar() {
    ...
    useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/alerts?limit=10'); setAlerts(r.data.data ?? []); return r.data.data; },
        staleTime: 30000,
    });
    ...
    return (<aside style={{
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--d-rim2)' }}>
        ...
        {storeAlerts.slice(0, 5).map(a => (<div key={a.id} style={{ padding: '8px 0', borderBottom: '1px solid var(--d-rim)', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            ...
            {!a.acknowledged && (<button onClick={() => ackMut.mutate(a.id)} style={{ background: 'none', border: '1px solid var(--d-rim2)', borderRadius: 4, color: 'var(--d-t3)', fontSize: 9, cursor: 'pointer', padding: '2px 6px', flexShrink: 0 }}>ACK</button>)}
          </div>))}
