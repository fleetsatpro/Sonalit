...
const PanicCenter = React.memo(function PanicCenter() {
    ...
    const { data, isError, refetch } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/panic'); return r.data; },
        staleTime: 15000,
    ...
    if (isError)
        return <DataError section='Panic Center' onRetry={refetch}/>;
    const isActive = panicState?.status === 'active' || data?.status === 'active';
    return (<div className='d-section-reveal d-card' style={{ padding: 16, border: isActive ? '1px solid var(--d-fire)' : '1px solid var(--d-rim2)', animation: isActive ? 'd-crit 1.2s ease-in-out infinite' : 'none' }}>
      ...
      {!isActive ? (<>
          ...
          {(data?.response_teams ?? []).map((t, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--d-rim)', fontSize: 12 }}>
      ...
      <button onClick={handleActivate} disabled={activateMut.isPending} style={{
      ...
      {confirm && (<button onClick={() => setConfirm(false)} style={{ marginTop: 6, width: '100%', background: 'none', border: 'none', color: 'var(--d-t3)', cursor: 'pointer', fontSize: 11 }}>
          Cancel
        </button>)}
    </div>);
});
