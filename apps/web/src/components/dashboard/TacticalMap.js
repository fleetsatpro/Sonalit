...
const EA_CENTER = [35.5, 1.2];
const EA_ZOOM = 5.2;
const GAUGE_PATH = 'M 31.1 98.9 A 48 48 0 1 1 98.9 98.9';
const ARC_LEN = 226.2;
function geoCircle(lat, lng, radiusKm, steps = 48) {
    const R = 6371;
    const pts = [];
...
const SAT_STYLE = {
    ...
    layers: [
        ...
        { id: 'sat-ref-tiles', type: 'raster', source: 'sat-ref', paint: { 'raster-opacity': 0.9 } },
    ],
};
...
function updateMapData(map, data) {
    const setData = (name, fc) => map.getSource(name)?.setData(fc);
    setData('sv-vehicles', {
    ...
    setData('sv-geofences', {
        type: 'FeatureCollection',
        features: (data.geofences ?? []).reduce((acc, g) => {
            if (g.type === 'corridor' && Array.isArray(g.path) && g.path.length >= 2) {
                const coords = g.path.map(([lat, lng]) => [lng ?? 0, lat ?? 0]);
...
function gaugeOffset(value, max) { return ARC_LEN * (1 - Math.min(1, value / max)); }
function gaugeColor(type, value) {
    if (value == null)
        return 'var(--d-t4)';
    switch (type) {
...
function Gauge({ label, value, max, unit, type }) {
...
const TacticalMap = React.memo(function TacticalMap() {
    ...
    const markerRafRef = useRef(new Map());
    ...
    const { data: mapData } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/map'); return r.data; },
        staleTime: 30000, refetchInterval: 60000,
    });
    const { data: vehicles } = useQuery({
        queryKey: ['dashboard-vehicles'],
        queryFn: async () => { const r = await api.get('/dashboard/vehicles'); return r.data.data ?? []; },
    ...
    useEffect(() => {
        ...
        map.on('load', () => {
            ...
            if (pendingDataRef.current) {
                updateMapData(map, pendingDataRef.current);
    ...
    useEffect(() => {
        ...
        const onLoad = () => {
            ...
            if (currentDataRef.current)
                updateMapData(map, currentDataRef.current);
        };
    ...
    return (<div style={expanded ? { position: 'fixed', inset: 0, zIndex: 500, background: 'var(--d-void)', display: 'flex', flexDirection: 'column', padding: 0 } : { padding: '16px 16px 0' }}>
      {/* Header */}
      ...
      <div style={{ position: 'relative', borderRadius: expanded ? 0 : 12, overflow: 'hidden', border: '1px solid var(--d-rim2)', marginBottom: expanded ? 0 : 12, flex: expanded ? 1 : undefined }}>
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
          {(mapData?.convoys ?? []).filter(c => c.lat && c.lng).map(c => {
            ...
            return (<div key={c.id} id={`convoy-marker-${c.id}`} data-lng={c.lng} data-lat={c.lat} style={{ position: 'absolute', top: 0, left: 0, transform: `translate(${ix}px, ${iy}px) rotate(${c.heading}deg)`, willChange: 'transform' }}>
      ...
      {/* Vehicle chips */}
      {vehicles && vehicles.length > 0 && (<div className='d-hscroll' style={{ marginBottom: 12, gap: 8, paddingBottom: 6 }}>
          {vehicles.map(v => (<button key={v.id} onClick={() => setSelectedVehicle(v.id === selectedVehicleId ? null : v.id)} style={{ flex: '0 0 auto', scrollSnapAlign: 'start', padding: '6px 12px', background: v.id === selectedVehicleId ? 'var(--d-sg)' : 'var(--d-well)', border: `1px solid ${v.id === selectedVehicleId ? 'var(--d-sig)' : 'var(--d-rim2)'}`, borderRadius: 6, cursor: 'pointer', color: v.id === selectedVehicleId ? 'var(--d-sig)' : 'var(--d-t2)', fontSize: 11, fontFamily: 'IBM Plex Mono, monospace' }}>{v.registration}</button>))}
        </div>)}

      {/* Telemetry row */}
      {selectedVehicle && (<div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', padding: '10px 12px', background: 'var(--d-well)', borderRadius: 8, border: '1px solid var(--d-rim2)' }}>
      ...
      {/* Gauges */}
      {selectedVehicle && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 16 }}>
...
function TelItem({ label, value, color }) {
    return (<div>
...
function relSec(iso) { return Math.floor((Date.now() - new Date(iso).getTime()) / 1000); }
