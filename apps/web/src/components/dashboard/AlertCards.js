    ...
    useEffect(() => {
        const tick = () => {
            ...
            if (sec < 60)
                ...
                setVal(`${Math.floor(sec / 60)}m`);
            else
                setVal(`${Math.floor(sec / 3600)}h`);
        };
        tick();
...
function AlertCard({ alert, onAck }) {
    ...
    return (<div style={{
      ...
      {alert.summary && (<div style={{ fontSize: 11, color: 'var(--d-t2)', fontFamily: 'IBM Plex Mono, monospace', lineHeight: 1.4 }}>{alert.summary}</div>)}
      {!alert.acknowledged && (<button onClick={() => onAck(alert.id)} style={{
...
const AlertCards = React.memo(function AlertCards() {
    ...
    const { isError, refetch } = useQuery({
        ...
        queryFn: async () => {
            const r = await api.get('/dashboard/alerts?limit=10');
    ...
    const handleAck = useCallback((id) => ackMut.mutate(id), [ackMut]);
    if (isError)
        return <DataError section='Alert Cards' onRetry={refetch}/>;
...
function SectionHeader({ label, count }) {
    return (<div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      {count != null && (<span style={{ fontSize: 10, background: 'var(--d-fg)', color: 'var(--d-fire)', borderRadius: 10, padding: '1px 8px', fontFamily: 'Orbitron, sans-serif', fontWeight: 700 }}>{count}</span>)}
    </div>);
}
