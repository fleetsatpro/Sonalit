...
function CheckpointDot({ cp }) {
    ...
    return (<div title={cp.name} style={{
...
function ConvoyRowComp({ convoy }) {
    ...
    return (<div style={{
      ...
      <div style={{ padding: '0 14px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {convoy.checkpoints.length > 0 && (<div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {convoy.checkpoints.map((cp, i) => (<React.Fragment key={i}>
                <CheckpointDot cp={cp}/>
                {i < convoy.checkpoints.length - 1 && (<div style={{ flex: 1, height: 1, background: 'var(--d-rim2)', minWidth: 6 }}/>)}
              </React.Fragment>))}
          </div>)}
      </div>

      {/* Expanded details */}
      {expanded && (<div style={{ borderTop: '1px solid var(--d-rim)', padding: '10px 14px', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          <Detail label='Trucks' value={convoy.truck_count.toString()}/>
          <Detail label='Seal' value={convoy.seal_intact === true ? '✓ Intact' : convoy.seal_intact === false ? '✗ Compromised' : 'Unverified'} color={convoy.seal_intact === true ? 'var(--d-ok)' : convoy.seal_intact === false ? 'var(--d-fire)' : 'var(--d-t3)'}/>
          {convoy.checkpoints.map((cp, i) => (<Detail key={i} label={`CP${i + 1}: ${cp.name}`} value={cp.reached_at ? new Date(cp.reached_at).toLocaleTimeString([], { hour12: false }) : 'Pending'} color={cp.reached_at ? 'var(--d-ok)' : 'var(--d-t3)'}/>))}
        </div>)}
    </div>);
}
...
const ConvoyTracker = React.memo(function ConvoyTracker() {
    ...
    if (isError)
        return <DataError section='Convoy Tracker' onRetry={refetch}/>;
    return (<div className='d-section-reveal'>
      <SectionHeader />
      {storeConvoys.length === 0
            ...
            : storeConvoys.map(c => <ConvoyRowComp key={c.id} convoy={c}/>)}
    </div>);
});
