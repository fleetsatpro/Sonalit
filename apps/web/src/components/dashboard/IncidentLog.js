...
const SEV_COLOR = { critical: 'var(--d-fire)', high: 'var(--d-fire)', medium: 'var(--d-warn)', low: 'var(--d-ok)' };
function IncidentRow({ inc }) {
    ...
    return (<div style={{ background: 'var(--d-surf)', borderRadius: 8, marginBottom: 8, border: `1px solid ${color}22`, overflow: 'hidden' }}>
      <button onClick={() => setExpanded(e => !e)} style={{ width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: '10px 12px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {expanded && (<div style={{ padding: '0 12px 12px', borderTop: '1px solid var(--d-rim)', paddingTop: 10 }}>
...
function ActionBtn({ label, onClick, loading, color }) {
    return (<button onClick={onClick} disabled={loading} style={{ padding: '5px 12px', background: 'transparent', border: `1px solid ${color}66`, borderRadius: 6, color, fontSize: 10, cursor: 'pointer', fontFamily: 'IBM Plex Mono, monospace', letterSpacing: '.06em' }}>
      {loading ? '…' : label}
    </button>);
}
const IncidentLog = React.memo(function IncidentLog() {
    const { data, isError, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const r = await api.get('/incidents?status=active&limit=10');
                return r.data.data ?? [];
            }
            catch {
                return [];
    ...
    if (isError)
        return <DataError section='Incident Log' onRetry={refetch}/>;
    return (<div className='d-section-reveal d-card' style={{ padding: 16 }}>
      ...
      {(!data || data.length === 0) && (<div style={{ textAlign: 'center', color: 'var(--d-t3)', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, padding: 16 }}>No active incidents</div>)}
    </div>);
});
function SH({ count }) {
    return (<div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
      ...
      {count != null && count > 0 && (<span style={{ fontSize: 10, background: 'var(--d-fg)', color: 'var(--d-fire)', borderRadius: 10, padding: '1px 8px', fontFamily: 'Orbitron, sans-serif', fontWeight: 700 }}>{count}</span>)}
    </div>);
}
