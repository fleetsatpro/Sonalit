...
function ConfidenceBar({ confidence, color }) {
    ...
    return (<div style={{ height: 3, background: 'var(--d-lift2)', borderRadius: 2, overflow: 'hidden' }}>
...
const AIIntelligence = React.memo(function AIIntelligence() {
    const { data, isError, refetch } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/predictions'); return r.data.data ?? []; },
        staleTime: 120000,
    });
    if (isError)
        return <DataError section='AI Intelligence' onRetry={refetch}/>;
    return (<div className='d-section-reveal d-card' style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {(data ?? []).map(p => {
            ...
            return (<div key={p.id} style={{ background: 'var(--d-surf)', borderRadius: 8, padding: '12px 14px', border: `1px solid ${color}22` }}>
