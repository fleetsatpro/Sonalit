...
const BorderCrossings = React.memo(function BorderCrossings() {
    const { data, isError, refetch } = useQuery({
        ...
        queryFn: async () => { const r = await api.get('/dashboard/borders'); return r.data.data ?? []; },
        staleTime: 120000,
    });
    if (isError)
        return <DataError section='Border Crossings' onRetry={refetch}/>;
    const borders = data ?? [];
    if (borders.length === 0) {
    ...
    return (<div className='d-section-reveal d-card' style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {borders.map((b, i) => {
            ...
            return (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid var(--d-rim)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {b.active_convoys.length > 0 && (<div style={{ fontSize: 10, color: 'var(--d-sig)', fontFamily: 'IBM Plex Mono, monospace', marginTop: 2 }}>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                {b.queue_minutes != null && (<div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: 16, fontWeight: 700, color: b.status === 'held' ? 'var(--d-fire)' : b.status === 'busy' ? 'var(--d-warn)' : 'var(--d-t2)' }}>
                    {b.queue_minutes}m
                  </div>)}
              </div>
            </div>);
