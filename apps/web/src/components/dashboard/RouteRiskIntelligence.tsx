...
interface RouteRisk {
  ...
  risk: 'high' | 'medium' | 'low'; risk_score: number;
  last_incident_summary: string | null; active_convoys: string[];
}
...
function RiskBar({ score }: { score: number }) {
  ...
  return (
    <div style={{ height: 4, background: 'var(--d-lift2)', borderRadius: 2, overflow: 'hidden', marginTop: 6 }}>
...
const RouteRiskIntelligence = React.memo(function RouteRiskIntelligence() {
  const { data, isError, refetch } = useQuery<RouteRisk[]>({
    queryKey: ['dashboard-route-risk'],
    queryFn: async () => { const r = await api.get<{ data: RouteRisk[] }>('/dashboard/route-risk'); return r.data.data ?? []; },
    staleTime: 60000,
  });
        ...
        {rows.map((row, i) => {
          ...
          return (
            <div key={i} style={{ borderBottom: '1px solid var(--d-rim)', paddingBottom: 12 }}>
              ...
              {row.last_incident_summary && (
                <div style={{ fontSize: 11, color: 'var(--d-t2)', marginTop: 3 }}>{row.last_incident_summary}</div>
              )}
              {row.active_convoys.length > 0 && (
                <div style={{ fontSize: 10, color: 'var(--d-sig)', fontFamily: 'IBM Plex Mono, monospace', marginTop: 3 }}>
