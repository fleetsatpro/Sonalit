...
const Topbar = React.memo(function Topbar({ onMenuOpen, onDispatch }) {
    ...
    const { data: alertCount } = useQuery({
        ...
        queryFn: async () => {
            try {
                const r = await api.get('/dashboard/alerts?limit=20');
    ...
    return (<header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={onMenuOpen} style={{ background: 'none', border: 'none', color: 'var(--d-t2)', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center' }} className='d-desktop-hide' aria-label='Open menu'><Menu size={20} strokeWidth={1.8}/></button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, position: 'relative' }}>
        <div style={{ position: 'relative' }}>
          <button onClick={toggleBell} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, color: 'var(--d-t2)', position: 'relative', display: 'flex', alignItems: 'center' }} aria-label='Notifications'>
            <Bell size={18} strokeWidth={1.8}/>
            {(alertCount ?? 0) > 0 && (<span style={{
        ...
        }} aria-label='Quick dispatch'><Zap size={12} style={{ marginRight: 4, verticalAlign: 'middle' }}/>DISPATCH</button>
      </div>
