...
const Topbar = React.memo(function Topbar({ onMenuOpen }: TopbarProps) {
  ...
  const goBack = useCallback(() => {
    // Browser history is the source of truth for Back. Only fall back to
    // Orbit when this tab has no prior navigation entry at all.
    if (window.history.length > 1) {
      void navigate({ to: '..', from: pathname });
      return;
    }
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {showBack && (
          ...
          ><ArrowLeft size={17} strokeWidth={2} /></button>
        )}
        <Link
          to='/home'
          title='Home — Orbit'
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 4 }}>
          <div style={{ position: 'relative', width: 8, height: 8 }}>
            ...
            <div style={{ position: 'absolute', inset: -4, borderRadius: '50%', border: '1.5px solid var(--d-sig)', animation: 'd-sonar 2s ease-out infinite', opacity: 0 }} />
          </div>
          <span style={{ fontSize: 11, fontFamily: 'IBM Plex Mono, monospace', color: 'var(--d-sig)', letterSpacing: '.1em', fontWeight: 600 }}>LIVE</span>
        </div>
      </div>

      <div style={{ fontFamily: 'Space Mono, ui-monospace, monospace', fontWeight: 700, fontSize: 20, color: 'var(--d-sig)', letterSpacing: '.08em', textShadow: '0 0 20px var(--d-sglow), 0 0 40px var(--d-sg)', userSelect: 'none', fontVariantNumeric: 'tabular-nums' }}>
        {time}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, position: 'relative' }}>
        <div style={{ position: 'relative' }}>
          ...
          >
            ...
            {(alertCount ?? 0) > 0 && (
              <span style={{ position: 'absolute', top: 2, right: 2, background: 'var(--d-fire)', color: '#fff', borderRadius: '50%', width: 16, height: 16, fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Orbitron, sans-serif' }}>{Math.min(alertCount ?? 0, 99)}</span>
            )}
          </button>
