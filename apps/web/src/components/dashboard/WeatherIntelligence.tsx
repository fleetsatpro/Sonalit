...
function etaImpact(cond: string, wind: number | null): string | null {
  const c = cond.toLowerCase();
  if (c.includes('blizzard') || c.includes('storm') || c.includes('hurricane')) return 'SEVERE — expect significant delays. Consider route diversion.';
  if (c.includes('thunder')) return 'HIGH — thunderstorm risk. Advise drivers to park until clear.';
  if (c.includes('heavy rain') || c.includes('torrential')) return 'HIGH — heavy rain reduces visibility. Add 30–60 min buffer.';
  if (c.includes('rain') || c.includes('shower') || c.includes('drizzle')) return 'MODERATE — wet roads expected. Reduce speed, increase following distance.';
  if (c.includes('fog') || c.includes('mist') || c.includes('haze')) return 'MODERATE — low visibility. Use fog lights, reduce speed.';
  if (wind != null && wind >= 60) return 'MODERATE — strong crosswinds. Caution on open roads.';
  if (c.includes('dust') || c.includes('sand')) return 'MODERATE — dust reduces visibility. Keep windows closed.';
  return null;
}
...
const WeatherIntelligence = React.memo(function WeatherIntelligence() {
  ...
  return (
    <div className='d-section-reveal d-card' style={{ padding: 16 }}>
      ...
      {selected && (
        <WeatherDetail city={selected} onClose={() => setSelectedIdx(null)} />
      )}
    </div>
  );
});

function WeatherDetail({ city, onClose }: { city: WeatherCity; onClose: () => void }) {
  ...
  return (
    <div style={{ marginTop: 12, padding: '14px 16px', background: 'var(--d-surf)', border: `1px solid ${isAlert ? 'var(--d-warn)' : 'var(--d-rim)'}`, borderRadius: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
      {/* Header */}
      ...
      {/* ETA impact */}
      {impact && (
        <div style={{ padding: '8px 12px', background: 'rgba(255,144,64,.08)', border: '1px solid rgba(255,144,64,.25)', borderRadius: 6 }}>
      ...
      {!impact && (
        <div style={{ padding: '6px 12px', background: 'rgba(34,197,94,.06)', border: '1px solid rgba(34,197,94,.2)', borderRadius: 6 }}>
