...
const NEXT_MODE = { map: 'satellite', satellite: 'hybrid', hybrid: 'map' };
const MODE_LABEL = { map: 'Map', satellite: 'Satellite', hybrid: 'Hybrid' };
...
export default function CesiumLiveMap({ locations, vehicleMap, selectedId, onSelect, onDeselect, geofences, guardianDevices, }) {
    const containerRef = useRef(null);
    const viewerRef = useRef(null);
    const entitiesRef = useRef(new globalThis.Map());
    const geofenceIdsRef = useRef([]);
    const guardianRef = useRef(new globalThis.Map());
    ...
    useEffect(() => {
        ...
        for (const loc of locations) {
            ...
            if (existing) {
                ...
                if (existing.label) {
                    existing.label.text.setValue(label);
                }
            }
            else {
        ...
        for (const geo of geofences) {
            ...
            if (geo.type === 'circle' && geo.radius != null) {
                ...
                if (!Array.isArray(coords?.path) || coords.path.length < 2)
                    continue;
                const positions = coords.path.map(([lat, lng]) => Cesium.Cartesian3.fromDegrees(lng ?? 0, lat ?? 0, 0));
    ...
    if (initFailed) {
        return (<div className="flex items-center justify-center w-full h-full bg-gray-950 text-gray-500 text-sm">
        3D map unavailable
      </div>);
    }
    return (<div style={{ width: '100%', height: '100%', position: 'relative' }}>
      <div ref={containerRef} style={{ width: '100%', height: '100%' }}/>
      <button onClick={() => setMapMode(m => NEXT_MODE[m])} className="absolute bottom-4 left-4 z-10 flex items-center gap-1.5 bg-gray-900/90 hover:bg-gray-800 text-white text-xs font-medium px-3 py-2 rounded-lg border border-gray-700 shadow-lg transition-colors" title={`Switch to ${MODE_LABEL[NEXT_MODE[mapMode]]}`}>
