...
interface DmsConfig {
  id: string;
  dms_enabled: boolean;
  dms_timeout_minutes: number | null;
  dms_suspended_until: string | null;
...
export function DeadMansSwitchPanel({ deviceId, deviceName }: Props): React.ReactElement {
  const queryClient = useQueryClient();
  const [timeoutMinutes, setTimeoutMinutes] = useState<number>(60);
  const [suspendMinutes, setSuspendMinutes] = useState<number>(30);

  const { data, isLoading, isError } = useQuery<{ data: DmsConfig }>({
    queryKey: ['device-dms', deviceId],
    queryFn: () =>
      api.get<{ data: DmsConfig }>(`/guardian/devices/${deviceId}/dms`).then((r) => r.data),
  });

  const patchDms = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      api.patch(`/guardian/devices/${deviceId}/dms`, body),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        {isSuspended && (
          <div className="col-span-2 bg-yellow-900/30 border border-yellow-800/50 rounded-lg p-2.5">
      ...
      <div className="flex items-end gap-3">
        ...
        {dms.dms_enabled ? (
          <button
            type="button"
            disabled={patchDms.isPending}
            onClick={() => patchDms.mutate({ dms_enabled: false })}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-red-700 hover:bg-red-600 disabled:opacity-50 text-white text-sm rounded transition-colors"
          >
          ...
          <button
            type="button"
            disabled={patchDms.isPending}
            onClick={() => patchDms.mutate({ dms_enabled: true, dms_timeout_minutes: timeoutMinutes })}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-green-700 hover:bg-green-600 disabled:opacity-50 text-white text-sm rounded transition-colors"
          >
      ...
      {/* Suspend */}
      {dms.dms_enabled && (
        <div className="flex items-end gap-3 pt-1 border-t border-gray-800">
          <div className="flex-1">
          ...
          <button
            type="button"
            disabled={patchDms.isPending}
            onClick={() => patchDms.mutate({ suspend_minutes: suspendMinutes })}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-700 hover:bg-yellow-600 disabled:opacity-50 text-white text-sm rounded transition-colors"
          >
          ...
          {isSuspended && (
            <button
              type="button"
              disabled={patchDms.isPending}
              onClick={() => patchDms.mutate({ suspend_minutes: 0 })}
