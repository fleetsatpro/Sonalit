...
const DispatchSheet = React.memo(function DispatchSheet({ open, onClose }: DispatchSheetProps) {
  ...
  const { data: vehicles } = useQuery({
    ...
    queryFn: async () => { const r = await api.get<{data: Array<{id:string;registration:string}>}>('/vehicles'); return r.data.data; },
    staleTime: 60000,
  });
  const { data: drivers } = useQuery({
    queryKey: ['drivers-list'],
    queryFn: async () => { const r = await api.get<{data: Array<{id:string;name:string}>}>('/drivers'); return r.data.data; },
    staleTime: 60000,
  });
