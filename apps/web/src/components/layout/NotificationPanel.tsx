...
const NotificationPanel = React.memo(function NotificationPanel({ open, onClose }: NotificationPanelProps) {
  ...
  const { data: notifications = [] } = useQuery<Notification[]>({
    ...
    queryFn: async () => {
      try {
        const r = await api.get<{ data: Notification[] }>('/notifications?limit=10');
        return r.data.data ?? [];
      } catch { return []; }
    },
    staleTime: 30000,
  ...
  return (
    <div ref={ref} style={{
      ...
      {notifications.length === 0 ? (
        <div style={{ padding: 24, textAlign: 'center', color: 'var(--d-t3)', fontSize: 13, fontFamily: 'IBM Plex Mono, monospace' }}>
          No notifications
        </div>
      ) : (
