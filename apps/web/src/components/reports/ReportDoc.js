...
// ── Sub-components ────────────────────────────────────────────────────────────
const SEAL_STATUS_COLOR = {
    intact: '#22c55e',
    broken: '#ef4444',
    missing: '#f59e0b',
    replaced: '#a78bfa',
};
function PhotoBox({ url = undefined, label }) {
...
function TruckBlock({ truck, session }) {
    ...
    const sealPhoto = (pos) => photos.find(p => p.photo_type === 'seal' && p.seal_position === pos);
    const sealRecord = (pos) => seals.find(s => s.seal_position === pos);
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        {truck.cfo_name && (<span style={{ fontSize: 11, color: '#6b7280', fontFamily: 'Space Grotesk, sans-serif' }}>
            CFO: {truck.cfo_name}
          </span>)}
      </div>
      <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            ...
            {sealPositions.slice(0, 2).map(pos => (<PhotoBox key={pos} url={sealPhoto(pos)?.photo_url ?? undefined} label={`Seal ${pos}`}/>))}
          </div>
        </div>
        {/* Seal register */}
        <div>
...
function CfoUploadsSection({ uploads }) {
    ...
    return (<>
      {['sod', 'eod'].map(phase => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
              {photos.map(p => (<div key={p.id} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <a href={p.photo_url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', aspectRatio: '4/3', background: '#f3f4f6',
                        borderRadius: 4, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                    <img src={p.photo_url} alt={p.photo_type} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                  </a>
                  ...
                  {p.lat && p.lng && (<span style={{ fontSize: 7, color: '#9ca3af', fontFamily: 'JetBrains Mono, monospace' }}>
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function ReportDoc({ detail, onRegenerate, regenerating, }) {
    ...
    return (<div style={{
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 32 }}>
        {dr?.pdf_url && (<a href={dr.pdf_url} target="_blank" rel="noopener noreferrer" style={{
      ...
      <Section label="A — Convoy Details">
        ...
        {dr && (<div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {/* B — Route */}
      {detail.waypoints.length > 0 && (<Section label="B — Route Waypoints">
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11,
                fontFamily: 'JetBrains Mono, monospace' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['#', 'Time', 'Lat', 'Lng', 'Speed km/h', 'Heading'].map(h => (<th key={h} style={{ padding: '6px 10px', textAlign: 'left',
                    fontWeight: 600, color: '#6b7280', borderBottom: '1px solid #e5e7eb',
                    fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>))}
                </tr>
              </thead>
              <tbody>
                {detail.waypoints.slice(0, 20).map((wp, i) => (<tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
            ...
            {detail.waypoints.length > 20 && (<div style={{ fontSize: 10, color: '#9ca3af', padding: '6px 10px',
                    fontFamily: 'JetBrains Mono, monospace' }}>
      ...
      {detail.cfo_uploads && detail.cfo_uploads.length > 0 && (<Section label="C — CFO App Photos">
          <CfoUploadsSection uploads={detail.cfo_uploads}/>
        </Section>)}

      {/* D — SOD (legacy device photos) */}
      {detail.trucks.some(t => t.photos.some(p => p.session === 'sod')) && (<Section label="D — Start of Day (SOD)">
          {detail.trucks.map(t => <TruckBlock key={t.id} truck={t} session="sod"/>)}
        </Section>)}

      {/* E — EOD (legacy device photos) */}
      {detail.trucks.some(t => t.photos.some(p => p.session === 'eod')) && (<Section label="E — End of Day (EOD)">
      ...
      <Section label="E — Summary & Certification">
        ...
        <div style={{ marginTop: 40, borderTop: '1px solid #e5e7eb', paddingTop: 16,
          ...
          {['Dispatcher Signature', 'CFO Lead Signature', 'Date'].map(label => (<div key={label}>
