...
function getRequestId() {
    return window._lastRequestId
        ?? `client-${Math.random().toString(36).slice(2)}`;
}
export function RootErrorComponent({ error, reset }) {
    ...
    return (<div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-slate-800 border border-slate-700 rounded-xl p-6 text-center">
        ...
        <div className="flex gap-2 justify-center">
          <button onClick={reset} className="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-500 rounded-lg text-sm font-medium transition-colors">
            <RefreshCw size={14}/>
            Reload
          </button>
          <a href={`https://github.com/fleetsatpro/sonalit/issues/new?title=Error: ${encodeURIComponent(error.message)}&body=Request+ID:+${encodeURIComponent(requestId)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm font-medium transition-colors">
