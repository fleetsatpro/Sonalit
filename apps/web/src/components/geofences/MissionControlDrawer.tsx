import { X, Truck, MapPin, Gauge, Fuel as FuelIcon, Wifi, TrendingUp, Sparkles } from 'lucide-react';
...
export default function MissionControlDrawer({
  open, onClose, selectedVehicle, vehicles, selectedVehicleId, onSelectVehicle, onSetStatus,
  ...
  return (
    <div className={`absolute top-0 right-0 h-full w-80 bg-black/90 backdrop-blur border-l border-gray-700 flex flex-col transition-transform duration-300 ${open ? '' : 'translate-x-full'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        ...
        <div>
          ...
          <div className="max-h-40 overflow-y-auto space-y-1.5">
            {vehicles.map(v => (
              <button key={v.id} onClick={() => onSelectVehicle(v)} className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded text-sm font-semibold ${selectedVehicleId === v.id ? 'bg-cyan-900/40 text-cyan-300' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
