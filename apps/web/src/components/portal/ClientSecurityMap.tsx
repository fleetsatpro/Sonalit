...
export function ClientSecurityMap({
  ...
  const load = React.useCallback(async () => {
    try {
      ...
      if (vRes.status === 401 || rRes.status === 401) {
  ...
  useEffect(() => {
    if (selected && !vehicles.some((v) => v.vehicle_id === selected)) setSelected(null);
  }, [selected, vehicles]);

  const points = useMemo(
    () => vehicles
      .filter((v) => isValidPosition(v.current_lat, v.current_lng))
      .map((v) => ({
        id: v.vehicle_id,
        lat: v.current_lat as number,
        lng: v.current_lng as number,
        label: v.registration,
        alerting: v.carries_my_cargo && alerting,
      })),
    [vehicles, alerting],
  );
  ...
  const geo = useMemo(
    () => ({
      ...
      geometry: {
        ...
        coordinates: validReplay.map((p) => [p.lng, p.lat]),
      },
      properties: {},
    }),
    [validReplay],
    ...
    <div className="relative overflow-hidden rounded-2xl border border-white/[.08] bg-[#06101a]" style={{ height: 360 }}>
      ...
      >
        ...
        {validReplay.length > 1 && (
          <Source id="security-replay" type="geojson" data={geo}>
            <Layer
              id="security-replay-line"
              type="line"
              paint={{
                'line-color': '#f97316',
                'line-width': 4,
        ...
        {points.map((p) => (
          <Marker key={p.id} longitude={p.lng} latitude={p.lat} anchor="center">
            <button
              ...
              className={`flex h-10 w-10 items-center justify-center rounded-full border-2 bg-[#08131e] shadow-[0_0_0_6px_rgba(249,115,22,.08)] transition hover:scale-110 ${
                selected === p.id
                  ? 'border-white bg-orange-500 text-white'
                  : p.alerting
                    ? 'border-red-300/80 text-red-300'
                    : 'border-orange-300/70 text-orange-300'
              }`}
            >
              <Truck size={15} />
            </button>
      ...
      <div className="absolute left-3 top-3 rounded-lg border border-white/10 bg-[#071019]/90 px-2.5 py-1.5 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <span className={`h-1.5 w-1.5 rounded-full ${points.length ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
          <span className="text-[9px] font-bold uppercase tracking-[.16em] text-white/70">Security</span>
          <span className="font-mono text-[9px] text-white/25">{points.length} positioned</span>
        </div>
      </div>
      ...
      <div className="absolute right-3 top-3 flex gap-2">
        ...
        {onFullscreen && (
          <button
            ...
            className="rounded-lg border border-white/10 bg-[#071019]/90 p-2 text-white/45 hover:text-white/80"
          >
            <Maximize2 size={14} />
          </button>
        )}
      ...
      {selectedVehicle && (
        <div className="absolute bottom-3 right-3 w-[230px] rounded-xl border border-white/10 bg-[#071019]/95 p-3 backdrop-blur-md">
          <div className="flex items-center justify-between gap-3">
            <span className="truncate font-mono text-[11px] text-white/80">{selectedVehicle.registration || 'Vehicle'}</span>
            <span className={`shrink-0 font-mono text-[9px] ${selectedVehicle.last_ping_at && age(selectedVehicle.last_ping_at).endsWith('s') ? 'text-emerald-300' : 'text-white/30'}`}>
