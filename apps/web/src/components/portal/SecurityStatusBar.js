...
function ensureSweepStyle() {
    if (typeof document === 'undefined')
        return;
    if (document.getElementById(SWEEP_STYLE_ID))
        return;
...
// ── Component ─────────────────────────────────────────────────────────────────
export function SecurityStatusBar({ convoyId, initialStatus = null }) {
    ...
    useEffect(() => {
        if (!convoyId)
            return;
        const unsub = subscribe(`portal#${convoyId}`, evt => {
    ...
    if (forbidden)
        return (<div className="flex gap-3 p-4 rounded-xl border border-red-700/40 bg-red-900/20 mb-4">
    ...
    if (!cfg)
        return null;
    const { Icon } = cfg;
    return (<div className={`relative overflow-hidden rounded-xl border px-4 py-3 mb-4 portal-reveal${cfg.sweep ? ' security-sweep-anim' : ''}`} style={{ background: cfg.bg, borderColor: cfg.border }} role="status" aria-live="polite">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {status.last_checkin_at && (<p className="mono text-[11px] text-white/30 mt-1">
              Last check-in: {relativeTime(status.last_checkin_at)}
            </p>)}
        </div>
        {/* Status dot */}
        <span className={`w-2.5 h-2.5 rounded-full shrink-0 mt-1 ${cfg.dot}${status.level === 'critical' ? ' security-dot-pulse' : ''}`} aria-hidden="true"/>
      </div>
    </div>);
}
