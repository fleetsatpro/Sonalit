...
export function PortalShell({ children, className = '' }) {
    return (<div className={`portal-root relative overflow-x-hidden ${className}`} style={{ background: 'var(--p-bg)', minHeight: '100dvh' }}>
      {children}
    </div>);
}
const ACCENT = {
...
export function StatCard({ label, value, sub, accent = 'default', className = '' }) {
    return (<div className={`rounded-xl border p-4 shadow-lg ${ACCENT[accent]} ${className}`} style={{ background: 'linear-gradient(145deg,var(--p-surface),var(--p-surface2))' }}>
...
export function Badge({ label, variant }) {
    ...
    return (<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${cls}`}>
      {label}
    </span>);
}
// ── ProgressBar ───────────────────────────────────────────────────────────────
export function ProgressBar({ pct, className = '' }) {
    return (<div className={`h-1.5 rounded-full bg-white/10 overflow-hidden ${className}`}>
      <div className="h-full rounded-full bg-orange-500 transition-all duration-700" style={{ width: `${Math.min(Math.max(pct, 0), 100)}%` }}/>
    </div>);
}
// ── RouteRibbon ───────────────────────────────────────────────────────────────
export function RouteRibbon({ origin, destination, progress }) {
    return (<div className="flex items-center gap-2 text-xs w-full overflow-hidden">
      ...
      <div className="flex-1 flex items-center gap-0 min-w-0">
        <div className="h-px flex-1 bg-white/10 relative">
          {progress != null && (<div className="absolute inset-y-0 left-0 bg-orange-500/60 rounded-full" style={{ width: `${progress}%` }}/>)}
        </div>
...
// ── MapShell ──────────────────────────────────────────────────────────────────
export function MapShell({ children, className = '', style }) {
    return (<div className={`relative rounded-xl overflow-hidden border border-white/[0.07] ${className}`} style={{ background: '#0a0f1c', ...style }}>
      {children}
    </div>);
}
export function TruckRow({ registration, driverName, isMyCargo, speedKmh, lastPing }) {
    return (<div className={`flex items-center justify-between px-3 py-2.5 rounded-lg border ${isMyCargo ? 'border-orange-500/30 bg-orange-500/5' : 'border-white/[0.06] bg-white/[0.02]'}`}>
      ...
      <div className="flex items-center gap-3 shrink-0">
        {speedKmh != null && (<span className="mono text-xs text-white/50">{Math.round(speedKmh)} km/h</span>)}
        {lastPing && (<span className="mono text-xs text-white/30">{relativeTime(lastPing)}</span>)}
...
export function Avatar({ name, size = 'md' }) {
    ...
    return (<div className={`${sz} rounded-full bg-orange-900/50 border border-orange-700/40 flex items-center justify-center shrink-0`}>
...
export function EscortPanel({ callsign, role, company, vehicleReg }) {
    return (<div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-blue-900/10 border border-blue-700/20">
...
// ── RosterItem ────────────────────────────────────────────────────────────────
export function RosterItem({ name, sub, badge }) {
...
export function LedgerEvent({ seq, kind, location, timestamp, officer, sealIntact, hash, verified, isLast }) {
...
export function BottomNav({ items }) {
    return (<nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/[0.07] flex safe-b" style={{ background: 'rgba(7,11,22,0.97)', backdropFilter: 'blur(12px)' }}>
      {items.map((item, i) => (<button key={i} onClick={item.onClick} className={`flex-1 flex flex-col items-center gap-1 py-3 transition-colors ${item.active ? 'text-orange-400' : 'text-white/40 hover:text-white/70'}`}>
          {item.icon}
