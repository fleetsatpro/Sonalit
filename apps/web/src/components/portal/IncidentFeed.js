...
function IncidentCard({ incident }) {
    ...
    return (<article className="rounded-xl border overflow-hidden portal-reveal" style={{
            background: 'var(--p-surface)',
            borderColor: isActive ? `${color}33` : 'rgba(255,255,255,0.07)',
        }}>
      {/* Header row */}
      <button className="w-full flex items-start gap-3 px-4 py-3 text-left" onClick={() => setExpanded(e => !e)} aria-expanded={expanded}>
        ...
        {/* Phase chip */}
        <span className="shrink-0 text-[10px] font-bold tracking-widest px-2 py-0.5 rounded" style={{
      ...
      {/* Expanded body */}
      {expanded && (<div className="px-4 pb-4 space-y-3 border-t border-white/[0.05]">
          <p className="text-xs text-white/60 pt-3 leading-relaxed">{incident.summary}</p>

          {/* Timeline */}
          <div className="space-y-2 mt-2">
            {incident.timeline.map((ev, idx) => (<div key={idx} className="flex items-start gap-2.5">
                <div className="flex flex-col items-center shrink-0 mt-0.5">
                  <div className="w-2 h-2 rounded-full" style={{ background: PHASE_COLOR[ev.phase] ?? '#ffffff40' }}/>
                  {idx < incident.timeline.length - 1 && (<div className="w-px flex-1 min-h-[14px]" style={{ background: 'rgba(255,255,255,0.1)' }}/>)}
                </div>
                <div className="pb-2">
...
export function IncidentFeed({ convoyId, onLevelChange }) {
    ...
    useEffect(() => {
        ...
        fetch(`${API}/portal/convoy/${encodeURIComponent(convoyId)}/incidents`, { credentials: 'include' })
            ...
            if (res.status === 403) {
                setErrorMsg('Not authorised for this convoy');
                return null;
            }
            if (!res.ok) {
    ...
    useEffect(() => {
        if (!convoyId)
            return;
        return subscribe(`portal#${convoyId}`, evt => {
    ...
    if (loading) {
        ...
        {[0, 1].map(i => (<div key={i} className="h-16 rounded-xl border border-white/[0.07] animate-pulse" style={{ background: 'var(--p-surface)', animationDelay: `${i * 80}ms` }}/>))}
      </div>);
    }
    if (errorMsg) {
        return (<div className="flex gap-3 p-4 rounded-xl border border-red-700/40 bg-red-900/20">
