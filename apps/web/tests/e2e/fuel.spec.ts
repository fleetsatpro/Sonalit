...
async function mockCommonRoutes(page: import('@playwright/test').Page) {
  ...
  await page.route(url => url.toString().includes('/api/v1/fuel/summary'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) })
  );
  await page.route(url => url.toString().includes('/api/v1/fuel/stations'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) })
  );
...
test.describe('Fuel Management — cross-tenant isolation (RULE B)', () => {
  ...
  test('direct GET of Org B fuel entry returns 404', async ({ page }) => {
    await page.route(`**/api/v1/fuel/${ORG_B_FUEL_ENTRY_ID}`, route =>
  ...
  test('driver leaderboard does not expose Org B driver rows', async ({ page }) => {
    ...
    await page.route(url => url.toString().includes('/api/v1/behaviour/leaderboard'), route =>
      route.fulfill({
        status: 200,
    ...
    const response = await page.evaluate(async () => {
      ...
      const body = await r.json() as { data: Array<{ driver_id: string; org_id: string }> };
      return body.data;
    });
