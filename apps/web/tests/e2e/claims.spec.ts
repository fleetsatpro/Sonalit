...
async function mockCommonRoutes(page: import('@playwright/test').Page) {
  ...
  await page.route(url => url.toString().includes('/api/v1/claims'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: ORG_A_CLAIMS }) })
  );
  await page.route(url => url.toString().includes('/api/v1/vehicles'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) })
  );
  await page.route(url => url.toString().includes('/api/v1/incidents'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) })
  );
}
...
test.describe('Claims — cross-tenant isolation', () => {
  ...
  test('direct GET of Org B claim returns 404', async ({ page }) => {
    await page.route(`**/api/v1/claims/${ORG_B_CLAIM_ID}`, route =>
  ...
  test('PATCH status on Org B claim returns 404', async ({ page }) => {
    await page.route(`**/api/v1/claims/${ORG_B_CLAIM_ID}/status`, route =>
      route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) })
    );
    const status = await page.evaluate(async (id) => {
