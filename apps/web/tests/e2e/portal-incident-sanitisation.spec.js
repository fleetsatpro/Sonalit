...
function stubSecurity(page) {
    return page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/security`), route => route.fulfill({
        status: 200,
...
function stubIncidents(page, incidents) {
    return page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/incidents`), route => route.fulfill({
        status: 200,
        contentType: 'application/json',
...
test.describe('§00 Incident Sanitisation', () => {
    ...
    test('status bar shows correct level colour class for secure convoy', async ({ page }) => {
        await page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/security`), route => route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                data: {
