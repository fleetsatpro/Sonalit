...
test.describe('Cargo Owner Control Tower', () => {
  ...
  test('shows real convoy and live tracking controls', async ({ page }) => {
    await page.goto('/cargo-portal');
    ...
    await expect(page.getByRole('button', { name: /Open shipment workspace/i })).toBeVisible({ timeout: 4000 });
  });

  test('client 360 is real-data backed and reflects active workspace count', async ({ page }) => {
    await page.goto('/cargo-portal');
    await page.getByRole('button', { name: /Client 360/i }).click();
    await expect(page.getByText('ACME Logistics')).toBeVisible({ timeout: 8000 });
    await expect(page.getByText('Active workspaces')).toBeVisible({ timeout: 4000 });
    await expect(page.getByText('1').first()).toBeVisible({ timeout: 4000 });
  });
});
...
test.describe('Portal UX — Track page', () => {
  test('track page shows live telemetry workspace', async ({ page }) => {
    ...
    await page.route(url => url.toString().includes('/api/v1/portal/convoy/conv-1/vehicles'), route =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [{ vehicle_id:'v1', registration:'NDE 123 GP', make:'Volvo', model:'FH', type:'truck', driver_name:'A. Mkhize', current_lat:-29.5, current_lng:30.5, speed_kmh:80, heading_deg:45, last_ping_at:new Date(Date.now()-60_000).toISOString(), carries_my_cargo:true }] }) }));
    await page.goto('/portal/convoy/conv-1/track');
    await expect(page.getByText('CVY-TRACK-001')).toBeVisible({ timeout: 8000 });
    await expect(page.getByText('LIVE CARGO ATLAS')).toBeVisible({ timeout: 8000 });
    await expect(page.getByText('NDE 123 GP')).toBeVisible({ timeout: 5000 });
  });
});
