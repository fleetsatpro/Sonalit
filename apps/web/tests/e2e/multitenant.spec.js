...
test.describe('Multi-tenant isolation — cross-tenant detail reads return 404', () => {
    ...
    test('direct API call to Org B vehicle UUID returns 404, not 403', async ({ page }) => {
        ...
        await page.route(`**/api/v1/vehicles/${ORG_B_VEHICLE_ID}`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
    ...
    test('direct API call to Org B convoy UUID returns 404', async ({ page }) => {
        await page.route(`**/api/v1/convoys/${ORG_B_CONVOY_ID}`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
        const response = await page.evaluate(async (id) => {
    ...
    test('direct API call to Org B driver UUID returns 404', async ({ page }) => {
        await page.route(`**/api/v1/drivers/${ORG_B_DRIVER_ID}`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
        const response = await page.evaluate(async (id) => {
...
test.describe('Multi-tenant isolation — cross-tenant mutations return 404 (not 403)', () => {
    ...
    test('PATCH convoy status on Org B convoy returns 404', async ({ page }) => {
        await page.route(url => url.toString().includes(`/api/v1/convoys/${ORG_B_CONVOY_ID}/`), route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
        const response = await page.evaluate(async (id) => {
    ...
    test('cross-tenant child assignment returns 422 with clean error', async ({ page }) => {
        ...
        await page.route(`**/api/v1/convoys/${ORG_A_CONVOY_ID}/assign`, route => route.fulfill({
            status: 422,
            contentType: 'application/json',
...
test.describe('Multi-tenant isolation — Centrifugo connection JWT', () => {
    ...
    test('connection token sub claim matches Org A user, not Org B', async ({ page }) => {
        ...
        if (capturedToken) {
            const decoded = jwtDecode(capturedToken);
...
test.describe('Multi-tenant isolation — API key scoping', () => {
    ...
    test('org-A API key cannot read org-B alerts', async ({ page }) => {
        await page.route(`**/api/v1/alerts/${ORG_B_ALERT_ID}`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
