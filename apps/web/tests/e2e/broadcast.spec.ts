...
async function mockCommonRoutes(page: import('@playwright/test').Page) {
  ...
  await page.route(url => url.toString().includes('/api/v1/canned-messages'), route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: CANNED_MESSAGES }) })
  );
}
...
test.describe('Broadcast — convoys list shows broadcast button for active convoys', () => {
  ...
  test('active convoy shows Radio broadcast button', async ({ page }) => {
    ...
    const broadcastBtn = page.locator('button[title="Broadcast"]').or(
...
test.describe('Broadcast — BroadcastPanel interactions', () => {
  ...
  test('successful send shows recipient count', async ({ page }) => {
    await page.route(url => url.toString().includes('/broadcast'), route =>
      route.fulfill({
        status: 201,
        contentType: 'application/json',
