...
async function mockCommonRoutes(page) {
    ...
    await page.route(url => url.toString().includes('/api/v1/shifts/on-duty'), route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: ON_DUTY }) }));
    await page.route(url => url.toString().includes('/api/v1/shifts/coverage'), route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) }));
    await page.route(url => url.toString().includes('/api/v1/shifts') && !url.toString().match(/on-duty|coverage|handover/), route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: ORG_A_SHIFTS }) }));
...
test.describe('Shifts — create new shift', () => {
    ...
    test('New Shift button opens modal', async ({ page }) => {
        const newBtn = page.getByRole('button', { name: /new shift|schedule/i }).or(page.getByText(/new shift/i).first());
...
test.describe('Shifts — cross-tenant isolation', () => {
    ...
    test('direct GET of Org B shift handover returns 404', async ({ page }) => {
        await page.route(`**/api/v1/shifts/${ORG_B_SHIFT_ID}/handover`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
    ...
    test('DELETE of Org B shift returns 404', async ({ page }) => {
        await page.route(`**/api/v1/shifts/${ORG_B_SHIFT_ID}`, route => route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'Not found' }) }));
        const status = await page.evaluate(async (id) => {
