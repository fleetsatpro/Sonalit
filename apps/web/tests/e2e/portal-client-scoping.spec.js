...
async function stubClientSession(page) {
    ...
    await page.route(url => url.toString().includes('/api/v1/portal/shipments'), route => route.fulfill({
        status: 200,
...
function stubConvoyX(page, endpoint) {
    page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/${endpoint}`), route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) }));
}
/** Stub convoy-y endpoint as 403 (server-enforced §00) */
function stubConvoyY403(page, endpoint) {
    return page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_Y}/${endpoint}`), route => route.fulfill({
        status: 403,
        contentType: 'application/json',
...
async function stubUnauthenticated(page) {
    await page.route(url => url.toString().includes('/api/v1/portal/'), route => route.fulfill({ status: 401, contentType: 'application/json', body: JSON.stringify({ error: 'Not authenticated' }) }));
}
// ── Tests ──────────────────────────────────────────────────────────────────────
test.describe('§00 Portal Client Scoping', () => {
    ...
    test('custody page shows chain-valid banner when chain is intact', async ({ page }) => {
        page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/custody`), route => route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                data: [{
    ...
    test('co-load count shown but other consignment detail never visible on convoy page', async ({ page }) => {
        page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/overview`), route => route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                data: {
