      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          data: [
            ...
            {
              convoy_id: 'convoy-stale-zero',
              reference: 'SHP-PORTAL-003',
              status: 'in_transit',
              origin: 'Mombasa',
              destination: 'Nairobi',
              eta: null,
              ...
              reports: [
                {
                  ...
                  received_photo_count: 10,
                  generated_at: new Date().toISOString(),
                  content_hash: 'a'.repeat(64),
                  download_path: `/portal/reports/${CONVOY_X}/2026-09-15/download`,
                },
              ],
              documents: [],
            },
...
test.describe('Portal UX — Client Workspace', () => {
  ...
  test('generated convoy report is arranged under its real convoy', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Open PDF', exact: true })).toHaveAttribute(
      'href',
      /portal\/reports\/.*\/2026-09-15\/download/,
    );
  });
});
    ...
    await page.route(
      ...
      (route) =>
        route.fulfill({
          ...
          body: JSON.stringify({
            data: [],
            meta: { chain_valid: true, event_count: 0 },
          }),
        }),
    );
