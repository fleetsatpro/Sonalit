...
function stubShipments(page) {
    return page.route(url => url.toString().includes('/api/v1/portal/shipments'), route => route.fulfill({
        status: 200,
...
test.describe('Portal UX — Track page', () => {
    test('track page shows ETA ribbon and deep-track header', async ({ page }) => {
        await page.route(url => url.toString().includes(`/api/v1/portal/convoy/${CONVOY_X}/overview`), route => route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                data: {
