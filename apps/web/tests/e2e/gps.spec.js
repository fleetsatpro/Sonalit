...
async function mockGpsRoutes(page) {
    await page.route(url => url.toString().includes('/api/v1/vehicles'), route => route.fulfill({
        status: 200,
    ...
    await page.route(url => url.toString().includes('/api/v1/dashboard/vehicles'), route => route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
            data: VEHICLES.map(v => ({
    ...
    await page.route(url => url.toString().includes('/api/v1/dashboard/convoys'), route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) }));
...
test.describe('GPS ingest — API contract', () => {
    ...
    test('invalid GPS fix POST (missing lat/lng) returns 422', async ({ page }) => {
        ...
        await page.route(url => url.toString().includes('/api/v1/gps/ingest'), route => route.fulfill({
            status: 422,
            contentType: 'application/json',
