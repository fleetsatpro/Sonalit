...
export function KPICard({ label, value, delta, trend, sparkline }: KPICardProps) {
  ...
  const color = trend === 'up' ? '#33d6a8' : '#ff5c5c';

  const sparkPath = sparkline?.map((v, i, arr) => {
    const x = (i / (arr.length - 1)) * 100;
    const y = 34 - (v / 100) * 32;
    return `${x},${Math.max(2, Math.min(32, y))}`;
  }).join(' ');

  const areaPath = sparkline && sparkPath
    ? `M0,34 L${sparkPath} L100,34 Z`
    : undefined;

  return (
    <div className="glass p-4 relative overflow-hidden group">
      <div className="font-mono text-2xs text-text-2 tracking-wide">{label}</div>
      <div className="font-display font-extrabold text-[28px] mt-1.5 text-text-0 tabular">{displayValue}</div>
      ...
      {sparkPath && (
        <svg className="absolute right-0 bottom-0 left-0 h-[34px] opacity-40 group-hover:opacity-60 transition-opacity" viewBox="0 0 100 34" preserveAspectRatio="none">
          {areaPath && <path d={areaPath} fill={color} opacity="0.15" />}
          <polyline points={sparkPath} fill="none" stroke={color} strokeWidth="1.8" />
        </svg>
      )}
    </div>
  );
