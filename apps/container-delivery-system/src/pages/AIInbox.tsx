...
const conversations: Conversation[] = [
  {
    id: 'c1', name: 'John Kamau', category: 'ground', initials: 'JK', color: '#ff7a00', phone: '+254 712 •• 214', online: true, status: 'review', unread: 2, time: '14:32', preview: 'Container TGHU3456789 clamped at Nakuru warehouse. Departing shortly.', confidence: 94,
    messages: [
    ...
    groups: [
      { label: 'Shipment', fields: [{ label: 'Container', value: 'TGHU 345678-9', confidence: 98 }, { label: 'Truck', value: 'KDK 456P', confidence: 97 }, { label: 'E-Lock', value: 'SSL-2218', confidence: 95 }, { label: 'Seal No.', value: 'KE-2024-88901', confidence: 92 }] },
      { label: 'Logistics', fields: [{ label: 'Origin', value: 'Nakuru WH', confidence: 96 }, { label: 'Destination', value: 'Mombasa Port', confidence: 88 }, { label: 'Weight', value: '24.5 tons', confidence: 94 }, { label: 'ETA', value: '18:40', confidence: 91 }] },
    ],
  },
  {
    id: 'c2', name: 'Grace Wanjiru', category: 'ground', initials: 'GW', color: '#33d6a8', phone: '+254 733 •• 021', online: true, status: 'pending', unread: 1, time: '13:55', preview: 'Checkpoint cleared at Gilgil. Proceeding to Naivasha for pickup.', confidence: 72,
    messages: [
    ...
    groups: [{ label: 'Shipment', fields: [{ label: 'Container', value: 'CMAU 558122-3', confidence: 90 }, { label: 'Truck', value: '', confidence: 0, missing: true }, { label: 'Checkpoint', value: 'Gilgil', confidence: 85 }, { label: 'Destination', value: 'Naivasha WH', confidence: 78 }] }],
  },
  {
    id: 'c3', name: 'Hassan Omar', category: 'port', initials: 'HO', color: '#ffb020', phone: '+254 722 •• 400', online: false, status: 'synced', unread: 0, time: '12:10', preview: 'Container MSCU7712340 unclamped and delivered to EPZ yard.', confidence: 98,
    messages: [
    ...
    groups: [{ label: 'Delivery', fields: [{ label: 'Container', value: 'MSCU 771234-0', confidence: 99 }, { label: 'Bay', value: 'Bay 3', confidence: 97 }, { label: 'Auth Code', value: 'UC-8834', confidence: 100 }, { label: 'Lock Returned', value: 'SSL-1190', confidence: 96 }] }],
  },
  {
    id: 'c4', name: 'Samuel Kiptoo', category: 'ground', initials: 'SK', color: '#ff5c5c', phone: '+254 701 •• 456', online: true, status: 'review', unread: 3, time: '14:45', preview: 'Engine warning light on. Stopped at Mtito Andei service center.', confidence: 65,
    messages: [
    ...
    groups: [{ label: 'Incident', fields: [{ label: 'Vehicle', value: 'KCE 771D', confidence: 95 }, { label: 'Container', value: 'HLXU 990331-2', confidence: 92 }, { label: 'Issue', value: 'Engine warning', confidence: 78 }, { label: 'Location', value: 'Mtito Andei', confidence: 88 }, { label: 'Delay', value: '2-3 hours', confidence: 65 }] }],
  },
];
...
export default function AIInbox() {
  ...
  return (
    <div className="p-6 pb-10 animate-fade-in h-[calc(100vh-64px)]">
      <PageHeader
        title="AI Inbox"
        description="WhatsApp intake · Auto-extraction · Field verification"
        actions={
      ...
      <div className="grid grid-cols-[280px_1fr_320px] gap-0 h-[calc(100%-48px)]">
        <Card className="p-0 rounded-r-none border-r border-hair overflow-y-auto">
          <div className="px-3 py-2.5 border-b border-hair">
            <SearchInput value={search} onChange={setSearch} placeholder="Search conversations..." className="w-full min-w-0" />
          </div>
          {filteredConversations.map((c) => (
            ...
            >
              <div className="w-[34px] h-[34px] rounded-lg flex items-center justify-center text-xs font-bold flex-none" style={{ background: `${c.color}22`, color: c.color }}>
                {c.initials}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className="text-xs-tight font-semibold text-text-0 truncate">{c.name}</span>
                  <span className="text-2xs font-mono text-text-2 flex-none ml-1">{c.time}</span>
                </div>
        ...
        <Card className="p-0 rounded-none flex flex-col">
          <div className="px-4 py-3 border-b border-hair flex items-center gap-3">
            <div className="w-[30px] h-[30px] rounded-lg flex items-center justify-center text-[11px] font-bold" style={{ background: `${selected.color}22`, color: selected.color }}>
              {selected.initials}
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            {selected.messages.map((m) => (
              <div key={m.id} className={`flex ${m.mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] px-3.5 py-2.5 rounded-xl text-xs-tight ${m.mine ? 'bg-cds-orange text-white rounded-br-md' : 'bg-ink-3 text-text-0 rounded-bl-md'}`}>
                  <div>{m.text}</div>
                  <div className={`text-2xs mt-1 ${m.mine ? 'text-white/60' : 'text-text-2'} font-mono text-right`}>{m.time}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="px-4 py-3 border-t border-hair">
            <div className="flex gap-2">
              <input type="text" placeholder="Type a reply..." className="flex-1 bg-ink-2 border border-glass-border rounded-lg px-3.5 py-2 text-xs-tight placeholder:text-text-2" />
              <Button>Send</Button>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-0 rounded-l-none border-l border-hair overflow-y-auto">
          <div className="px-4 py-3 border-b border-hair">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-2xs text-text-1">{selected.confidence}%</span>
              </div>
            </div>
            <div className="text-2xs text-text-2 mt-0.5 font-mono">
          ...
          <div className="px-4 py-3 space-y-4">
            {selected.groups.map((group) => (
              <div key={group.label}>
                <div className="text-2xs font-mono text-text-2 uppercase tracking-wider mb-2">{group.label}</div>
                <div className="space-y-2">
                  {group.fields.map((field) => (
                    <div key={field.label} className="flex items-center gap-2">
                      <ConfidenceRing value={field.confidence} size={22} />
                      <div className="flex-1 min-w-0">
                        <div className="text-2xs text-text-2">{field.label}</div>
                        {field.missing ? (
                          <div className="text-xs text-cds-red italic">Missing — ask driver</div>
                        ) : (
                          <div className="text-xs-tight text-text-0 font-mono">{field.value}</div>
                        )}
                      </div>
                      <span className="text-2xs font-mono text-text-2 flex-none">{field.confidence}%</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="px-4 py-3 border-t border-hair space-y-2">
            ...
            {selected.status === 'synced' && (
              <div className="text-center text-xs text-cds-teal font-medium py-2">Synced to CDS</div>
            )}
