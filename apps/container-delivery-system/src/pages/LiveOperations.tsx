...
export default function LiveOperations() {
  return (
    <div className="p-6 pb-10 animate-fade-in">
      <h2 className="font-display font-bold text-[17px] m-0">Live Operations</h2>
      <p className="text-[12px] text-text-2 m-0 mb-4 mt-1">Real-time truck positions, geofences, and route history across all active trips.</p>
      ...
      <div className="grid grid-cols-[1.8fr_1fr] gap-4 items-start">
        <Card className="h-[480px] p-0 relative overflow-hidden">
          <div className="w-full h-full relative" style={{
            ...
            <div className="absolute top-3.5 left-4 z-10">
              ...
              <div className="text-[10.5px] text-text-2 font-mono">{activeTrips.length} active · geofence: Mariakani, Voi, Mtito Andei</div>
            </div>
            ...
            <div className="absolute w-[9px] h-[9px] rounded-sm bg-cds-teal shadow-[0_0_10px_rgba(51,214,168,0.6)]" style={{ left: '92%', top: '88%' }} />
        ...
        <Card className="max-h-[480px] overflow-y-auto p-4">
          ...
          {activeTrips.map((t) => (
            <div key={t.id} className="flex gap-2.5 py-2.5 border-b border-[rgba(255,255,255,0.07)] last:border-b-0 cursor-pointer hover:bg-ink-2 rounded-lg px-2 -mx-2 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[12.5px] text-text-0">{t.id} <span className="text-text-2">— {t.truck}</span></div>
                <div className="text-[10.5px] text-text-2 mt-0.5 font-mono">{t.origin} → {t.dest} · ETA {t.eta}</div>
              </div>
