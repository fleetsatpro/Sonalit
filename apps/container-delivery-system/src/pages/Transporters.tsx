  ...
  return (
    <div className="p-6 pb-10 animate-fade-in">
      <div className="flex items-center justify-between mb-1">
        <div>
          <h2 className="font-display font-bold text-[17px] m-0">Transporters</h2>
          <p className="text-[12px] text-text-2 m-0 mt-1">Performance across {transporters.length} contracted transport companies.</p>
        </div>
      ...
      <Card className="p-0 mt-4">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-[12.5px]">
            ...
            <tbody>
              {transporters.map((t) => (
                <tr key={t.name} className="border-t border-[rgba(255,255,255,0.07)] cursor-pointer hover:bg-ink-2 transition-colors">
                  <td className="px-3.5 py-3 text-text-0 font-semibold">{t.name}</td>
                  <td className="px-3.5 py-3 text-text-0">{t.contact}<div className="text-[10.5px] text-text-2 mt-0.5">{t.phone}</div></td>
                  <td className="px-3.5 py-3 font-mono text-text-0">{t.trucks}</td>
                  <td className="px-3.5 py-3">
                    <div className="flex items-center gap-2">
                      <ProgressBar value={t.onTime} width="w-[70px]" color={t.onTime >= 90 ? 'bg-cds-teal' : t.onTime >= 80 ? 'bg-cds-amber' : 'bg-cds-red'} />
                      <span className="font-mono text-text-1">{t.onTime}%</span>
                    </div>
                  </td>
