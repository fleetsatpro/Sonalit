...
import { deviceAPI, vehiclesAPI } from '../services/api';
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[['TOTAL',devices.length,'text-slate-300'],['ONLINE',devices.filter(d=>d.status==='active').length,'text-success'],['OFFLINE',devices.filter(d=>d.status!=='active').length,'text-danger'],['LOW BATTERY',devices.filter(d=>(d.battery||100)<20).length,'text-amber-400']].map(([l,v,c])=>(
      ...
      <Card>
        ...
        : devices.length===0 ? <EmptyState icon={Cpu} title="No devices registered" subtitle="Register a GPS device to start tracking" action={<Button size="sm" onClick={()=>setModal(true)}>Register Device</Button>}/>
