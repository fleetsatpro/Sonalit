...
export default function AlertsPage() {
  ...
  const acknowledge = async (id) => {
    try { await alertsAPI.acknowledge(id); setAlerts(prev => prev.filter(a => a.id !== id)); setSelected(null); }
    catch (e) { console.error(e); }
  };

  const escalate = async (id) => {
    try { await alertsAPI.update(id, { severity: 'critical', escalated: true }); load(true); setSelected(null); }
    catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[['Critical', counts.critical||0, 'text-red-400', 'bg-red-500/10 border-red-500/20'],
          ['High',     counts.high||0,     'text-amber-400','bg-amber-500/10 border-amber-500/20'],
          ['Medium',   counts.medium||0,   'text-blue-400', 'bg-blue-500/10 border-blue-500/20'],
          ['Low',      counts.low||0,      'text-success',  'bg-success/10 border-success/20'],
        ].map(([l,v,c,bg]) => (
      ...
      <Card>
        {loading
          ...
          : alerts.length === 0
            ? <EmptyState icon={CheckCircle2} title="No active alerts" subtitle="All clear — no unresolved incidents" />
            : (
                  ...
                  <div key={a.id} onClick={() => setSelected(a.id)}
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={e => { e.stopPropagation(); acknowledge(a.id); }}
                        className="px-2.5 py-1 bg-success/10 border border-success/20 rounded-lg text-xs font-mono text-success hover:bg-success/20 transition-all">ACK</button>
                      <button onClick={e => { e.stopPropagation(); escalate(a.id); }}
                        className="px-2.5 py-1 bg-danger/10 border border-danger/20 rounded-lg text-xs font-mono text-danger hover:bg-danger/20 transition-all">ESC</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sel && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Badge severity={sel.severity} dot>{sel.severity?.toUpperCase()}</Badge>
              <span className="text-xs font-mono text-slate-500">{sel.type}</span>
            </div>
