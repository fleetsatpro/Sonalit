    ...
    <div className="space-y-5 max-w-[1600px]">
      ...
      <Card>
        {loading
          ...
          : incidents.length === 0
            ? <EmptyState icon={Shield} title="No incidents" subtitle="Create one to get started" />
            : (
            ...
            <div className="flex gap-2 flex-wrap">
              {STATUSES.map(s => (
                <button key={s} onClick={() => updateStatus(sel.id, s)}
                  className={`px-3 py-1 text-xs font-mono rounded-lg border transition-all ${sel.status===s?'bg-gold/10 border-gold/30 text-gold':'border-white/5 text-slate-500 hover:border-white/10'}`}>
                  {s.toUpperCase()}
                </button>
              ))}
            </div>
            ...
            <div className="border-t border-white/5 pt-4">
              ...
              <div className="flex gap-2">
                <input value={comment} onChange={e => setComment(e.target.value)} placeholder="Add a note..."
                  className="flex-1 bg-navy-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-slate-200 outline-none focus:border-gold/50" />
