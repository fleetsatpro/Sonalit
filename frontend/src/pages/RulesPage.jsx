...
import { Zap, Plus, Toggle, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-5 max-w-[1600px]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-xs font-mono mt-0.5">IF condition THEN action · {rules.filter(r=>r.enabled).length} active rules</p>
        </div>
      ...
      <Card>
        ...
        : rules.length===0 ? <EmptyState icon={Zap} title="No automation rules" subtitle="Create rules to automate alerts and actions" action={<Button size="sm" onClick={()=>setModal(true)}>Create Rule</Button>}/>
            ...
            {rules.map(r=>(
              <div key={r.id} className="px-5 py-4 flex items-center gap-4 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={()=>remove(r.id)} className="p-1.5 rounded-lg hover:bg-danger/10 text-slate-600 hover:text-danger transition-colors"><Trash2 size={13}/></button>
                </div>
              </div>
            ))}
          </div>}
