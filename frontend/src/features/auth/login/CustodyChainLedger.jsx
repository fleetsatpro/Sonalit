...
export default function CustodyChainLedger() {
  const reducedMotion = useReducedMotion();
  const [blocks, setBlocks] = useState(() =>
    Array.from({ length: 8 }, (_, i) => makeBlock(i))
  );
  const idxRef = useRef(8);
  ...
  const height = useMemo(
    () => (START_HEIGHT + idxRef.current).toLocaleString(),
    [blocks]
  );
        ...
        {blocks.map((b) => (
          <div key={b.id} className={'block' + (b.e.w ? ' warn' : '')}>
            ...
            <div className="block-card">
              ...
              {/* eslint-disable-next-line react/no-danger -- redaction spans are static & sanitized above */}
